...
describe("ProfileService", () => {
  ...
  it("creates the minimal local custom-craft profile and preserves its first timestamp", async () => {
    ...
    expect(first).toMatchObject({ ok: true, profile: { activityName: "مشغل ليان", currency: "JOD", activityType: "custom_craft", createdAt: "2026-08-22T00:00:00.000Z" } });
    expect(second).toMatchObject({ ok: true, profile: { activityName: "مشغل ليان للحرف", createdAt: "2026-08-22T00:00:00.000Z", updatedAt: "2026-08-22T01:00:00.000Z" } });
  });
});
