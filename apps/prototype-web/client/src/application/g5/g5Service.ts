import { convertQuantityMilli, type CatalogItem, type DirectConversion, type MeasurementUnit } from "@micro-domain/catalog/index.js";
...
function toQuantityMilli(quantity: number): number | null {
  ...
  if (!Number.isSafeInteger(scaled) || scaled <= 0 || Math.abs(quantity - scaled / 1000) > Number.EPSILON) return null;
  return scaled;
}

function normalizeQuantity(quantityMilli: number, sourceUnitId: string, targetUnitId: string, units: readonly MeasurementUnit[], conversions: readonly DirectConversion[]): number | null {
  ...
  const conversion = conversions.find((candidate) => candidate.active && candidate.fromUnitId === sourceUnitId && candidate.toUnitId === targetUnitId);
  if (!source || !target || source.dimension !== target.dimension || !conversion || conversion.dimension !== source.dimension) return null;
  try { return convertQuantityMilli(quantityMilli, conversion).quantityMilli; } catch { return null; }
}

function orderInputs(orders: readonly StoredCraftOrder[], catalogItems: readonly CatalogItem[], units: readonly MeasurementUnit[], conversions: readonly DirectConversion[], from: string, to: string): G5OrderInput[] {
  const periodOrders = orders.flatMap(({ order, catalogItemId }) => {
    const delivered = deliveredOn({ id: order.id, order, catalogItemId, deliveryDate: "", agreementSource: null, createdAt: order.createdAt, updatedAt: order.createdAt });
    if (!delivered || !inPeriod(delivered, from, to)) return [];
    return [{ order, catalogItemId, delivered }];
  });
  const organizedUnits = periodOrders.filter(({ order }) => order.resultStatus === "final").map(({ catalogItemId }) => catalogItemId ? catalogItems.find((item) => item.id === catalogItemId)?.unitId ?? null : null);
  ...
  return periodOrders.map(({ order, catalogItemId, delivered }, index) => {
    ...
    if (catalogItemId) {
      ...
      unitLabel = catalog?.unitLabel ?? (sourceUnitId ? units.find((unit) => unit.id === sourceUnitId)?.nameAr ?? null : null);
      if (!catalog || !sourceUnitId || !units.some((unit) => unit.id === sourceUnitId)) {
        ...
        quantityMilli = rawQuantityMilli === null ? null : normalizeQuantity(rawQuantityMilli, sourceUnitId, targetUnitId, units, conversions);
        if (quantityMilli === null) quantityIssue = rawQuantityMilli === null ? "invalid" : "needs_conversion";
        else { unitKey = targetUnitId; unitLabel = catalogItems.find((item) => item.unitId === targetUnitId)?.unitLabel ?? units.find((unit) => unit.id === targetUnitId)?.nameAr ?? null; }
      }
    }
    return { id: order.id, itemName: order.itemName, deliveredOn: delivered, resultStatus: order.resultStatus, quantityMilli, unitKey, unitLabel, quantityIssue, recognizedRevenueMinor: order.recognizedRevenueMinor, recognizedCostMinor: order.recognizedCostMinor };
  });
}

function expenseInputs(events: readonly FinancialEvent[], from: string, to: string): G5ExpenseInput[] {
  return events.filter((event) => inPeriod(event.occurredOn, from, to) && (event.operatingExpenseDeltaMinor > 0 || event.expenseContext?.sharedProjectShare?.allocation === "unallocated")).map((event) => {
    const share = event.expenseContext?.sharedProjectShare;
    const unallocatedShared = share?.allocation === "unallocated";
    return {
      id: event.id,
      amountMinor: unallocatedShared ? share.totalAmountMinor ?? event.amountMinor : event.operatingExpenseDeltaMinor,
      ...
      sharedProjectShareBasis: event.expenseContext?.relationship === "shared" ? share?.basis ?? null : null,
...
function payables(events: readonly FinancialEvent[], purchases: readonly SupplierPurchase[]) {
  ...
  for (const event of events) if (event.type === "payable_settlement_cash" && event.relatedEventId && event.correctionType !== "reverse") settlements.set(event.relatedEventId, (settlements.get(event.relatedEventId) ?? 0) + event.amountMinor);
  const eventPayables = events.filter((event) => event.type === "operating_expense_payable" && event.payableDeltaMinor > 0 && !events.some((candidate) => candidate.correctionType === "reverse" && candidate.correctionOfEventId === event.id)).flatMap((event) => {
...
export class G5Service {
  ...
  async listLinkOptions(): Promise<G5Result<G5LinkOptions>> {
    ...
    if (!orders.ok || !events.ok) return { ok: false, code: "storage_error", message: "تعذر قراءة الأرصدة القابلة للربط." };
    const paidByEvent = new Map<string, number>();
    for (const event of events.value) if (event.type === "payable_settlement_cash" && event.relatedEventId && event.correctionType !== "reverse") paidByEvent.set(event.relatedEventId, (paidByEvent.get(event.relatedEventId) ?? 0) + event.amountMinor);
    const payableEvents = events.value.filter(event => event.type === "operating_expense_payable" && event.payableDeltaMinor > 0 && !events.value.some(candidate => candidate.correctionType === "reverse" && candidate.correctionOfEventId === event.id)).flatMap(event => {
    ...
    return { ok: true, value: { orders: orders.value.filter(({ order }) => order.receivableMinor > 0).map(({ order }) => ({ id: order.id, label: `${order.customerName} · ${order.itemName}`, amountMinor: order.receivableMinor })), payableEvents } };
  }

  async readDecision(from: string, to: string): Promise<G5Result<G5Decision>> {
    const [position, orders, events, purchases, declarations, catalogItems, units, conversions] = await Promise.all([this.projectFinance.readPosition(), this.store.listOrders(), this.store.listFinancialEvents(), this.store.listSupplierPurchases(), this.store.listShortCashDeclarations(), this.store.listCatalogItems(), this.store.listMeasurementUnits(), this.store.listDirectConversions()]);
    if (!position.ok || !orders.ok || !events.ok || !purchases.ok || !declarations.ok || !catalogItems.ok || !units.ok || !conversions.ok) return { ok: false, code: "storage_error", message: "تعذر قراءة بيانات G5 المحلية." };
    const contributionOrders = orderInputs(orders.value, catalogItems.value, units.value, conversions.value, from, to);
    const period = calculateBreakEven(from, to, contributionOrders, expenseInputs(events.value, from, to));
    const shortCash = calculateShortCash({ from, to, recordedCashMinor: position.value.recordedCashMinor, receivables: receivables(orders.value), payables: payables(events.value, purchases.value), declarations: declarations.value });
    return { ok: true, value: { period, shortCash, declarations: declarations.value, truth: "هذه قراءة مشتقة من السجل المحلي وإعلانات المالك. لا تحفظ نتيجة مالية جديدة، ولا تحول الإعلان إلى قبض أو دفع فعلي، ولا تقدم توصية ملزمة." } };
  }
  ...
  async reverseDeclaration(idToReverse: string, note: string, idempotencyKey: string): Promise<G5Result<ShortCashDeclaration>> {
    ...
    try {
      ...
      return saved.ok ? { ok: true, value: saved.value, reused: saved.value.id !== reversal.id } : { ok: false, code: "storage_error", message: "تعذر حفظ تصحيح إعلان السيولة ذريًا؛ بقي الأصل محفوظًا." };
    } catch (error) {
  ...
  private async validateRelation(input: G5DeclarationInput, declarations: readonly ShortCashDeclaration[]): Promise<G5Result<null>> {
    ...
    if (input.relatedEventId) {
      if (input.direction !== "commitment") return { ok: false, code: "validation_error", message: "ربط الحدث مخصص لالتزامات المصروف فقط." };
      ...
      if (events.value.some((candidate) => candidate.correctionType === "reverse" && candidate.correctionOfEventId === event.value!.id)) return { ok: false, code: "validation_error", message: "لا يمكن ربط إعلان بالتزام مالي عُكس." };
      const paid = events.value.filter((candidate) => candidate.type === "payable_settlement_cash" && candidate.relatedEventId === event.value!.id && candidate.correctionType !== "reverse").reduce((sum, candidate) => sum + candidate.amountMinor, 0);
      const alreadyDeclared = activeLinkedDeclarationTotal(declarations, input);
