...
export class PreferenceService {
  ...
  async save(theme: ThemePreference): Promise<PreferenceResult> { const result = await this.store.savePreferences({ id: localPreferencesId, theme, updatedAt: this.now() }); return result.ok ? { ok: true, preference: result.value.theme } : { ok: false, code: "storage_error", message: "تعذر حفظ تفضيل المظهر المحلي." }; }
}
