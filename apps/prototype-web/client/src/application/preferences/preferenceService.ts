...
export class PreferenceService {
  ...
  async save(theme: ThemePreference): Promise<PreferenceResult> { const current = await this.store.getPreferences(); if (!current.ok) return { ok: false, code: "storage_error", message: "تعذر قراءة تفضيل المظهر المحلي." }; const result = await this.store.savePreferences({ id: localPreferencesId, theme, dailyScheduleCapacityMinutes: current.value?.dailyScheduleCapacityMinutes ?? null, updatedAt: this.now() }); return result.ok ? { ok: true, preference: result.value.theme } : { ok: false, code: "storage_error", message: "تعذر حفظ تفضيل المظهر المحلي." }; }
}
