  ...
  materials: [
    {
      ...
      name: "طحين",
      unit: "kilogram",
      openingQuantityMilli: 2500,
      openingValueMinor: 8750,
      source: "جرد",
      knowledge: "estimated",
      occurredOn: "2026-08-01",
      note: "تقدير آخر شراء",
...
describe("GuidedOpeningImportService", () => {
  it("previews then atomically confirms a valid opening position", async () => {
    ...
    const preview = await service.prepare(JSON.stringify(valid));
    expect(preview).toMatchObject({
      ok: true,
      value: {
        summary: { acceptedWallets: 1, acceptedMaterials: 1, acceptedCashMinor: 12500, estimatedRecords: 1 },
      },
    });
    if (!preview.ok) return;
    await expect(store.readSnapshot()).resolves.toMatchObject({
      ok: true,
      value: { profile: null, cashWallets: [] },
    });
    await expect(service.confirm(preview.value)).resolves.toMatchObject({
      ok: true,
      value: { acceptedWallets: 1 },
    });
    await expect(store.readSnapshot()).resolves.toMatchObject({
      ok: true,
      value: {
        profile: { activityName: "مخبز صغير" },
        cashWallets: [{ name: "درج" }],
  ...
  it("rejects a non-empty store and does not overwrite it", async () => {
    ...
    const result = await new GuidedOpeningImportService(store).prepare(JSON.stringify(valid));
    expect(result).toMatchObject({ ok: false, code: "non_empty_store" });
    await expect(store.getProfile()).resolves.toMatchObject({ ok: true, value: { activityName: "قديم" } });
  });
  ...
  it("is idempotent when the same import is retried", async () => {
    ...
    await service.confirm(preview.value);
    const retry = await service.prepare(JSON.stringify(valid));
    expect(retry).toMatchObject({ ok: true, reused: true });
    const confirmed = await service.confirm(preview.value);
    expect(confirmed).toMatchObject({ ok: true, reused: true });
