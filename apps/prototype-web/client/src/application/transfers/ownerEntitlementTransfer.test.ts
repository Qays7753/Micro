...
import { createOwnerEntitlementPolicy, createOwnerEntitlementPolicySuccessor, createOwnerEntitlementRecord } from "@micro-domain/owner-entitlement/index.js";
...
describe("O1 local transfer boundary", () => {
  ...
  it("round-trips changed successor terms and rejects a broken policy chain without writing", async () => {
    const source = new MemoryLocalStore(); await source.saveOwnerEntitlementPolicy(policy); await source.saveOwnerEntitlementRecord(record);
    const ended = createOwnerEntitlementPolicy({ ...policy, status: "ended", endsOn: "2026-08-31" });
    const successor = createOwnerEntitlementPolicySuccessor({ id: "policy-transfer-successor", version: 2, kind: "sale_percentage", amountMinor: null, percentageBps: 1250, unitLabel: null, startsOn: "2026-09-01", endsOn: null, source: "تعديل نسبة", note: "خليفة متغيرة", status: "active", idempotencyKey: "policy-transfer-successor", createdAt: "2026-09-01T08:00:00.000Z", seriesId: policy.seriesId, successorOfPolicyId: policy.id });
    await expect(source.commitOwnerEntitlementPolicySuccessor(ended, successor)).resolves.toMatchObject({ ok: true });
    const exported = await new LocalTransferService(source).createExport(); if (!exported.ok) throw new Error("export should succeed");
    const target = new MemoryLocalStore(); const transfers = new LocalTransferService(target); const preview = transfers.prepareImport(JSON.stringify(exported.value)); if (!preview.ok) throw new Error(preview.message);
    expect(preview.value.file.data.ownerEntitlementPolicies).toEqual(expect.arrayContaining([expect.objectContaining({ id: successor.id, kind: "sale_percentage", family: "completed_sale_percentage", percentageBps: 1250 })]));
    await expect(transfers.confirmImport(preview.value)).resolves.toMatchObject({ ok: true });
    const broken = structuredClone(exported.value); const brokenSuccessor = broken.data.ownerEntitlementPolicies?.find(candidate => candidate.id === successor.id); if (!brokenSuccessor) throw new Error("successor should be exported"); brokenSuccessor.successorOfPolicyId = "missing-policy";
    const seededTarget = new MemoryLocalStore(); await seededTarget.saveOwnerEntitlementPolicy(policy); expect(new LocalTransferService(seededTarget).prepareImport(JSON.stringify(broken))).toMatchObject({ ok: false, code: "validation_error" }); await expect(seededTarget.listOwnerEntitlementPolicies()).resolves.toMatchObject({ ok: true, value: [{ id: policy.id }] });
  });
  ...
  it("accepts the previous O1 schema pair and initializes absent O1 arrays empty", async () => {
    ...
    expect(new LocalTransferService(new MemoryLocalStore()).prepareImport(JSON.stringify(previous))).toMatchObject({ ok: true, value: { file: { version: 17, schemaVersion: 26, data: { ownerEntitlementPolicies: [], ownerEntitlementRecords: [], ownerEntitlementOpeningBalances: [], ownerMovements: [], allocationPolicies: [] } } } });
  });
});
