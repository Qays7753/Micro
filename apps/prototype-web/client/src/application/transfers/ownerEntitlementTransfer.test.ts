...
describe("O1 local transfer boundary", () => {
  ...
  it("accepts the previous O1 schema pair and initializes absent O1 arrays empty", async () => {
    ...
    expect(new LocalTransferService(new MemoryLocalStore()).prepareImport(JSON.stringify(previous))).toMatchObject({ ok: true, value: { file: { version: 13, schemaVersion: 22, data: { ownerEntitlementPolicies: [], ownerEntitlementRecords: [], ownerEntitlementOpeningBalances: [], ownerMovements: [] } } } });
  });
});
