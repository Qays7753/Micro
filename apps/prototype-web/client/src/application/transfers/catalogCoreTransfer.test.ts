...
describe("G4-A catalog transfer boundary", () => {
  it("round-trips units, a direct conversion, and a ready-yield template without financial or inventory effects", async () => {
    ...
    const kg = await catalog.createUnit({ nameAr: "كيلوغرام", dimension: "mass", operationKey: "unit:kg:transfer" });
    const gram = await catalog.createUnit({ nameAr: "غرام", dimension: "mass", operationKey: "unit:g:transfer" });
    if (!kg.ok || !gram.ok) throw new Error("units should be created");
    await catalog.createConversion({ fromUnitId: kg.unit.id, toUnitId: gram.unit.id, numerator: 1000, denominator: 1, note: "1 كيلوغرام = 1000 غرام", operationKey: "conversion:kg-g:transfer" });
    const item = await catalog.create({ kind: "product", name: "خلطة", unitLabel: "غرام", unitId: gram.unit.id, operationKey: "catalog:mix:transfer" });
    if (!item.ok) throw new Error("item should be created");
    await catalog.createTemplate({ catalogItemId: item.item.id, title: "دفعة معتادة", note: null, components: [{ id: "c-1", name: "مادة", quantityMilli: 1000, unitId: kg.unit.id, note: null }], yield: { quantityMilli: 1000, unitId: kg.unit.id }, operationKey: "template:mix:transfer" });
    ...
    expect(preview.value.summary).toMatchObject({ catalogItems: 1, measurementUnits: 2, directConversions: 1, catalogTemplates: 1, financialEvents: 0, inventoryMovements: 0 });
    expect(preview.value.file.data.catalogTemplates).toEqual([expect.objectContaining({ yieldReadiness: "ready" })]);
    await expect(transfers.confirmImport(preview.value)).resolves.toMatchObject({ ok: true, value: { catalogTemplates: 1 } });
    const targetSnapshot = await target.readSnapshot();
    if (!targetSnapshot.ok) throw new Error("target snapshot should read");
    expect(targetSnapshot.value.measurementUnits).toEqual(expect.arrayContaining([expect.objectContaining({ nameAr: "غرام" }), expect.objectContaining({ nameAr: "كيلوغرام" })]));
    expect(targetSnapshot.value.catalogTemplates).toEqual([expect.objectContaining({ yieldReadiness: "ready" })]);
  ...
  it("rejects a tampered ready yield when its explicit conversion is missing, before writing anything", async () => {
    const source = new MemoryLocalStore();
    const catalog = new CatalogService(source, () => now);
    const unit = await catalog.createUnit({ nameAr: "قطعة", dimension: "count", operationKey: "unit:piece:tamper" });
    if (!unit.ok) throw new Error("unit should be created");
    const item = await catalog.create({ kind: "product", name: "صندوق", unitLabel: "قطعة", unitId: unit.unit.id, operationKey: "catalog:box:tamper" });
    if (!item.ok) throw new Error("item should be created");
    await catalog.createTemplate({ catalogItemId: item.item.id, title: "قالب", note: null, components: [], yield: { quantityMilli: 1000, unitId: unit.unit.id }, operationKey: "template:box:tamper" });
    const exported = await new LocalTransferService(source).createExport();
    if (!exported.ok) throw new Error("export should succeed");
    const tampered = structuredClone(exported.value);
    tampered.data.catalogTemplates = tampered.data.catalogTemplates?.map(template => ({ ...template, yieldReadiness: "ready", yield: { quantityMilli: 1000, unitId: "missing-unit" } }));
    const target = new MemoryLocalStore();
    const transfers = new LocalTransferService(target);
  ...
  it("migrates the previous catalog schema by adding empty G4-A arrays and a null organized unit", async () => {
    const source = new MemoryLocalStore();
    const catalog = new CatalogService(source, () => now);
    const item = await catalog.create({ kind: "service", name: "تغليف", unitLabel: "جلسة", operationKey: "catalog:wrap:migrate" });
    if (!item.ok) throw new Error("item should be created");
    const exported = await new LocalTransferService(source).createExport();
    if (!exported.ok) throw new Error("export should succeed");
    ...
    expect(preview).toMatchObject({ ok: true, value: { file: { version: 17, schemaVersion: 26, data: { catalogItems: [{ id: item.item.id, unitId: null }], measurementUnits: [], directConversions: [], catalogTemplates: [], allocationPolicies: [] } } } });
  });
});
