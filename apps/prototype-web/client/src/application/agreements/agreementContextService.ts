...
export type AgreementContextView = Pick<
  StoredCraftOrder,
  "id" | "agreementSource" | "followUpSummary" | "followUpDate" | "followUpReason" | "followUpEvents"
>;
...
const asView = (stored: StoredCraftOrder): AgreementContextView => ({
  id: stored.id,
  agreementSource: stored.agreementSource ?? null,
  followUpSummary: stored.followUpSummary ?? null,
  followUpDate: stored.followUpDate ?? null,
  followUpReason: stored.followUpReason ?? null,
  followUpEvents: stored.followUpEvents ?? [],
});
...
function validateInput(input: AgreementContextInput, previousDate: string | null) {
  if (input.agreementSource !== null && !sources.has(input.agreementSource))
    return failure("اختر مصدر اتفاق من القائمة أو اتركه غير محدد.");
  if (
    input.followUpSummary !== null &&
    (input.followUpSummary.trim().length < 2 || input.followUpSummary.trim().length > 240)
  )
    return failure("اكتب ملخص متابعة قصيرًا من حرفين إلى 240 حرفًا، أو اتركه فارغًا.");
  if (input.followUpDate !== null && !isValidLocalDate(input.followUpDate))
    return failure("أدخل تاريخ متابعة محليًا صحيحًا بصيغة YYYY-MM-DD.");
  const dateChanged = input.followUpDate !== previousDate;
  if (input.followUpDate === null && previousDate === null && input.followUpReason !== null)
    return failure("سبب المتابعة لا يحفظ دون تاريخ متابعة.");
  if (input.followUpDate !== null && previousDate === null && !input.followUpReason)
    return failure("اكتب هدفًا أو سببًا قصيرًا لموعد المتابعة.");
  if (dateChanged && previousDate !== null && !input.followUpReason)
    return failure("تغيير تاريخ متابعة موجود يحتاج سببًا مكتوبًا.");
  if (
    input.followUpReason !== null &&
    (input.followUpReason.trim().length < 2 || input.followUpReason.trim().length > 160)
  )
    return failure("اكتب سبب متابعة قصيرًا من حرفين إلى 160 حرفًا.");
  return null;
}
...
export class AgreementContextService {
  ...
  async save(id: string, input: AgreementContextInput): Promise<AgreementContextResult<StoredCraftOrder>> {
    ...
    const next: AgreementContextInput = {
      agreementSource: input.agreementSource,
      followUpSummary: normalized(input.followUpSummary),
      followUpDate: normalized(input.followUpDate),
      followUpReason: normalized(input.followUpReason),
    };
    const invalid = validateInput(next, previousDate);
    if (invalid) return invalid;
    const previousSummary = stored.followUpSummary ?? null;
    const previousSource = stored.agreementSource ?? null;
    const previousReason = stored.followUpReason ?? null;
    const effectiveReason = next.followUpDate === null ? null : (next.followUpReason ?? previousReason);
    if (
      previousSource === next.agreementSource &&
      previousSummary === next.followUpSummary &&
      previousDate === next.followUpDate &&
      previousReason === effectiveReason
    )
    ...
    if (previousDate !== next.followUpDate) {
      const event: FollowUpEvent = {
        id: `${id}:follow-up:${events.length + 1}`,
        type: previousDate === null ? "created" : "changed",
        idempotencyKey: `${id}:follow-up:${previousDate ?? "none"}:${next.followUpDate ?? "none"}`,
        createdAt: timestamp,
        previousDate,
        followUpDate: next.followUpDate,
        reason: next.followUpReason ?? "تحديث سياق المتابعة",
      };
      events.push(event);
    }
    const updated: StoredCraftOrder = {
      ...stored,
      agreementSource: next.agreementSource,
      followUpSummary: next.followUpSummary,
      followUpDate: next.followUpDate,
  ...
  async dueFollowUps(): Promise<AgreementContextResult<FollowUpRead>> {
    ...
    return {
      ...
      value: {
        due: withDate
          .filter(stored => (stored.followUpDate ?? "") <= today)
