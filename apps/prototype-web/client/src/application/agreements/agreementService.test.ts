...
describe("AgreementService", () => {
  it("creates a provisional agreement and records deposit as collected cash, not profit", async () => {
    const { store, costs, draft } = await preparedDraft();
    const service = new AgreementService(store, costs, () => "2026-08-22T01:00:00.000Z");
    const result = await service.createFromDraft(draft, {
    ...
    await expect(
      store.getSchedule(`schedule-${result.ok ? result.stored.id : "missing"}`),
    ).resolves.toMatchObject({
      ok: true,
      value: {
        orderId: result.ok ? result.stored.id : "missing",
  ...
  it("returns the existing local agreement on retry instead of duplicating events or cash", async () => {
    const { store, costs, draft } = await preparedDraft();
    const service = new AgreementService(store, costs, () => "2026-08-22T01:00:00.000Z");
    const first = await service.createFromDraft(draft, {
      agreedPriceMinor: 2200,
      deliveryDate: "2026-08-30",
      depositMinor: 500,
    ...
    const linked = await store.getDraft(draft.id);
    if (!linked.ok || !linked.value) throw new Error("linked draft should exist");
    const repeated = await service.createFromDraft(linked.value, {
      agreedPriceMinor: 2200,
      deliveryDate: "2026-08-30",
      depositMinor: 500,
      agreementSource: null,
    ...
    expect(repeated).toMatchObject({
      ok: true,
      stored: { id: first.stored.id, order: { collectedMinor: 500, events: first.stored.order.events } },
    });
  });
  ...
  it("requires a valid agreement and starts execution without adding a second cash event", async () => {
    ...
    await expect(
      service.createFromDraft(draft, {
        agreedPriceMinor: 1000,
        deliveryDate: "",
        depositMinor: 0,
        agreementSource: null,
      }),
    ).resolves.toMatchObject({ ok: false, code: "validation_error" });
    const agreed = await service.createFromDraft(draft, {
      agreedPriceMinor: 2200,
      deliveryDate: "2026-08-30",
      depositMinor: 500,
      agreementSource: null,
    ...
    expect(started).toMatchObject({
      ok: true,
      stored: { order: { status: "in_progress", collectedMinor: 500, depositCollectedMinor: 500 } },
    });
    await expect(service.startExecution(agreed.stored.id)).resolves.toMatchObject({
      ok: true,
      stored: { order: { events: started.ok ? started.stored.order.events : [] } },
    });
  });
});
