...
export type RecordActualTimeInput = {
  orderId: string;
  minutes: number;
  recordedOn: string;
  note: string | null;
...
export type ReverseActualTimeInput = {
  targetId: string;
  recordedOn: string;
  reason: string;
  operationKey: string;
...
export class ActualTimeService {
  ...
  async readOperatingMode(): Promise<ActualTimeResult<OperatingModeValue>> {
    ...
    return {
      ok: true,
      value: {
        workMode: mode(current.value?.workMode),
  ...
  async reverse(input: ReverseActualTimeInput): Promise<ActualTimeResult<ActualTimeRecord>> {
    ...
    if (retried) return { ok: true, value: retried, reused: true };
    try {
      const record = reverseActualTimeRecord(
        {
          id: createId(),
          target,
          recordedOn: input.recordedOn,
