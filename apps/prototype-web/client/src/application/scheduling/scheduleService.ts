...
export type ScheduleOverview = {
  overdue: readonly ScheduledOrder[];
  today: readonly ScheduledOrder[];
  upcoming: readonly ScheduledOrder[];
...
function initialSchedule(order: StoredCraftOrder): ScheduleEntry {
  ...
  return {
    ...
    events: [
      {
        id: `${order.id}:schedule-created`,
        type: "created",
        idempotencyKey: `${order.id}:schedule-created`,
        createdAt: timestamp,
...
function scheduleSort(a: ScheduledOrder, b: ScheduledOrder) {
  return (
    a.schedule.scheduledFor.localeCompare(b.schedule.scheduledFor) ||
    (a.schedule.scheduledTime ?? "99:99").localeCompare(b.schedule.scheduledTime ?? "99:99") ||
    b.order.updatedAt.localeCompare(a.order.updatedAt)
  );
}
...
function buildScheduleDay(
  ...
  const timed = ordered.filter(
    item => item.schedule.scheduledTime !== null && item.schedule.durationMinutes !== null,
  );
  ...
  return {
    ...
    items: ordered,
    scheduledMinutes,
    unknownTimingCount: ordered.length - timed.length,
    conflictCount: conflicts.size,
    overCapacity: dailyCapacityMinutes !== null && scheduledMinutes > dailyCapacityMinutes,
  };
}
...
export class ScheduleService {
  ...
  private async all(): Promise<
    ScheduleResult<{ schedules: readonly ScheduleEntry[]; orders: readonly StoredCraftOrder[] }>
  > {
    const [schedulesResult, ordersResult] = await Promise.all([
  ...
  private async reconciled(): Promise<
    ScheduleResult<{ schedules: readonly ScheduleEntry[]; orders: readonly StoredCraftOrder[] }>
  > {
    ...
    for (const order of records.value.orders) {
      ...
      const orderSchedules = schedules.filter(
        candidate => candidate.orderId === order.id && activeScheduleStatus(candidate.status),
      );
      if (orderSchedules.length === 0) continue;
      const delivery = order.order.events.find(
      ...
      for (const schedule of orderSchedules) {
        ...
        const completed = schedule.events.some(event => event.idempotencyKey === idempotencyKey)
          ? schedule
          : {
              ...schedule,
              ...
              events: [
                ...schedule.events,
                {
                  id: `${schedule.id}:completed:${schedule.events.length + 1}`,
  ...
  private async dailyCapacity(): Promise<ScheduleResult<number | null>> {
  ...
  async setDailyCapacity(minutes: number | null): Promise<ScheduleResult<number | null>> {
    ...
    if (!current.ok)
      return { ok: false, code: "storage_error", message: "تعذر قراءة تفضيلات الجدول المحلية." };
  ...
  async overview(): Promise<ScheduleResult<ScheduleOverview>> {
    ...
    const byId = new Map(records.value.orders.map(order => [order.id, order]));
    const overdue: ScheduledOrder[] = [];
    const day: ScheduledOrder[] = [];
    const upcoming: ScheduledOrder[] = [];
    let completedOrClosed = 0;
    for (const schedule of records.value.schedules) {
      const order = byId.get(schedule.orderId);
      if (!order) continue;
      if (!activeForOrder(schedule, order)) {
        completedOrClosed += 1;
        continue;
      }
      const item: ScheduledOrder = {
        schedule,
        order,
        bucket:
          schedule.scheduledFor < today ? "overdue" : schedule.scheduledFor === today ? "today" : "upcoming",
      };
      if (item.bucket === "overdue") overdue.push(item);
      else if (item.bucket === "today") day.push(item);
      else upcoming.push(item);
    }
    const active = [...day, ...upcoming].sort(scheduleSort);
    const week = Array.from({ length: 7 }, (_, index) => {
      const date = plusDays(today, index);
      return buildScheduleDay(
        date,
        active.filter(item => item.schedule.scheduledFor === date),
        capacity.value,
      );
    });
    return {
      ok: true,
      value: {
        overdue: overdue.sort(scheduleSort),
        today: day.sort(scheduleSort),
        upcoming: upcoming.sort(scheduleSort),
        week,
  ...
  async monthOverview(month: string): Promise<ScheduleResult<MonthOverview>> {
    ...
    const today = localDateKey(this.now());
    const byId = new Map(records.value.orders.map(order => [order.id, order]));
    const items: ScheduledOrder[] = [];
    for (const schedule of records.value.schedules) {
      const order = byId.get(schedule.orderId);
      if (order && activeForOrder(schedule, order) && schedule.scheduledFor.startsWith(`${month}-`))
        items.push({
          schedule,
          order,
          bucket:
    ...
    const days = Array.from({ length: daysInMonth(month) }, (_, index) => {
      const date = plusDays(firstDay, index);
      return buildScheduleDay(
        date,
        items.filter(item => item.schedule.scheduledFor === date),
        capacity.value,
      );
    });
    ...
    return {
      ...
      value: {
        ...
        overdueCount: items.filter(item => item.schedule.scheduledFor < today).length,
        scheduledMinutes: timed.reduce((total, item) => total + item.schedule.durationMinutes!, 0),
        unknownTimingCount: items.length - timed.length,
        dailyCapacityMinutes: capacity.value,
      },
    };
  }

  async reconcileDelivery(orderId: string): Promise<ScheduleResult<ScheduleEntry>> {
  ...
  async updateTiming(id: string, input: ScheduleTimingInput): Promise<ScheduleResult<ScheduleEntry>> {
    ...
    const next: ScheduleEntry = {
      ...
      status: changedDate ? "postponed" : current.value.status,
      postponeReason: changedDate ? reason : current.value.postponeReason,
      updatedAt: timestamp,
      events: [
        ...current.value.events,
        {
          id: `${id}:${type}:${current.value.events.length + 1}`,
          type,
          idempotencyKey,
          createdAt: timestamp,
