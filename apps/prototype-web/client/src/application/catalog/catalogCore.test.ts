...
function unit(
  id: string,
  nameAr: string,
  dimension: "count" | "mass" | "volume" | "time" | "distance" | "area",
) {
...
describe("catalog core domain", () => {
  it("supports the six shared dimensions and exact direct conversion without floating point truth", () => {
    expect([
      unit("count", "قطعة", "count"),
      unit("mass", "كيلوغرام", "mass"),
      unit("volume", "لتر", "volume"),
      unit("time", "ساعة", "time"),
      unit("distance", "متر", "distance"),
      unit("area", "متر مربع", "area"),
    ]).toHaveLength(6);
    const conversion = createDirectConversion({
      ...
      createdAt: at,
      createdOperationKey: "conversion:kg-g",
    });
    ...
    expect(() =>
      convertQuantityMilli(
        ...
        createDirectConversion({
          ...
          numerator: 2,
          denominator: 3,
          createdOperationKey: "conversion:not-exact",
        }),
      ),
    ).toThrow("لا يمكن تمثيل");
  });
  ...
  it("rejects invalid quantities and never allows a zero or negative conversion factor", () => {
    expect(() =>
      createMeasurementUnit({
        ...
        symbol: null,
        createdAt: at,
        createdOperationKey: "unit:bad",
      }),
    ).toThrow("اسم الوحدة مطلوب");
    expect(() =>
      createDirectConversion({
        id: "bad",
        fromUnitId: "kg",
        ...
        note: "غير صالح",
        createdAt: at,
        createdOperationKey: "conversion:bad",
      }),
    ).toThrow("بسط عامل التحويل");
    expect(() =>
      createDirectConversion({
        id: "bad-dimension",
        fromUnitId: "kg",
        toUnitId: "g",
        dimension: "unsupported" as "mass",
        numerator: 1,
        denominator: 1,
        note: "غير صالح",
        createdAt: at,
        createdOperationKey: "conversion:bad-dimension",
      }),
    ).toThrow("بُعد التحويل غير مدعوم");
    expect(() =>
      convertQuantityMilli(
        0,
        createDirectConversion({
          ...
          note: "صريح",
          createdAt: at,
          createdOperationKey: "conversion:good",
        }),
      ),
    ).toThrow("الكمية");
  });
  ...
  it("keeps a template optional and records an explicit not-ready yield instead of guessing", () => {
    const withoutYield = createCatalogTemplate({
      id: "template-1",
      ...
      createdAt: at,
      createdOperationKey: "template:1",
    });
    const needsConversion = createCatalogTemplate({
      id: "template-2",
      catalogItemId: "item-1",
      title: "مراجعة",
      note: null,
      components: [{ id: "component-1", name: "مادة", quantityMilli: 1250, unitId: "kg", note: null }],
      yield: { quantityMilli: 1000, unitId: "g" },
      yieldReadiness: "needs_conversion",
      revision: 2,
      sourceTemplateId: withoutYield.id,
      createdAt: at,
      createdOperationKey: "template:2",
    });
    expect(withoutYield).toMatchObject({ yield: null, yieldReadiness: "not_configured" });
    expect(needsConversion).toMatchObject({
      revision: 2,
      sourceTemplateId: "template-1",
      yieldReadiness: "needs_conversion",
      components: [{ quantityMilli: 1250 }],
    });
  });
});
...
describe("CatalogService G4-A", () => {
  it("creates units and a catalog item with an optional organized unit, then retries without duplication", async () => {
    ...
    const createdUnit = await service.createUnit({
      nameAr: "قطعة",
      dimension: "count",
      operationKey: "unit:piece",
    });
  ...
  it("keeps unit, conversion, and template creation idempotent on retry", async () => {
    const store = new MemoryLocalStore();
    const service = new CatalogService(store, () => at);
    const firstUnit = await service.createUnit({
      nameAr: "قطعة",
      dimension: "count",
      operationKey: "unit:idempotent",
    });
    const retriedUnit = await service.createUnit({
      nameAr: "اسم لا يبدل العملية",
      dimension: "count",
      operationKey: "unit:idempotent",
    });
    ...
    const firstConversion = await service.createConversion({
      fromUnitId: firstUnit.unit.id,
      toUnitId: "other-unit",
      numerator: 1,
      denominator: 1,
      note: "لن تحفظ",
      operationKey: "conversion:invalid",
    });
    expect(firstConversion).toMatchObject({ ok: false });
    const otherUnit = await service.createUnit({
      nameAr: "دزينة",
      dimension: "count",
      operationKey: "unit:dozen:idempotent",
    });
    if (!otherUnit.ok) throw new Error("second unit should be created");
    const conversion = await service.createConversion({
      fromUnitId: firstUnit.unit.id,
      toUnitId: otherUnit.unit.id,
      numerator: 1,
      denominator: 12,
      note: "12 قطعة = 1 دزينة",
      operationKey: "conversion:idempotent",
    });
    const retriedConversion = await service.createConversion({
      fromUnitId: firstUnit.unit.id,
      toUnitId: otherUnit.unit.id,
      numerator: 99,
      denominator: 1,
    ...
    expect(conversion.conversion).toMatchObject({
      fromUnitId: firstUnit.unit.id,
      toUnitId: otherUnit.unit.id,
      numerator: 1,
      denominator: 12,
    });
    expect(convertQuantityMilli(12_000, conversion.conversion)).toEqual({
      quantityMilli: 1_000,
      exact: true,
    });
    ...
    const item = await service.create({
      ...
      name: "تجهيز",
      unitLabel: null,
      operationKey: "catalog:idempotent",
    });
    ...
    const template = await service.createTemplate({
      ...
      components: [],
      yield: null,
      operationKey: "template:idempotent",
    });
    const retriedTemplate = await service.createTemplate({
      catalogItemId: item.item.id,
      title: "عنوان لا يبدل العملية",
      note: null,
      components: [],
      yield: null,
      operationKey: "template:idempotent",
  ...
  it("rejects cross-dimension conversion and duplicate active pairs while preserving an inactive revision path", async () => {
    ...
    const liter = await service.createUnit({
      nameAr: "لتر",
      dimension: "volume",
      operationKey: "unit:liter",
    });
    ...
    const cross = await service.createConversion({
      fromUnitId: kg.unit.id,
      toUnitId: liter.unit.id,
      numerator: 1,
      denominator: 1,
      note: "لا كثافة",
      operationKey: "conversion:cross",
    });
    const valid = await service.createConversion({
      fromUnitId: kg.unit.id,
      toUnitId: gram.unit.id,
      numerator: 1000,
      denominator: 1,
      note: "تحويل صريح",
      operationKey: "conversion:kg-g",
    });
    const duplicate = await service.createConversion({
      fromUnitId: kg.unit.id,
      toUnitId: gram.unit.id,
      numerator: 1,
      denominator: 1,
      note: "عامل متناقض",
      operationKey: "conversion:kg-g-2",
    });
    ...
    await expect(service.deactivateConversion(valid.conversion.id)).resolves.toMatchObject({
      ok: true,
    ...
    await expect(
      service.createConversion({
        ...
        denominator: 1,
        note: "نسخة موثقة",
        operationKey: "conversion:kg-g-3",
      }),
    ).resolves.toMatchObject({ ok: true, conversion: { active: true } });
  });

  it("keeps templates planning-only and creates a dated revision without changing catalog history", async () => {
    const store = new MemoryLocalStore();
    const service = new CatalogService(store, () => at);
    const count = await service.createUnit({
      nameAr: "قطعة",
      dimension: "count",
      operationKey: "unit:piece:candle",
    });
    ...
    const created = await service.create({
      ...
      name: "شمعة",
      unitLabel: "قطعة",
      unitId: count.unit.id,
      operationKey: "catalog:candle",
    });
    ...
    const first = await service.createTemplate({
      ...
      components: [],
      yield: null,
      operationKey: "template:candle:1",
    });
    if (!first.ok) throw new Error("template should be created");
    const second = await service.reviseTemplate(first.template.id, {
      title: "تجهيز محدث",
      note: "مراجعة مستقبلية",
      components: [{ id: "wax", name: "شمع", quantityMilli: 1000, unitId: "missing", note: null }],
      yield: null,
      operationKey: "template:candle:2",
    });
    expect(second).toMatchObject({ ok: false, message: expect.stringContaining("وحدة نشطة") });
    const mass = await service.createUnit({
      nameAr: "غرام",
      dimension: "mass",
      operationKey: "unit:gram:template",
    });
    if (!mass.ok) throw new Error("mass unit should be created");
    const crossDimensionYield = await service.reviseTemplate(first.template.id, {
      title: "مراجعة وزن",
      note: null,
      components: [],
      yield: { quantityMilli: 1000, unitId: mass.unit.id },
      operationKey: "template:candle:cross",
    });
    ...
    expect(snapshot).toMatchObject({
      ok: true,
      value: {
        catalogItems: [{ id: created.item.id }],
        catalogTemplates: [{ id: first.template.id, active: true }],
      },
    });
  });
});
