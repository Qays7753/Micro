import { assertSameDimension, createCatalogItem, createCatalogTemplate, createDirectConversion, createMeasurementUnit, type CatalogItem, type CatalogItemKind, type CatalogTemplate, type CatalogTemplateComponent, type DirectConversion, type MeasurementUnit, type UnitDimension } from "@micro-domain/catalog/index.js";
import type { PrototypeLocalStore } from "@/storage/local/types";

export type CatalogFailure = { ok: false; code: "validation_error" | "storage_error" | "not_found"; message: string };
...
export type CreateCatalogInput = { kind: CatalogItemKind; name: string; unitLabel: string | null; unitId?: string | null; operationKey: string };
export type CreateUnitInput = { nameAr: string; dimension: UnitDimension; symbol?: string | null; operationKey: string };
export type CreateConversionInput = { fromUnitId: string; toUnitId: string; numerator: number; denominator: number; note: string; operationKey: string };
export type CreateTemplateInput = { catalogItemId: string; title?: string | null; note?: string | null; components: readonly CatalogTemplateComponent[]; yield: { quantityMilli: number; unitId: string } | null; operationKey: string };

const createId = (prefix: string) => typeof crypto !== "undefined" && "randomUUID" in crypto ? crypto.randomUUID() : `${prefix}-${Date.now()}-${Math.random().toString(16).slice(2)}`;
const normalizedName = (value: string) => value.trim().replace(/\s+/g, " ").toLocaleLowerCase("ar-JO");
const failure = (message: string, code: CatalogFailure["code"] = "validation_error"): CatalogFailure => ({ ok: false, code, message });

function resolveYieldReadiness(item: CatalogItem, value: { quantityMilli: number; unitId: string } | null, units: readonly MeasurementUnit[], conversions: readonly DirectConversion[]): "not_configured" | "ready" | "needs_conversion" {
  ...
  return conversions.some(conversion => conversion.active && conversion.fromUnitId === value.unitId && conversion.toUnitId === item.unitId) ? "ready" : "needs_conversion";
}
...
export class CatalogService {
  ...
  async create(input: CreateCatalogInput): Promise<CatalogResult> {
    ...
    if (input.unitId) {
      ...
      if (!unit.value || !unit.value.active) return failure("وحدة المرجع غير متاحة للاختيار الجديد.", "not_found");
    }
    if (existing.value.some(item => item.active && item.kind === input.kind && normalizedName(item.name) === normalizedName(input.name))) return failure("يوجد مرجع نشط بالاسم والنوع نفسيهما. عطّله أو اختر اسمًا يميزه.");
    try {
      const item = createCatalogItem({ id: createId("catalog"), kind: input.kind, name: input.name, unitLabel: input.unitLabel, unitId: input.unitId ?? null, createdAt: this.now(), createdOperationKey: input.operationKey });
      const saved = await this.store.saveCatalogItem(item);
      return saved.ok ? { ok: true, item: saved.value } : failure("تعذر حفظ مرجع العمل محليًا.", "storage_error");
    } catch (error) {
  ...
  async deactivate(id: string): Promise<CatalogResult> {
    ...
    const saved = await this.store.saveCatalogItem({ ...existing.value, active: false, updatedAt: this.now() });
    return saved.ok ? { ok: true, item: saved.value } : failure("تعذر تعطيل مرجع العمل محليًا.", "storage_error");
  }
  ...
  async createUnit(input: CreateUnitInput): Promise<UnitResult> {
    ...
    if (existing.value.some(unit => unit.active && unit.dimension === input.dimension && normalizedName(unit.nameAr) === normalizedName(input.nameAr))) return failure("يوجد اسم وحدة نشط في هذا البعد. اختر اسمًا أوضح.");
    try {
      const unit = createMeasurementUnit({ id: createId("unit"), nameAr: input.nameAr, dimension: input.dimension, symbol: input.symbol ?? null, createdAt: this.now(), createdOperationKey: input.operationKey });
      const saved = await this.store.saveMeasurementUnit(unit);
      return saved.ok ? { ok: true, unit: saved.value } : failure("تعذر حفظ وحدة القياس محليًا.", "storage_error");
    } catch (error) {
  ...
  async deactivateUnit(id: string): Promise<UnitResult> {
    ...
    const saved = await this.store.saveMeasurementUnit({ ...existing.value, active: false, updatedAt: this.now() });
    return saved.ok ? { ok: true, unit: saved.value } : failure("تعذر إيقاف وحدة القياس محليًا.", "storage_error");
  }
  ...
  async listConversions(options: { includeInactive?: boolean } = {}): Promise<ConversionListResult> {
    ...
    return { ok: true, conversions: result.value.filter(conversion => options.includeInactive || conversion.active) };
  }

  async createConversion(input: CreateConversionInput): Promise<ConversionResult> {
    const [unitResult, conversionResult] = await Promise.all([this.store.listMeasurementUnits(), this.store.listDirectConversions()]);
    if (!unitResult.ok || !conversionResult.ok) return failure("تعذر قراءة الوحدات والتحويلات المحلية.", "storage_error");
    const retry = conversionResult.value.find(conversion => conversion.createdOperationKey === input.operationKey);
    ...
    try {
      ...
      if (conversionResult.value.some(conversion => conversion.active && conversion.fromUnitId === from.id && conversion.toUnitId === to.id)) return failure("يوجد تحويل نشط لهذا الزوج. عطّل القديم وأنشئ نسخة موثقة عند الحاجة.");
      const conversion = createDirectConversion({ id: createId("conversion"), fromUnitId: from.id, toUnitId: to.id, dimension: from.dimension, numerator: input.numerator, denominator: input.denominator, note: input.note, createdAt: this.now(), createdOperationKey: input.operationKey });
      const saved = await this.store.saveDirectConversion(conversion);
      return saved.ok ? { ok: true, conversion: saved.value } : failure("تعذر حفظ التحويل محليًا.", "storage_error");
    } catch (error) {
  ...
  async deactivateConversion(id: string): Promise<ConversionResult> {
    ...
    const saved = await this.store.saveDirectConversion({ ...existing.value, active: false, updatedAt: this.now() });
    return saved.ok ? { ok: true, conversion: saved.value } : failure("تعذر إيقاف التحويل محليًا.", "storage_error");
  }

  async listTemplates(catalogItemId?: string, options: { includeInactive?: boolean } = {}): Promise<TemplateListResult> {
    ...
    return { ok: true, templates: result.value.filter(template => options.includeInactive || template.active) };
  }

  private async buildTemplate(input: CreateTemplateInput, revision: number, sourceTemplateId: string | null): Promise<TemplateResult> {
    const [itemResult, unitsResult, conversionsResult] = await Promise.all([this.store.getCatalogItem(input.catalogItemId), this.store.listMeasurementUnits(), this.store.listDirectConversions()]);
    if (!itemResult.ok || !unitsResult.ok || !conversionsResult.ok) return failure("تعذر قراءة مرجع العمل والوحدات والتحويلات.", "storage_error");
    ...
    if (input.components.some(component => !activeUnits.some(unit => unit.id === component.unitId))) return failure("كل مكوّن يحتاج وحدة نشطة مسجلة.");
    const yieldInput = input.yield;
    if (yieldInput && !activeUnits.some(unit => unit.id === yieldInput.unitId)) return failure("وحدة الناتج غير متاحة للاختيار الجديد.");
    ...
    if (itemUnit && outputUnit && itemUnit.dimension !== outputUnit.dimension) return failure("وحدة الناتج من بُعد مختلف؛ اختر وحدة من البعد نفسه أو اترك الناتج غير مهيأ.");
    const yieldReadiness = resolveYieldReadiness(item, yieldInput, unitsResult.value, conversionsResult.value);
    try {
      const template = createCatalogTemplate({ id: createId("template"), catalogItemId: item.id, title: input.title ?? null, note: input.note ?? null, components: input.components, yield: yieldInput, yieldReadiness, revision, sourceTemplateId, createdAt: this.now(), createdOperationKey: input.operationKey });
  ...
  async createTemplate(input: CreateTemplateInput): Promise<TemplateResult> {
    ...
    if (existing.value.some(template => template.active)) return failure("يوجد قالب فعال لهذا المرجع. استخدم مراجعة جديدة بدل إنشاء قالب ثانٍ.");
    const built = await this.buildTemplate(input, 1, null);
    if (!built.ok) return built;
    const saved = await this.store.saveCatalogTemplate(built.template);
    return saved.ok ? { ok: true, template: saved.value } : failure("تعذر حفظ قالب العمل محليًا.", "storage_error");
  }

  async reviseTemplate(previousId: string, input: Omit<CreateTemplateInput, "catalogItemId">): Promise<TemplateResult> {
    ...
    const built = await this.buildTemplate({ ...input, catalogItemId: previousResult.value.catalogItemId }, previousResult.value.revision + 1, previousResult.value.id);
    if (!built.ok) return built;
    const result = await this.store.commitCatalogTemplateRevision({ ...previousResult.value, active: false, updatedAt: this.now() }, built.template);
  ...
  async deactivateTemplate(id: string): Promise<TemplateResult> {
    ...
    const saved = await this.store.saveCatalogTemplate({ ...existing.value, active: false, updatedAt: this.now() });
    return saved.ok ? { ok: true, template: saved.value } : failure("تعذر إيقاف القالب محليًا.", "storage_error");
  }
  ...
  async readRecordedMargins(): Promise<CatalogMarginsResult> {
    ...
    if (!catalog.ok || !orders.ok || !movements.ok) return failure("تعذر قراءة الأعمال المرتبطة لحساب الهامش المسجل.", "storage_error");
