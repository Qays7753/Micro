...
const fact = (id: "cash" | "receivables" | "payables" | "owner_capital", state: "known" | "incomplete" | "not_initialized", valueMinor: number | null) => ({ id, label: id, state, valueMinor, currency: "JOD" as const, source: "السجل المحلي", period: "2026-08", helper: "مصدر محلي" });
const baseInput = (): HomeControlCenterInput => ({
  ...
  facts: [fact("cash", "known", 1250), fact("receivables", "incomplete", 0), fact("payables", "not_initialized", null), fact("owner_capital", "known", 1250)],
  attention: [],
  optionalModules: [],
  recentChanges: [],
});
...
describe("buildHomeControlCenterViewModel", () => {
  it("keeps known JOD facts with source, period, and value while clearing incomplete values instead of inventing zero", () => {
    ...
    expect(model.facts).toMatchObject([
      { id: "cash", state: "known", valueMinor: 1250, currency: "JOD", source: "السجل المحلي", period: "2026-08" },
  ...
  it("orders unique attention by priority and caps the result at three items", () => {
    const model = buildHomeControlCenterViewModel({ ...baseInput(), attention: [
      { id: "late", priority: 3, kind: "order", title: "طلب متأخر", reason: "التسليم يحتاج متابعة", action: action("late") },
      { id: "draft", priority: 1, kind: "draft", title: "مسودة", reason: "لم تتحول إلى اتفاق", action: action("draft") },
      { id: "draft", priority: 1, kind: "draft", title: "مسودة مكررة", reason: "لا تُعرض مرتين", action: action("draft") },
      { id: "cost", priority: 2, kind: "cost", title: "تكلفة ناقصة", reason: "سعر الساعة غير معروف", action: action("cost") },
      { id: "follow-up", priority: 4, kind: "follow_up", title: "متابعة", reason: "تاريخ مسجل", action: action("follow-up") },
    ] });
  ...
  it("shows only optional modules with actual data or a relevant setup action", () => {
    const model = buildHomeControlCenterViewModel({ ...baseInput(), optionalModules: [
      { id: "inventory", label: "المخزون", state: "empty", action: null },
      { id: "schedule", label: "الجدول", state: "needs_setup", action: action("schedule") },
      { id: "supplier_commitments", label: "التزامات الموردين", state: "available", action: action("suppliers") },
  ...
  it("keeps the recent activity bounded to five useful changes and preserves the primary CTA", () => {
    const model = buildHomeControlCenterViewModel({ ...baseInput(), primaryAction: action("draft-1", "استأنف المسودة"), recentChanges: Array.from({ length: 7 }, (_, index) => ({ id: `change-${index}`, occurredOn: `2026-08-${String(index + 1).padStart(2, "0")}`, title: `تغيير ${index}`, detail: "تفصيل محلي", href: "/review" })) });
