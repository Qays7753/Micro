...
async function storedDeliveredOrder({ debt, costInput = knownCost }: { debt: boolean; costInput?: CostEditorInput }) {
  ...
  const costs = new CostService(store, () => "2026-08-22T00:01:00.000Z"); const withCost = await costs.saveSnapshot(saved.draft, costInput); if (!withCost.ok) throw new Error("cost should save");
...
describe("FinancialPulseService", () => {
  ...
  it("excludes a delivered incomplete result from final-only recognition and counts its missing knowledge", async () => {
    const incompleteCost: CostEditorInput = { materialItems: [{ name: "ورق", quantity: 1, unit: "قطعة", unitPriceMinor: 600, confidence: "known" }], time: null, packagingMinor: 0, deliveryMinor: 0, wasteMinor: 0, safetyBufferMinor: 0, quantity: 1 };
    ...
    expect(pulse).toMatchObject({ deliveredOrSettledOrderCount: 1, registeredCollectionsMinor: 500, registeredDebtMinor: 1700, finalResultOrderCount: 0, recognizedRevenueFromFinalOrdersMinor: 0, recognizedCostFromFinalOrdersMinor: 0, resultsAwaitingKnowledgeCount: 1 });
  });
