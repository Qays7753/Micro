...
const knownCost: CostEditorInput = {
...
async function storedDeliveredOrder({
  debt,
  costInput = knownCost,
}: {
  debt: boolean;
  costInput?: CostEditorInput;
}) {
  ...
  await fulfillment.deliver(agreed.stored.id);
  if (debt) await fulfillment.registerRemainingDebt(agreed.stored.id);
  const list = await store.listOrders();
  if (!list.ok) throw new Error("orders should list");
  return { store, order: list.value[0] };
}
...
describe("FinancialPulseService", () => {
  it("separates registered collection, post-delivery debt, and final-order recognition without inventing project profit", async () => {
    const debt = await storedDeliveredOrder({ debt: true });
    const paid = await storedDeliveredOrder({ debt: false });
    const pulse = summarizeLocalCraftOrders([debt.order, paid.order]);
    expect(pulse).toMatchObject({
      totalOrderCount: 2,
      deliveredOrSettledOrderCount: 2,
      registeredCollectionsMinor: 1000,
      registeredDebtMinor: 1700,
      finalResultOrderCount: 2,
      recognizedRevenueFromFinalOrdersMinor: 4400,
      recognizedCostFromFinalOrdersMinor: 3000,
    });
    expect("projectProfitMinor" in pulse).toBe(false);
  });

  it("excludes a delivered incomplete result from final-only recognition and counts its missing knowledge", async () => {
    const incompleteCost: CostEditorInput = {
      materialItems: [{ name: "ورق", quantity: 1, unit: "قطعة", unitPriceMinor: 600, confidence: "known" }],
      time: null,
      packagingMinor: 0,
      deliveryMinor: 0,
      wasteMinor: 0,
    ...
    const delivered = await storedDeliveredOrder({ debt: true, costInput: incompleteCost });
    const pulse = summarizeLocalCraftOrders([delivered.order]);
    expect(pulse).toMatchObject({
      deliveredOrSettledOrderCount: 1,
      registeredCollectionsMinor: 500,
      registeredDebtMinor: 1700,
      finalResultOrderCount: 0,
      recognizedRevenueFromFinalOrdersMinor: 0,
      recognizedCostFromFinalOrdersMinor: 0,
      resultsAwaitingKnowledgeCount: 1,
    });
  });

  it("reads the pulse through the LocalStore port", async () => {
    const { store } = await storedDeliveredOrder({ debt: true });
    await expect(new FinancialPulseService(store).read()).resolves.toMatchObject({
      ok: true,
      pulse: { registeredCollectionsMinor: 500, registeredDebtMinor: 1700, finalResultOrderCount: 1 },
    });
  });
});
