...
function completedMaterialOrder(id: string) { const cost = calculateCostSnapshot(`${id}-cost`, { currency: "JOD", materialItems: [{ name: "خشب", quantity: 1, unit: "قطعة", unitPriceMinor: 1000, priceDate: "2026-08-01", source: "user_input", confidence: "known" }], time: { minutes: 60, hourlyRateMinor: 500, confidence: "known" }, packagingMinor: 100, deliveryMinor: 0, wasteMinor: 0, safetyBufferMinor: 0, quantity: 1, createdAt: "2026-08-01T09:00:00.000Z", freshnessDays: null }); let order = createCraftOrder({ id, customerName: "عميلة", itemName: "قطعة خشب", specifications: "اختبار COGS", quantity: 1, agreedPriceMinor: 3000, costSnapshot: cost, createdAt: "2026-08-01T09:00:00.000Z" }); for (const [to, stamp] of [["provisional_agreement", "2026-08-01T10:00:00.000Z"], ["confirmed", "2026-08-01T11:00:00.000Z"], ["in_progress", "2026-08-02T09:00:00.000Z"], ["ready", "2026-08-03T09:00:00.000Z"], ["delivered", "2026-08-05T09:00:00.000Z"]] as const) order = transitionOrder(order, { to, idempotencyKey: `${id}-${to}`, createdAt: stamp }); return order; }
function inventoryFixture(orderId: string) { const material = createMaterial({ id: `${orderId}-material`, name: "خشب", unit: "piece", createdAt: "2026-08-01T09:00:00.000Z", createdOperationKey: `${orderId}-material-create` }); const opening = createInventoryMovement({ id: `${orderId}-opening`, materialId: material.id, type: "opening", occurredOn: "2026-08-01", recordedAt: now(), quantityDeltaMilli: 10000, valueDeltaMinor: 10000, note: "افتتاح مادة", operationKey: `${orderId}-opening` }); const consumption = createInventoryMovement({ id: `${orderId}-consumption`, materialId: material.id, type: "consumption", occurredOn: "2026-08-05", recordedAt: now(), quantityDeltaMilli: -1000, valueDeltaMinor: -1200, note: "استهلاك مثبت", operationKey: `${orderId}-consumption`, orderId }); return { material, opening, consumption }; }
describe("ProjectFinancialService", () => {
  ...
  it("keeps a deferred shared total visible and in cash without treating it as a result expense", async () => {
    ...
    await expect(finance.readRecordedPeriodResult("2026-08-01", "2026-08-31")).resolves.toMatchObject({ ok: true, value: { recordedOperatingExpenseMinor: 0, sharedProjectExpenseMinor: 0, sharedUnallocatedExpenseMinor: 500, sharedUnallocatedExpenseCount: 1, expenseNeedsReviewCount: 1, resultMinor: 0, status: "incomplete" } });
    ...
    await expect(finance.readRecordedPeriodResult("2026-08-23", "2026-08-23")).resolves.toMatchObject({ ok: true, value: { sharedUnallocatedExpenseMinor: 500, sharedUnallocatedExpenseCount: 1, resultMinor: 0, status: "incomplete" } });
    await expect(finance.readRecordedPeriodResult("2026-08-24", "2026-08-24")).resolves.toMatchObject({ ok: true, value: { sharedUnallocatedExpenseMinor: -500, sharedUnallocatedExpenseCount: 0, resultMinor: 0, status: "recorded_only" } });
  });

  it("uses valued consumption for the material component once while retaining non-material Snapshot costs", async () => { const store = new MemoryLocalStore(); const finance = new ProjectFinancialService(store, now); const order = completedMaterialOrder("cogs-order"); const fixture = inventoryFixture(order.id); await store.saveOrder({ id: order.id, order, deliveryDate: "2026-08-05", agreementSource: "test", createdAt: order.createdAt, updatedAt: "2026-08-05T09:00:00.000Z" }); await store.commitInventory(fixture.material, [fixture.opening, fixture.consumption]); const result = await finance.readRecordedPeriodResult("2026-08-01", "2026-08-31"); expect(result).toMatchObject({ ok: true, value: { recognizedRevenueMinor: 3000, recognizedDirectCostMinor: 1600, snapshotDirectCostMinor: 1600, recordedCogsMinor: 1200, effectiveDirectCostMinor: 1800, cogsStatus: "recorded", cogsMissingOrderCount: 0, resultMinor: 1200, status: "recorded_only" } }); if (!result.ok) throw new Error("period result should read"); expect(result.value.cogsReasons).toEqual([]); });

  it("keeps Snapshot as the explicit fallback when consumption is absent and does not call purchase receipt COGS", async () => { const store = new MemoryLocalStore(); const finance = new ProjectFinancialService(store, now); const order = completedMaterialOrder("snapshot-order"); const material = createMaterial({ id: "snapshot-material", name: "خشب", unit: "piece", createdAt: "2026-08-01T09:00:00.000Z", createdOperationKey: "snapshot-material-create" }); const receipt = createInventoryMovement({ id: "snapshot-receipt", materialId: material.id, type: "purchase_receipt", occurredOn: "2026-08-04", recordedAt: now(), quantityDeltaMilli: 1000, valueDeltaMinor: 1000, note: "استلام شراء", operationKey: "snapshot-receipt", purchaseId: "purchase-1" }); await store.saveOrder({ id: order.id, order, deliveryDate: "2026-08-05", agreementSource: "test", createdAt: order.createdAt, updatedAt: "2026-08-05T09:00:00.000Z" }); await store.commitInventory(material, [receipt]); await expect(finance.readRecordedPeriodResult("2026-08-01", "2026-08-31")).resolves.toMatchObject({ ok: true, value: { snapshotDirectCostMinor: 1600, recordedCogsMinor: 0, effectiveDirectCostMinor: 1600, cogsStatus: "not_available", cogsMissingOrderCount: 1, resultMinor: 1400, status: "recorded_only" } }); });

  it("excludes reversed consumption and unlinked or non-final consumption from COGS without changing the order Snapshot", async () => { const store = new MemoryLocalStore(); const finance = new ProjectFinancialService(store, now); const order = completedMaterialOrder("reversal-order"); const fixture = inventoryFixture(order.id); const reversal = createInventoryMovement({ id: "reversal-consumption", materialId: fixture.material.id, type: "reversal", occurredOn: "2026-08-06", recordedAt: now(), quantityDeltaMilli: 1000, valueDeltaMinor: 1200, note: "عكس استهلاك", reason: "تصحيح اختبار", operationKey: "reversal-consumption", reversesMovementId: fixture.consumption.id }); const unlinked = { ...fixture.consumption, id: "unlinked-consumption", orderId: null, operationKey: "unlinked-consumption", valueDeltaMinor: -500 } as typeof fixture.consumption; const nonFinalOrder = completedMaterialOrder("future-order"); nonFinalOrder.status = "in_progress"; nonFinalOrder.resultStatus = "incomplete"; await store.saveOrder({ id: order.id, order, deliveryDate: "2026-08-05", agreementSource: "test", createdAt: order.createdAt, updatedAt: "2026-08-05T09:00:00.000Z" }); await store.saveOrder({ id: nonFinalOrder.id, order: nonFinalOrder, deliveryDate: "2026-08-05", agreementSource: "test", createdAt: nonFinalOrder.createdAt, updatedAt: "2026-08-05T09:00:00.000Z" }); await store.commitInventory(fixture.material, [fixture.opening, fixture.consumption, reversal, unlinked]); await expect(finance.readRecordedPeriodResult("2026-08-01", "2026-08-31")).resolves.toMatchObject({ ok: true, value: { recordedCogsMinor: 0, effectiveDirectCostMinor: 1600, cogsStatus: "not_available", cogsMissingOrderCount: 1, resultMinor: 1400, status: "incomplete" } }); });
});
