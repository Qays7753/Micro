...
describe("ProjectFinancialService", () => {
  ...
  it("calculates a recorded period result from final delivered orders and general expenses without counting owner money", async () => {
    ...
    await finance.record({ type: "operating_expense_cash", amountMinor: 300, occurredOn: "2026-08-06", note: "توصيل", counterparty: null, relatedEventId: null, idempotencyKey: "period-expense" });
    ...
    const payable = await finance.record({ type: "operating_expense_payable", amountMinor: 400, occurredOn: "2026-08-06", note: "مواد مستحقة", counterparty: "مورد", relatedEventId: null, idempotencyKey: "period-payable" }); if (!payable.ok) throw new Error("payable should save");
