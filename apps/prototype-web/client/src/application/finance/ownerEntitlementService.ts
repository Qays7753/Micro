...
export type OwnerEntitlementReversalInput = { recordId: string; occurredOn: string; reason: string; idempotencyKey: string };
export type OwnerOpeningBalanceReversalInput = { balanceId: string; occurredOn: string; reason: string; idempotencyKey: string };
...
const signedRecordTotal = (values: readonly OwnerEntitlementRecord[]) => values.reduce((sum, value) => sum + (value.reversalOfId ? -value.amountMinor : value.amountMinor), 0);
const signedOpeningTotal = (values: readonly OwnerEntitlementOpeningBalance[]) => values.reduce((sum, value) => sum + (value.reversalOfId ? -value.amountMinor : value.amountMinor), 0);
const signedMovementTotal = (values: readonly OwnerMovement[], selector: (movement: OwnerMovement) => number) => values.reduce((sum, value) => sum + selector(value) * (value.reversalOfId ? -1 : 1), 0);
...
export class OwnerEntitlementService {
  ...
  async readOverview(): Promise<OwnerEntitlementResult<OwnerEntitlementOverview>> {
    ...
    const openingBalanceSettlementMinor = movements.value.reduce((sum, movement) => sum + movement.openingBalanceDeltaMinor, 0);
    ...
    const netMovementAmount = (reason: OwnerMovementReason) => Math.max(0, signedMovementTotal(movements.value.filter(movement => movement.reason === reason), movement => movement.amountMinor));
    ...
    const remainingEntitlementBalanceMinor = openingBalanceRemainingMinor + approvedEntitlementMinor + movements.value.reduce((sum, movement) => sum + movement.entitlementDeltaMinor, 0);
    ...
    return { ok: true, value: { policies: policies.value, activePolicies, entitlements: entitlements.value, openingBalances: openingBalances.value, movements: movements.value, walletBalances, approvedEntitlementMinor, openingBalanceMinor, openingBalanceSettlementMinor, openingBalanceRemainingMinor, drawnForEntitlementMinor, drawnBeforeEntitlementMinor, ownerDrawMinor, returnedForPriorDrawMinor, returnedAsCapitalMinor, remainingEntitlementBalanceMinor, cashMovementMinor, balanceState, truth: "الاستحقاق المسجل ليس قبضًا ولا يغير كاش المشروع. السحب والإرجاع الفعليان يغيران محفظة الكاش فقط وفق سببهما؛ الاستثمار الجديد مستقل عن الاستحقاق، والسحب غير المرتبط بسياسة يبقى Owner Draw مستقلًا.", nextAction: activePolicies.length === 0 ? "أضف سياسة مؤرخة إذا أردت تسجيل استحقاق جديد؛ لا ينشئ النظام استحقاقًا من تاريخ سابق تلقائيًا." : balanceState === "positive" ? "يمكن تسجيل سحب لتسوية استحقاق أو افتتاح موجب ضمن المصدر المتبقي، أو تسجيل واقعة فعلية أخرى بسبب واضح." : balanceState === "negative" ? "راجع السحوبات السابقة وسجل إرجاعًا لتسوية سحب سابق أو افتتاح سالب إذا كان هذا ما حدث فعليًا." : "الرصيد مسوى حاليًا؛ لا تسجل حركة بلا سبب واضح." } };
  }
  ...
  async createPolicy(input: OwnerPolicyInput): Promise<OwnerEntitlementResult<OwnerEntitlementPolicy>> {
    ...
    try {
      ...
      if (existing.value.some(other => other.seriesId === policy.seriesId)) return { ok: false, code: "validation_error", message: "السلسلة موجودة؛ استخدم إجراء الخليفة المؤرخ بدل تعديل الإصدار يدويًا." };
      const saved = await this.store.saveOwnerEntitlementPolicy(policy); return saved.ok ? { ok: true, value: saved.value } : failure("تعذر حفظ سياسة الاستحقاق؛ لم يتم تأكيد العملية.");
    } catch (error) { return { ok: false, code: "validation_error", message: error instanceof Error ? error.message : "بيانات السياسة غير صالحة." }; }
  }

  async createPolicySuccessor(policyId: string, input: OwnerPolicySuccessorInput): Promise<OwnerEntitlementResult<OwnerEntitlementPolicy>> {
    const policies = await this.store.listOwnerEntitlementPolicies(); if (!policies.ok) return failure("تعذر قراءة سلسلة سياسة الاستحقاق.");
  ...
  async calculate(policyId: string, periodFrom: string, periodTo: string): Promise<OwnerEntitlementResult<ReturnType<typeof calculateOwnerEntitlement>>> {
    ...
    const reversedTimeIds = new Set(timeRecords.value.filter(record => record.reversalOfId !== null).map(record => record.reversalOfId));
    const activeTimeRecords = timeRecords.value.filter(record => record.minutesDelta > 0 && !reversedTimeIds.has(record.id));
    ...
    const evidence: OwnerEntitlementEvidence = { periodFrom, periodTo, completedWorkCount: finalOrders.length, completedWorkKeys: finalOrders.map(item => item.stored.order.id), completedSaleMinor: finalOrders.reduce((sum, item) => sum + item.stored.order.recognizedRevenueMinor, 0), completedSaleKeys: finalOrders.map(item => item.stored.order.id), unitQuantity: finalOrders.reduce((sum, item) => sum + item.stored.order.quantity, 0), unitSourceKeys: finalOrders.map(item => item.stored.order.id), timeQuantity: relevantTimeRecords.reduce((sum, record) => sum + record.minutesDelta, 0) || null, timeSourceKeys: relevantTimeRecords.map(record => record.id) };
    if (policy.value.kind === "profit_share") { if (!this.periodResultReader) return { ok: true, value: calculateOwnerEntitlement(policy.value, { ...evidence, recognizedProfitMinor: null, recognizedProfitStatus: "invalid" }) }; const result = await this.periodResultReader(periodFrom, periodTo); if (!result.ok) return failure(result.message); evidence.recognizedProfitMinor = result.value.resultMinor; evidence.recognizedProfitStatus = result.value.status; evidence.recognizedProfitKeys = [`g3:${periodFrom}:${periodTo}`]; }
  ...
  async recordEntitlement(input: OwnerEntitlementRecordInput): Promise<OwnerEntitlementResult<OwnerEntitlementRecord>> {
    ...
    const calculated = await this.calculate(input.policyId, input.periodFrom, input.periodTo); if (!calculated.ok) return calculated; if (calculated.value.amountMinor === null) return { ok: false, code: "validation_error", message: calculated.value.nextAction };
    const sourceKeys = calculated.value.sourceKeys.length > 0 ? calculated.value.sourceKeys : [`period:${policy.value.id}:${input.periodFrom}:${input.periodTo}`];
    const activeRecords = activeOriginals(existing.value);
    const policyValue = policy.value;
    const samePolicy = activeRecords.filter(record => record.policyId === policyValue.id && record.policyVersion === policyValue.version);
    const sourceOverlap = samePolicy.some(record => record.sourceKeys.some(key => sourceKeys.includes(key)));
    if (sourceOverlap || (periodExclusive(policyValue.kind) && samePolicy.some(record => rangesOverlap(record.periodFrom, record.periodTo, input.periodFrom, input.periodTo)))) return { ok: false, code: "validation_error", message: "يوجد استحقاق نشط يغطي المصدر أو الفترة نفسها؛ لم يتكرر الحق. اعكس السجل السابق أولًا إذا كان خطأ." };
    try { const record = createOwnerEntitlementRecord({ id: id("entitlement"), policyId: policy.value.id, policyVersion: policy.value.version, periodFrom: input.periodFrom, periodTo: input.periodTo, occurredOn: input.occurredOn, recordedAt: this.now(), amountMinor: calculated.value.amountMinor, knowledge: calculated.value.knowledge === "incomplete" ? "partial" : calculated.value.knowledge, calculationBasis: calculated.value.calculationBasis, baseMinor: calculated.value.baseMinor, quantity: calculated.value.quantity, sourceKeys, note: input.note, idempotencyKey: input.idempotencyKey, reversalOfId: null, reversalReason: null }); const saved = await this.store.saveOwnerEntitlementRecord(record); return saved.ok ? { ok: true, value: saved.value } : failure("تعذر حفظ الاستحقاق؛ لم يتغير الكاش."); } catch (error) { return { ok: false, code: "validation_error", message: error instanceof Error ? error.message : "بيانات الاستحقاق غير صالحة." }; }
  }

  async reverseEntitlement(input: OwnerEntitlementReversalInput): Promise<OwnerEntitlementResult<OwnerEntitlementRecord>> {
    const [records, movements] = await Promise.all([this.store.listOwnerEntitlementRecords(), this.store.listOwnerMovements()]); if (!records.ok || !movements.ok) return failure("تعذر قراءة سجل الاستحقاق.");
    const repeated = records.value.find(record => record.reversalOfId !== null && record.idempotencyKey === input.idempotencyKey); if (repeated) return { ok: true, value: repeated, reused: true };
    const source = records.value.find(record => record.id === input.recordId); if (!source) return { ok: false, code: "validation_error", message: "لم نجد سجل الاستحقاق الأصلي." }; if (source.reversalOfId) return { ok: false, code: "validation_error", message: "لا يمكن عكس عكس سابق." }; if (records.value.some(record => record.reversalOfId === source.id)) return { ok: false, code: "validation_error", message: "عُكس هذا الاستحقاق سابقًا؛ لا يُنشأ عكس ثانٍ." }; if (activeOriginals(movements.value).some(movement => movement.relatedEntitlementId === source.id)) return { ok: false, code: "validation_error", message: "اعكس أو سوِّ حركات هذا الاستحقاق أولًا؛ لا نترك مصدرًا مسجلًا بلا رصيد متوازن." };
    try { const reversal = createOwnerEntitlementRecordReversal({ id: id("entitlement-reversal"), source, occurredOn: input.occurredOn, recordedAt: this.now(), reason: input.reason, idempotencyKey: input.idempotencyKey }); const saved = await this.store.commitOwnerEntitlementRecordReversal(source.id, reversal); return saved.ok ? { ok: true, value: saved.value } : failure("تعذر حفظ عكس الاستحقاق؛ بقي الأصل محفوظًا."); } catch (error) { return { ok: false, code: "validation_error", message: error instanceof Error ? error.message : "بيانات عكس الاستحقاق غير صالحة." }; }
  }
  ...
  async setOpeningBalance(input: OwnerOpeningBalanceInput): Promise<OwnerEntitlementResult<OwnerEntitlementOpeningBalance>> {
    ...
    if (activeOriginals(existing.value).length > 0) return { ok: false, code: "validation_error", message: "يوجد رصيد افتتاحي فعال؛ اعكسه أو صححه قبل إضافة طبقة افتتاحية جديدة." };
    try { const balance = createOwnerEntitlementOpeningBalance({ ...input, recordedAt: this.now(), reversalOfId: null, reversalReason: null }); const saved = await this.store.saveOwnerEntitlementOpeningBalance(balance); return saved.ok ? { ok: true, value: saved.value } : failure("تعذر حفظ الرصيد الافتتاحي؛ لم تُنشأ حركات ماضية."); } catch (error) { return { ok: false, code: "validation_error", message: error instanceof Error ? error.message : "بيانات الرصيد الافتتاحي غير صالحة." }; }
  }

  async reverseOpeningBalance(input: OwnerOpeningBalanceReversalInput): Promise<OwnerEntitlementResult<OwnerEntitlementOpeningBalance>> {
    const [balances, movements] = await Promise.all([this.store.listOwnerEntitlementOpeningBalances(), this.store.listOwnerMovements()]); if (!balances.ok || !movements.ok) return failure("تعذر قراءة الرصيد الافتتاحي.");
    const repeated = balances.value.find(balance => balance.reversalOfId !== null && balance.idempotencyKey === input.idempotencyKey); if (repeated) return { ok: true, value: repeated, reused: true };
    const source = balances.value.find(balance => balance.id === input.balanceId); if (!source) return { ok: false, code: "validation_error", message: "لم نجد الرصيد الافتتاحي الأصلي." }; if (source.reversalOfId) return { ok: false, code: "validation_error", message: "لا يمكن عكس عكس سابق." }; if (balances.value.some(balance => balance.reversalOfId === source.id)) return { ok: false, code: "validation_error", message: "عُكس هذا الرصيد الافتتاحي سابقًا؛ لا يُنشأ عكس ثانٍ." }; if (activeOriginals(movements.value).some(movement => movement.relatedOpeningBalanceId === source.id)) return { ok: false, code: "validation_error", message: "اعكس أو سوِّ حركات هذا الافتتاح أولًا؛ لا نترك مصدرًا مسجلًا بلا رصيد متوازن." };
    try { const reversal = createOwnerEntitlementOpeningBalanceReversal({ id: id("opening-reversal"), source, occurredOn: input.occurredOn, recordedAt: this.now(), reason: input.reason, idempotencyKey: input.idempotencyKey }); const saved = await this.store.commitOwnerEntitlementOpeningBalanceReversal(source.id, reversal); return saved.ok ? { ok: true, value: saved.value } : failure("تعذر حفظ عكس الرصيد الافتتاحي؛ بقي الأصل محفوظًا."); } catch (error) { return { ok: false, code: "validation_error", message: error instanceof Error ? error.message : "بيانات عكس الرصيد الافتتاحي غير صالحة." }; }
  }
  ...
  async recordMovement(input: OwnerMovementInput): Promise<OwnerEntitlementResult<{ movement: OwnerMovement; cashEntry: CashContinuityEntry }>> {
    const [movements, wallets, entitlements, openingBalances, cashEntries] = await Promise.all([this.store.listOwnerMovements(), this.store.listCashWallets(), this.store.listOwnerEntitlementRecords(), this.store.listOwnerEntitlementOpeningBalances(), this.store.listCashContinuityEntries()]); if (!movements.ok || !wallets.ok || !entitlements.ok || !openingBalances.ok || !cashEntries.ok) return failure("تعذر قراءة حركة المالك أو محفظة الكاش.");
    ...
    if (input.reason === "entitlement_settlement") { if (input.kind !== "draw" || !input.relatedEntitlementId || !activeEntitlementRecords.some(record => record.id === input.relatedEntitlementId)) return { ok: false, code: "validation_error", message: "اختر استحقاقًا مسجلًا وفعالًا لتسويته؛ لا تخمن السبب." }; const entitlement = activeEntitlementRecords.find(record => record.id === input.relatedEntitlementId)!; const settled = movements.value.filter(movement => movement.relatedEntitlementId === entitlement.id).reduce((sum, movement) => sum + movement.entitlementDeltaMinor, 0); if (input.amountMinor > entitlement.amountMinor + settled) return { ok: false, code: "validation_error", message: "لا يمكن أن يتجاوز السحب استحقاق هذا السجل المتبقي." }; }
    if (input.reason === "opening_balance_settlement") { if (!input.relatedOpeningBalanceId) return { ok: false, code: "validation_error", message: "اختر رصيدًا افتتاحيًا لتسويته؛ لا نخمن مصدر الحركة." }; const activeOpening = activeOriginals(openingBalances.value).find(balance => balance.id === input.relatedOpeningBalanceId); if (!activeOpening) return { ok: false, code: "validation_error", message: "اختر رصيدًا افتتاحيًا فعالًا؛ لا تسوِّ مصدرًا معكوسًا." }; const settled = movements.value.filter(movement => movement.relatedOpeningBalanceId === activeOpening.id).reduce((sum, movement) => sum + movement.openingBalanceDeltaMinor, 0); const remaining = activeOpening.amountMinor + settled; if ((remaining > 0 && input.kind !== "draw") || (remaining < 0 && input.kind !== "return") || remaining === 0 || input.amountMinor > Math.abs(remaining)) return { ok: false, code: "validation_error", message: "نوع الحركة أو مبلغ تسوية الافتتاح لا يطابق الرصيد المتبقي." }; }
