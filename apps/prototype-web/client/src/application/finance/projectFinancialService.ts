...
export class ProjectFinancialService {
  ...
  async readRecordedPeriodResult(from: string, to: string): Promise<FinanceResult<RecordedPeriodResult>> {
    ...
    return { ok: true, value: { from, to, recognizedRevenueMinor, recognizedDirectCostMinor, recordedOperatingExpenseMinor, projectOperatingExpenseMinor, sharedProjectExpenseMinor, sharedUnallocatedExpenseMinor, legacyUnclassifiedExpenseMinor, sharedEstimatedExpenseCount, sharedMissingBasisCount, sharedUnallocatedExpenseCount, legacyUnclassifiedExpenseCount, resultMinor: incomplete ? null : recognizedRevenueMinor - recognizedDirectCostMinor - recordedOperatingExpenseMinor, finalOrderCount: finals.length, excludedOrderCount, expenseNeedsReviewCount, status: incomplete ? "incomplete" : "recorded_only", reasons, truth: incomplete ? "هذه نتيجة الفترة التشغيلية المسجلة من السجل المحلي، لكنها ناقصة للأسباب الظاهرة. المصروف المشترك غير المحمل لا يخصم من النتيجة، وغياب دليل COGS أو التحميل لا يساوي صفرًا؛ هذه القراءة ليست صافي ربح نهائيًا." : "هذه نتيجة الأعمال المكتملة ناقص التكلفة المباشرة المسجلة ومصروف المشروع والحصص المشتركة المحملة لهذه الفترة. لا تؤكد COGS الفعلي خارج السجل أو التوزيع على المنتجات أو الضرائب، لذلك ليست صافي ربح نهائيًا." } };
  }
