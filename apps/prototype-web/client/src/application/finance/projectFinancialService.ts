 ...
 */
import { calculateSharedProjectShareMinor, createFinancialEvent, createFinancialReversal, summarizeFinancialEvents, type FinancialEvent, type FinancialEventType, type OperatingExpenseContext } from "@micro-domain/financial-event/index.js";
...
export type RecordedPeriodResult = { from: string; to: string; recognizedRevenueMinor: number; recognizedDirectCostMinor: number; snapshotDirectCostMinor: number; recordedCogsMinor: number; effectiveDirectCostMinor: number; cogsStatus: CogsStatus; cogsMissingOrderCount: number; unallocatedInventoryCostMinor: number; generalInventoryWasteMinor: number; cogsReasons: readonly string[]; cogsNextAction: string; recordedOperatingExpenseMinor: number; projectOperatingExpenseMinor: number; sharedProjectExpenseMinor: number; sharedUnallocatedExpenseMinor: number; legacyUnclassifiedExpenseMinor: number; sharedEstimatedExpenseCount: number; sharedMissingBasisCount: number; sharedUnallocatedExpenseCount: number; legacyUnclassifiedExpenseCount: number; resultMinor: number | null; finalOrderCount: number; excludedOrderCount: number; expenseNeedsReviewCount: number; status: "recorded_only" | "incomplete" | "invalid"; reasons: readonly string[]; truth: string };
...
export type FinancialRecordInput = { type: FinancialEventType; amountMinor?: number; occurredOn: string; note: string; counterparty: string | null; relatedEventId: string | null; expenseContext?: OperatingExpenseContext | null; idempotencyKey: string; sharedExpense?: SharedExpenseRecordInput };
...
function sharedExpenseIsUnallocated(event: FinancialEvent) { return event.expenseContext?.relationship === "shared" && event.expenseContext.sharedProjectShare?.allocation === "unallocated"; }
function isRecordedOperatingExpense(event: FinancialEvent) { return event.operatingExpenseDeltaMinor !== 0; }
function expenseNeedsReview(event: FinancialEvent) { return (event.operatingExpenseDeltaMinor > 0 && (!event.expenseContext || event.expenseContext.knowledge !== "known" || sharedExpenseHasMissingBasis(event))) || sharedExpenseIsUnallocated(event); }
type PeriodCogsReading = { snapshotDirectCostMinor: number; recordedCogsMinor: number; effectiveDirectCostMinor: number; cogsStatus: CogsStatus; cogsMissingOrderCount: number; unallocatedInventoryCostMinor: number; generalInventoryWasteMinor: number; cogsReasons: readonly string[]; cogsNextAction: string };
function activeInventoryMovements(movements: readonly InventoryMovement[]) { const reversedIds = new Set(movements.filter((movement) => movement.type === "reversal" && movement.reversesMovementId).map((movement) => movement.reversesMovementId)); return movements.filter((movement) => movement.type !== "reversal" && !reversedIds.has(movement.id)); }
function derivePeriodCogs(finals: readonly { order: { id: string; recognizedCostMinor: number; costSnapshot: { materialCostMinor: number } } }[], movements: readonly InventoryMovement[]): PeriodCogsReading {
  const active = activeInventoryMovements(movements); const finalOrderIds = new Set(finals.map((item) => item.order.id)); const qualified = active.filter((movement) => isCostBackedConsumption(movement) && finalOrderIds.has(movement.orderId!)); const byOrder = new Map<string, number>(); qualified.forEach((movement) => byOrder.set(movement.orderId!, (byOrder.get(movement.orderId!) ?? 0) + Math.abs(movement.valueDeltaMinor)));
  let snapshotDirectCostMinor = 0; let effectiveDirectCostMinor = 0; let cogsMissingOrderCount = 0; for (const item of finals) { const observedCogsMinor = byOrder.get(item.order.id) ?? 0; snapshotDirectCostMinor += item.order.recognizedCostMinor; effectiveDirectCostMinor += observedCogsMinor > 0 ? item.order.recognizedCostMinor - item.order.costSnapshot.materialCostMinor + observedCogsMinor : item.order.recognizedCostMinor; if (observedCogsMinor === 0) cogsMissingOrderCount += 1; }
  const recordedCogsMinor = qualified.reduce((sum, movement) => sum + Math.abs(movement.valueDeltaMinor), 0); const cogsOrderCount = finals.length - cogsMissingOrderCount; const cogsStatus: CogsStatus = finals.length === 0 ? "not_available" : cogsOrderCount === finals.length ? "recorded" : cogsOrderCount > 0 ? "partial" : "not_available"; const unallocatedInventoryCostMinor = active.filter((movement) => movement.type === "consumption" && !movement.orderId).reduce((sum, movement) => sum + Math.abs(movement.valueDeltaMinor), 0); const generalInventoryWasteMinor = active.filter((movement) => movement.type === "waste").reduce((sum, movement) => sum + Math.abs(movement.valueDeltaMinor), 0); const cogsReasons: string[] = [];
  if (finals.length > 0 && cogsStatus === "not_available") cogsReasons.push("لا توجد حركات استهلاك ذات قيمة تكلفة مثبتة مرتبطة بطلب نهائي؛ تستخدم القراءة Snapshot كبديل معلن."); if (cogsStatus === "partial") cogsReasons.push("تتوفر COGS مسجلة لبعض الأعمال النهائية فقط؛ تستخدم القراءة Snapshot لبقية الأعمال، فلا تُعرض كل التكلفة كـCOGS فعلية."); if (unallocatedInventoryCostMinor > 0) cogsReasons.push("توجد حركة استهلاك عامة أو بلا ارتباط صالح؛ لم توزع على عمل ولم تدخل COGS."); if (generalInventoryWasteMinor > 0) cogsReasons.push("يوجد هدر مخزون عام؛ لا يسمى COGS ولا يحمل على عمل تلقائيًا."); const cogsNextAction = cogsStatus === "recorded" && unallocatedInventoryCostMinor === 0 && generalInventoryWasteMinor === 0 ? "راجع أن حركات الاستهلاك تغطي المواد المقصودة؛ تبقى بقية عناصر التكلفة من Snapshot." : "سجل استهلاكًا بقيمة تكلفة محفوظة واربطه بعمل مكتمل، أو راجع الحركات العامة؛ لا تدخل صفرًا عند غياب الدليل."; return { snapshotDirectCostMinor, recordedCogsMinor, effectiveDirectCostMinor, cogsStatus, cogsMissingOrderCount, unallocatedInventoryCostMinor, generalInventoryWasteMinor, cogsReasons, cogsNextAction };
}
...
export class ProjectFinancialService {
  ...
  async readPosition(): Promise<FinanceResult<ProjectFinancialPosition>> {
    const [ordersResult, eventsResult, purchasesResult, walletsResult, continuityResult, ownerMovementsResult] = await Promise.all([this.store.listOrders(), this.store.listFinancialEvents(), this.store.listSupplierPurchases(), this.store.listCashWallets(), this.store.listCashContinuityEntries(), this.store.listOwnerMovements()]);
    if (!ordersResult.ok || !eventsResult.ok || !purchasesResult.ok || !walletsResult.ok || !continuityResult.ok || !ownerMovementsResult.ok) return { ok: false, code: "storage_error", message: "تعذر قراءة السجلات المالية المحلية." };
    const orderPulse = summarizeLocalCraftOrders(ordersResult.value); const project = summarizeFinancialEvents(eventsResult.value); const supplierMaterialPayablesMinor = purchasesResult.value.reduce((sum, purchase) => sum + purchase.payableMinor, 0); const supplierPurchaseCashPaidMinor = purchasesResult.value.reduce((sum, purchase) => sum + purchase.paidMinor, 0); const unallocatedCashMinor = orderPulse.registeredCollectionsMinor + project.cashMinor - supplierPurchaseCashPaidMinor; const walletCashMinor = continuityResult.value.reduce((sum, entry) => sum + entry.cashDeltaMinor, 0); const ownerCapitalFromMovementsMinor = ownerMovementsResult.value.reduce((sum: number, movement: OwnerMovement) => sum + movement.ownerCapitalDeltaMinor, 0);
    return { ok: true, value: { recordedCashMinor: unallocatedCashMinor + walletCashMinor, customerReceivablesMinor: orderPulse.registeredDebtMinor, supplierPayablesMinor: project.payableMinor + supplierMaterialPayablesMinor, ownerCapitalRecordedMinor: project.ownerCapitalMinor + ownerCapitalFromMovementsMinor, operatingExpensesRecordedMinor: project.operatingExpenseMinor, orderCollectionsMinor: orderPulse.registeredCollectionsMinor, projectEventCount: project.eventCount, supplierPurchaseCount: purchasesResult.value.length, supplierMaterialPayablesMinor, walletCashMinor, unallocatedCashMinor, cashWalletCount: walletsResult.value.length, truth: "الكاش المسجل يجمع رصيد المحافظ المعلن والكاش غير الموزع من الطلبات والأحداث وشراء المواد. استحقاق المالك لا يغير الكاش، بينما حركة السحب أو الإرجاع الفعلية تغير المحفظة بسببها؛ الاستثمار الجديد مستقل عن الاستحقاق." } };
  }
  ...
  async readRecordedPeriodResult(from: string, to: string): Promise<FinanceResult<RecordedPeriodResult>> {
    const [ordersResult, eventsResult, movementsResult] = await Promise.all([this.store.listOrders(), this.store.listFinancialEvents(), this.store.listInventoryMovements()]);
    if (!ordersResult.ok || !eventsResult.ok || !movementsResult.ok) return { ok: false, code: "storage_error", message: "تعذر قراءة نتيجة الفترة المحلية." };
    if (!isValidLocalDate(from) || !isValidLocalDate(to) || from > to) return { ok: true, value: { from, to, recognizedRevenueMinor: 0, recognizedDirectCostMinor: 0, snapshotDirectCostMinor: 0, recordedCogsMinor: 0, effectiveDirectCostMinor: 0, cogsStatus: "not_available", cogsMissingOrderCount: 0, unallocatedInventoryCostMinor: 0, generalInventoryWasteMinor: 0, cogsReasons: [], cogsNextAction: "صحح حدود الفترة قبل مراجعة COGS.", recordedOperatingExpenseMinor: 0, projectOperatingExpenseMinor: 0, sharedProjectExpenseMinor: 0, sharedUnallocatedExpenseMinor: 0, legacyUnclassifiedExpenseMinor: 0, sharedEstimatedExpenseCount: 0, sharedMissingBasisCount: 0, sharedUnallocatedExpenseCount: 0, legacyUnclassifiedExpenseCount: 0, resultMinor: null, finalOrderCount: 0, excludedOrderCount: 0, expenseNeedsReviewCount: 0, status: "invalid", reasons: ["الفترة المحلية غير صالحة؛ لا يمكن بناء نتيجة قابلة للقراءة."], truth: "لا توجد نتيجة رقمية لهذه الفترة لأن حدودها غير صالحة. صحح الشهر أو الفترة قبل الاعتماد على القراءة." } };
    ...
    const recognizedRevenueMinor = finals.reduce((total, item) => total + item.order.recognizedRevenueMinor, 0); const recognizedDirectCostMinor = finals.reduce((total, item) => total + item.order.recognizedCostMinor, 0); const cogs = derivePeriodCogs(finals, movementsResult.value); const periodEvents = eventsResult.value.filter((event) => inPeriod(event.occurredOn)); const operatingEvents = periodEvents.filter((event) => isRecordedOperatingExpense(event)); const sharedUnallocatedEvents = periodEvents.filter(sharedExpenseIsUnallocated); const sharedUnallocatedSources = sharedUnallocatedEvents.filter((event) => event.correctionType !== "reverse"); const reviewableOperatingEvents = [...operatingEvents.filter((event) => event.operatingExpenseDeltaMinor > 0), ...sharedUnallocatedSources]; const projectEvents = operatingEvents.filter((event) => event.expenseContext?.relationship === "project"); const sharedEvents = operatingEvents.filter((event) => event.expenseContext?.relationship === "shared"); const legacyEvents = operatingEvents.filter((event) => !event.expenseContext); const recordedOperatingExpenseMinor = operatingEvents.reduce((total, event) => total + event.operatingExpenseDeltaMinor, 0); const projectOperatingExpenseMinor = projectEvents.reduce((total, event) => total + event.operatingExpenseDeltaMinor, 0); const sharedProjectExpenseMinor = sharedEvents.reduce((total, event) => total + event.operatingExpenseDeltaMinor, 0); const sharedUnallocatedExpenseMinor = sharedUnallocatedEvents.reduce((total, event) => total + (event.correctionType === "reverse" ? -1 : 1) * (event.expenseContext?.sharedProjectShare?.totalAmountMinor ?? event.amountMinor), 0); const legacyUnclassifiedExpenseMinor = legacyEvents.reduce((total, event) => total + event.operatingExpenseDeltaMinor, 0); const sharedEstimatedExpenseCount = reviewableOperatingEvents.filter((event) => event.expenseContext?.relationship === "shared" && event.expenseContext.knowledge !== "known" && !sharedExpenseIsUnallocated(event)).length; const sharedMissingBasisCount = reviewableOperatingEvents.filter((event) => event.expenseContext?.relationship === "shared" && sharedExpenseHasMissingBasis(event)).length; const sharedUnallocatedExpenseCount = sharedUnallocatedSources.length; const legacyUnclassifiedExpenseCount = reviewableOperatingEvents.filter((event) => !event.expenseContext).length; const expenseNeedsReviewCount = reviewableOperatingEvents.filter(expenseNeedsReview).length; const reasons: string[] = []; if (excludedOrderCount > 0) reasons.push("توجد طلبات مسلّمة مستبعدة بسبب درجة المعرفة أو المراجعة."); if (sharedEstimatedExpenseCount > 0) reasons.push("توجد حصة مشروع مشتركة تقديرية أو تحتاج مراجعة."); if (sharedMissingBasisCount > 0) reasons.push("توجد حصة مشروع مشتركة بلا مصدر موثق."); if (sharedUnallocatedExpenseCount > 0) reasons.push("توجد مصاريف مشتركة غير محملة؛ حدد حصة المشروع قبل خصمها من النتيجة."); if (legacyUnclassifiedExpenseCount > 0) reasons.push("توجد مصروفات قديمة بلا سياق مالي."); const incomplete = reasons.length > 0;
    return { ok: true, value: { from, to, recognizedRevenueMinor, recognizedDirectCostMinor, snapshotDirectCostMinor: cogs.snapshotDirectCostMinor, recordedCogsMinor: cogs.recordedCogsMinor, effectiveDirectCostMinor: cogs.effectiveDirectCostMinor, cogsStatus: cogs.cogsStatus, cogsMissingOrderCount: cogs.cogsMissingOrderCount, unallocatedInventoryCostMinor: cogs.unallocatedInventoryCostMinor, generalInventoryWasteMinor: cogs.generalInventoryWasteMinor, cogsReasons: cogs.cogsReasons, cogsNextAction: cogs.cogsNextAction, recordedOperatingExpenseMinor, projectOperatingExpenseMinor, sharedProjectExpenseMinor, sharedUnallocatedExpenseMinor, legacyUnclassifiedExpenseMinor, sharedEstimatedExpenseCount, sharedMissingBasisCount, sharedUnallocatedExpenseCount, legacyUnclassifiedExpenseCount, resultMinor: recognizedRevenueMinor - cogs.effectiveDirectCostMinor - recordedOperatingExpenseMinor, finalOrderCount: finals.length, excludedOrderCount, expenseNeedsReviewCount, status: incomplete ? "incomplete" : "recorded_only", reasons, truth: incomplete ? "هذا هو صافي الربح التشغيلي المسجل من البنود الداخلة في الفترة، لكنه يحتاج مراجعة للأسباب الظاهرة. الرقم لا يشمل المصدر المشترك غير المحمل، وتوضح قراءة COGS هل استُخدم استهلاك مثبت أم Snapshot بديل؛ هذه القراءة ليست صافي ربح نهائيًا." : "هذا هو صافي الربح التشغيلي المسجل من الأعمال المكتملة والتكلفة المباشرة المستخدمة وفق مصدرها ومصروفات الفترة. لا يؤكد COGS كاملًا خارج الاستهلاك المثبت أو Snapshot، ولا التوزيع على المنتجات أو الضرائب؛ لذلك ليس صافي ربح نهائيًا." } };
  }
  ...
  async readFinancialInsights(from: string, to: string): Promise<FinanceResult<FinancialInsights>> {
    ...
    const coverageReasons: string[] = []; if (finals.length === 0) coverageReasons.push("لا توجد طلبات مسلّمة بنتيجة نهائية في الفترة."); if (periodResult.value.excludedOrderCount > 0) coverageReasons.push("توجد طلبات مسلّمة مستبعدة بسبب درجة المعرفة أو المراجعة."); if (reviewableFixed.some((event) => event.expenseContext?.knowledge !== "known")) coverageReasons.push("توجد مصروفات ثابتة تحتاج مراجعة أو تقديرًا."); if (reviewableOperating.some((event) => event.expenseContext?.behavior === "variable" || event.expenseContext?.behavior === "mixed" || event.expenseContext?.behavior === "unknown")) coverageReasons.push("توجد مصروفات متغيرة أو مختلطة لا تحمل تلقائيًا على هامش المساهمة."); if (movementCount > 0) coverageReasons.push("توجد حركات مخزون فعلية؛ تعرض نتيجة الفترة COGS الاختيارية عند اكتمال دليلها، لكنها لا تعيد كتابة Snapshot أو هامش اسم العمل."); if (directMarginMinor <= 0) coverageReasons.push("الهامش المباشر المسجل غير موجب."); if (fixedExpenseMinor <= 0) coverageReasons.push("لا توجد مصروفات ثابتة مسجلة ومعروفة للفترة.");
  ...
  async record(input: FinancialRecordInput): Promise<FinanceResult<FinancialEvent>> {
    ...
    if (input.sharedExpense && expenseContext?.relationship !== "shared") return { ok: false, code: "validation_error", message: "خيارات حصة المصروف لا تستخدم إلا مع مصروف مشترك." };
    if ((input.type === "operating_expense_cash" || input.type === "operating_expense_payable") && !expenseContext) return { ok: false, code: "validation_error", message: "حدد سياق المصروف ودرجة معرفته قبل الحفظ." };
    if (expenseContext?.relationship === "shared" && input.sharedExpense) {
      ...
      if (input.sharedExpense.mode === "percentage") {
        const calculatedShareMinor = calculateSharedProjectShareMinor(input.sharedExpense.sharedTotalAmountMinor, input.sharedExpense.sharedPercentageBps);
        amountMinor = calculatedShareMinor;
        expenseContext = { ...expenseContext, knowledge: "known", sharedProjectShare: { basis: "agreed_percentage", note, allocation: "allocated", totalAmountMinor: input.sharedExpense.sharedTotalAmountMinor, percentageBps: input.sharedExpense.sharedPercentageBps, calculatedShareMinor } };
      } else if (input.sharedExpense.mode === "defer") {
        amountMinor = input.sharedExpense.sharedTotalAmountMinor;
        expenseContext = { ...expenseContext, knowledge: "needs_review", sharedProjectShare: { basis: "needs_review", note, allocation: "unallocated", totalAmountMinor: input.sharedExpense.sharedTotalAmountMinor, percentageBps: null, calculatedShareMinor: null } };
      } else if (input.sharedExpense.mode === "estimate") {
        if (amountMinor === undefined) return { ok: false, code: "validation_error", message: "أدخل حصة المالك التقديرية قبل الحفظ." };
        expenseContext = { ...expenseContext, knowledge: "estimated", sharedProjectShare: { basis: "owner_estimate", note, allocation: "allocated", totalAmountMinor: null, percentageBps: null, calculatedShareMinor: null } };
      } else {
        if (amountMinor === undefined) return { ok: false, code: "validation_error", message: "أدخل مبلغ حصة المشروع قبل الحفظ." };
        expenseContext = { ...expenseContext, knowledge: "known", sharedProjectShare: { basis: "agreed_fixed_share", note, allocation: "allocated", totalAmountMinor: null, percentageBps: null, calculatedShareMinor: null } };
      }
    }
    if ((input.type === "operating_expense_cash" || input.type === "operating_expense_payable") && expenseContext?.relationship === "shared" && !expenseContext.sharedProjectShare) return { ok: false, code: "validation_error", message: "حدد كيف عرفت حصة المشروع من المصروف المشترك قبل الحفظ." };
    if (amountMinor === undefined) return { ok: false, code: "validation_error", message: "أدخل مبلغًا صالحًا قبل الحفظ." };
    if (input.type === "payable_settlement_cash") {
      ...
      if (amountMinor > source.amountMinor - paid) return { ok: false, code: "validation_error", message: "لا يمكن أن يتجاوز التسديد المتبقي المسجل على هذا الالتزام." };
    }
    try {
      const event = createFinancialEvent({ id: id(), type: input.type, amountMinor, occurredOn: input.occurredOn, recordedAt: this.now(), idempotencyKey: input.idempotencyKey, note: input.note, counterparty: input.counterparty, relatedEventId: input.relatedEventId, expenseContext });
