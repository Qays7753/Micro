  ...
  async readRecordedPeriodResult(from: string, to: string): Promise<FinanceResult<RecordedPeriodResult>> {
    ...
    return {
      ...
      value: {
        ...
        reasons,
        truth: incomplete
          ? "هذا هو صافي الربح التشغيلي المسجل من البنود الداخلة في الفترة، لكنه يحتاج مراجعة للأسباب الظاهرة. الرقم لا يشمل المصدر المشترك غير المحمل، وتوضح قراءة تكلفة البيع هل استُخدم استهلاك مثبت أم نسخة تكلفة بديلة؛ هذه القراءة ليست صافي ربح نهائيًا."
          : "هذا هو صافي الربح التشغيلي المسجل من الأعمال المكتملة والتكلفة المباشرة المستخدمة وفق مصدرها ومصروفات الفترة. لا يؤكد تكلفة البيع كاملة خارج الاستهلاك المثبت أو نسخة التكلفة، ولا التوزيع على المنتجات أو الضرائب؛ لذلك ليس صافي ربح نهائيًا.",
      },
    };
  }
  ...
  async readFinancialInsights(from: string, to: string): Promise<FinanceResult<FinancialInsights>> {
    ...
    return {
      ...
      value: {
        ...
        liquidity,
        truth:
          "هذه مؤشرات مشتقة من السجل المحلي في الفترة؛ لا تحفظ نتيجة جديدة ولا تحول الكاش أو المخزون أو الذمم إلى صافي ربح. نتيجة الفترة تعرض تكلفة البيع الاختيارية وفق العقد، بينما تبقى مؤشرات التغطية وهامش اسم العمل محافظة على نسخة التكلفة الخاصة بالطلب.",
      },
    };
  }
