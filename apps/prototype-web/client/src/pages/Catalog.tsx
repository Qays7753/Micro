...
export default function Catalog() {
  ...
  const perUnitPreview = policyKind === "per_output_unit" ? buildCatalogPerUnitPreview(selectedReading?.outputQuantityMilli ?? null, parseCatalogJodMinor(policyRate), selectedItemUnit?.nameAr ?? selectedItem?.unitLabel ?? "وحدة كاملة") : null;
  ...
  async function savePolicy() {
    ...
    const parsedRateMinor = policyKind === "per_output_unit" || policyKind === "actual_time" ? parseCatalogJodMinor(policyRate) : null;
    ...
    if ((policyKind === "manual_amount" && amountMinor === null) || ((policyKind === "per_output_unit" || policyKind === "actual_time") && parsedRateMinor === null) || (policyKind === "completed_revenue_percentage" && percentageBps === null)) { setMessage("أدخل أساس التحميل بصيغة موجبة واضحة؛ لا نستخدم صفرًا بدل البيانات الناقصة."); return; }
  ...
  const conversionPreview = conversionFromUnit && conversionToUnit && conversionNumerator.trim() && conversionDenominator.trim()
    ? buildCatalogConversionPreview(conversionFromUnit.nameAr, conversionToUnit.nameAr, parsePositiveSafeInteger(conversionNumerator) ?? 0, parsePositiveSafeInteger(conversionDenominator) ?? 0)
    : null;
