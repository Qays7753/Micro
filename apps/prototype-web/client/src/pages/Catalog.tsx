...
export const catalogConversionExactnessWarning = "لا يمكن تمثيل هذا المثال بدقة؛ صحح العامل بدل التقريب الخفي.";
export const catalogConversionDirectionText = (fromName: string, toName: string) => `المصدر: ${fromName.trim()} | الوجهة: ${toName.trim()}`;
export const buildCatalogConversionPreview = (fromName: string, toName: string, numerator: number, denominator: number, sampleQuantityMilli = 12_000) => {
  if (!Number.isSafeInteger(numerator) || numerator <= 0 || !Number.isSafeInteger(denominator) || denominator <= 0 || !Number.isSafeInteger(sampleQuantityMilli) || sampleQuantityMilli <= 0) return { exact: false, sourceQuantityMilli: sampleQuantityMilli, targetQuantityMilli: null, text: null, warning: catalogConversionExactnessWarning };
  const scaledNumerator = sampleQuantityMilli * numerator;
  if (!Number.isSafeInteger(scaledNumerator) || scaledNumerator % denominator !== 0) return { exact: false, sourceQuantityMilli: sampleQuantityMilli, targetQuantityMilli: null, text: null, warning: catalogConversionExactnessWarning };
  const targetQuantityMilli = scaledNumerator / denominator;
  if (!Number.isSafeInteger(targetQuantityMilli) || targetQuantityMilli <= 0) return { exact: false, sourceQuantityMilli: sampleQuantityMilli, targetQuantityMilli: null, text: null, warning: catalogConversionExactnessWarning };
  const sourceLabel = fromName.trim() || "وحدة المصدر";
  const targetLabel = toName.trim() || "وحدة الوجهة";
  return { exact: true, sourceQuantityMilli: sampleQuantityMilli, targetQuantityMilli, text: `${quantityLabel(sampleQuantityMilli)} ${sourceLabel} × ${numerator} ÷ ${denominator} = ${quantityLabel(targetQuantityMilli)} ${targetLabel}`, warning: null };
};
...
export const parseCatalogJodMinor = (value: string) => { const minor = parseEnglishNumericText(value.trim(), "money"); return minor !== null && minor > 0 ? minor : null; };
export const parseCatalogPercentageBps = (value: string) => { const bps = parseEnglishNumericText(value.trim(), "percentage"); return bps !== null && bps >= 1 && bps <= 10_000 ? bps : null; };
export const catalogAllocationKindLabel = (kind: RecurringWorkReading["policies"][number]["kind"]) => ({ manual_amount: "مبلغ يدوي للفترة", per_output_unit: "معدل لكل 1.000 وحدة كاملة", actual_time: "معدل لكل دقيقة فعلية", completed_revenue_percentage: "نسبة من الإيراد المكتمل" })[kind];
export const catalogPerUnitRateLabel = (unitName: string) => `المعدل لكل 1.000 ${unitName.trim() || "وحدة كاملة"} · د.أ`;
export const catalogPerUnitRoundingNote = "يُقرب مجموع الفترة مرة واحدة إلى أقرب قرش.";
export const buildCatalogPerUnitPreview = (quantityMilli: number | null, rateMinorPerWholeUnit: number | null, unitName: string) => {
  if (quantityMilli === null || rateMinorPerWholeUnit === null) return { allocationMinor: null, text: null, warning: "تحتاج المعاينة إلى كمية final ومعدل صالحين." };
  if (!Number.isSafeInteger(quantityMilli) || quantityMilli <= 0 || !Number.isSafeInteger(rateMinorPerWholeUnit) || rateMinorPerWholeUnit <= 0 || rateMinorPerWholeUnit > Number.MAX_SAFE_INTEGER / quantityMilli) return { allocationMinor: null, text: null, warning: "لا يمكن الحساب بأمان؛ راجع الكمية والمعدل قبل الحفظ." };
  const rawMinor = rateMinorPerWholeUnit * quantityMilli;
  if (!Number.isSafeInteger(rawMinor) || rawMinor > Number.MAX_SAFE_INTEGER - 500) return { allocationMinor: null, text: null, warning: "تجاوز الحساب الدقة الآمنة؛ لم يُقرب الرقم." };
  const allocationMinor = Math.floor((rawMinor + 500) / 1000);
  const label = unitName.trim() || "وحدة كاملة";
  return { allocationMinor, text: `${(quantityMilli / 1000).toFixed(3)} ${label} × ${(rateMinorPerWholeUnit / 100).toFixed(2)} د.أ لكل 1.000 ${label} = ${(allocationMinor / 100).toFixed(2)} د.أ`, warning: allocationMinor === 0 ? "الناتج 0.00 د.أ نتيجة حسابية معلنة بعد تقريب مجموع الفترة، وليس غياب بيانات." : null };
};
...
export default function Catalog() {
  ...
  const perUnitPreview = policyKind === "per_output_unit" ? buildCatalogPerUnitPreview(selectedReading?.outputQuantityMilli ?? null, policyRate, selectedItemUnit?.nameAr ?? selectedItem?.unitLabel ?? "وحدة كاملة") : null;
  ...
  function startPolicyRevision(policy: RecurringWorkReading["policies"][number]) {
    ...
    const existingRateMinor = policy.kind === "per_output_unit" ? policy.rateMinorPerWholeUnit : policy.rateMinor;
    setSelectedItemId(policy.catalogItemId); setPolicyRevisionId(policy.id); setPolicyKind(policy.kind); setPolicyAmount(policy.amountMinor); setPolicyRate(existingRateMinor); setPolicyPercentage(policy.percentageBps); setPolicyAmountValid(true); setPolicyRateValid(true); setPolicyPercentageValid(true); setPolicyUnitId(policy.unitId ?? ""); setPolicySource(policy.source); setPolicyReason(policy.reason); setPolicyNote(policy.note); setPolicyPeriodFrom(start); setPolicyPeriodTo(policy.endsOn ? `${year}-${String(monthNumber).padStart(2, "0")}-${String(monthEndDate(year!, monthNumber!)).padStart(2, "0")}` : month.to); setMessage("أنت تعدل نسخة جديدة؛ ستبقى السياسة السابقة محفوظة وتنتهي قبل بداية النسخة الجديدة.");
  }
  ...
  async function savePolicy() {
    ...
    const parsedRateMinor = policyKind === "per_output_unit" || policyKind === "actual_time" ? policyRate : null;
    ...
    if ((policyKind === "manual_amount" && (!policyAmountValid || amountMinor === null || amountMinor <= 0)) || ((policyKind === "per_output_unit" || policyKind === "actual_time") && (!policyRateValid || parsedRateMinor === null || parsedRateMinor <= 0)) || (policyKind === "completed_revenue_percentage" && (!policyPercentageValid || percentageBps === null || percentageBps <= 0 || percentageBps > 10_000))) { setMessage("أدخل أساس التحميل بصيغة موجبة واضحة؛ لا نستخدم صفرًا بدل البيانات الناقصة."); return; }
    ...
    setSaving(false);
    if (!result.ok) { setMessage(result.message); return; }
    setPolicyAmount(null); setPolicyRate(null); setPolicyPercentage(null); setPolicyAmountValid(true); setPolicyRateValid(true); setPolicyPercentageValid(true); setPolicySource(""); setPolicyReason(""); setPolicyNote(""); setPolicyRevisionId(null); notifyDataChanged(); await load(); setMessage("تم حفظ سياسة التحميل كقراءة تفسيرية مؤرخة؛ لم ينشأ منها قيد مالي أو تغيير في Snapshot.");
  }
  ...
  async function createConversion(): Promise<boolean> {
    ...
    if (!conversionNumeratorValid || !conversionDenominatorValid || numerator === null || denominator === null || numerator <= 0 || denominator <= 0) {
    ...
    setConversionFrom(""); setConversionTo(""); setConversionNumerator(null); setConversionDenominator(null); setConversionNumeratorValid(true); setConversionDenominatorValid(true); setConversionNote(""); notifyDataChanged(); await load(); setMessage("تم حفظ التحويل المباشر الصريح. لن نمر عبر وحدات أخرى تلقائيًا.");
    return true;
  }
  ...
  function addComponent() {
    ...
    if (!componentName.trim() || quantityMilli === null || !componentQuantityValid || quantityMilli <= 0 || !componentUnitId) { setMessage("أدخل اسم المكوّن وكمية موجبة حتى ثلاثة منازل ووحدة نشطة."); return; }
    ...
    setComponentName(""); setComponentQuantity(null); setComponentQuantityValid(true);
  }

  function resetTemplateForm() {
    setEditingTemplateId(null); setTemplateBaseline(null); setTemplateTitle(""); setTemplateNote(""); setTemplateComponents([]); setYieldEnabled(false); setYieldQuantity(null); setYieldQuantityValid(true);
  }
  ...
  async function saveTemplate(): Promise<boolean> {
    ...
    if (yieldEnabled && (!yieldQuantityValid || parsedYield === null || parsedYield <= 0 || !yieldUnitId)) { setMessage("أدخل كمية ناتج موجبة حتى ثلاثة منازل ووحدة ناتج."); return false; }
    setSaving(true); setMessage(null);
  ...
  const currentTemplateFingerprint = JSON.stringify({ title: templateTitle.trim(), note: templateNote.trim(), components: templateComponents, yield: yieldEnabled ? { quantity: yieldQuantity, unitId: yieldUnitId } : null });
  const hasTemplateDraft = Boolean(templateTitle.trim() || templateNote.trim() || templateComponents.length || yieldEnabled || yieldQuantity !== null);
  const templateDirty = isCatalogTemplateDirty(currentTemplateFingerprint, templateBaseline, hasTemplateDraft);
  const conversionDirty = Boolean(conversionFrom || conversionTo || conversionNumerator !== null || conversionDenominator !== null || !conversionNumeratorValid || !conversionDenominatorValid || conversionNote.trim());
  ...
  const conversionPreview = conversionFromUnit && conversionToUnit && conversionNumerator !== null && conversionDenominator !== null
    ? buildCatalogConversionPreview(conversionFromUnit.nameAr, conversionToUnit.nameAr, conversionNumerator, conversionDenominator)
    : null;
  ...
  return <section className="micro-page">
    ...
    <section className="micro-form-card">
      ...
      {showMeasurements ? <div className="micro-subsection-stack">
        ...
        <div className="micro-subsection"><div className="micro-subsection-heading"><div><span className="micro-overline">تحويل مباشر</span><h3>أضف تحويلًا واضحًا</h3></div><p>المعادلة: <bdi dir="ltr">كمية المصدر × البسط ÷ المقام = الناتج بوحدة الوجهة</bdi>، ولا نقرب إذا تعذر تمثيله.</p></div><div className="micro-form-grid"><label className="micro-field"><span>المصدر</span><select value={conversionFrom} onChange={event => setConversionFrom(event.target.value)}><option value="">اختر وحدة المصدر</option>{activeUnits.map(unit => <option key={unit.id} value={unit.id}>{unit.nameAr} · {dimensionLabel(unit.dimension)}</option>)}</select></label><label className="micro-field"><span>الوجهة</span><select value={conversionTo} onChange={event => setConversionTo(event.target.value)}><option value="">اختر وحدة الوجهة</option>{activeUnits.map(unit => <option key={unit.id} value={unit.id}>{unit.nameAr} · {dimensionLabel(unit.dimension)}</option>)}</select></label><label className="micro-field"><span>البسط <small dir="rtl">موجب</small></span><EnglishNumberInput value={conversionNumerator} kind="integer" onNumericChange={setConversionNumerator} onTextValidityChange={setConversionNumeratorValid} onEmptyChange={() => setConversionNumerator(null)} allowEmpty aria-label="بسط التحويل" /></label><label className="micro-field"><span>المقام <small dir="rtl">موجب</small></span><EnglishNumberInput value={conversionDenominator} kind="integer" onNumericChange={setConversionDenominator} onTextValidityChange={setConversionDenominatorValid} onEmptyChange={() => setConversionDenominator(null)} allowEmpty aria-label="مقام التحويل" /></label><label className="micro-field micro-field-wide"><span>لماذا هذا التحويل؟</span><input value={conversionNote} onChange={event => setConversionNote(event.target.value)} placeholder="مثال: 1 كيلوغرام = 1000 غرام" /></label></div>{conversionPreview ? <div className={`micro-conversion-preview ${conversionPreview.exact ? "" : "micro-conversion-preview-warning"}`} role="status"><strong>{conversionPreview.exact ? catalogConversionDirectionText(conversionFromUnit?.nameAr ?? "وحدة المصدر", conversionToUnit?.nameAr ?? "وحدة الوجهة") : "المعاينة غير دقيقة"}</strong><p>{conversionPreview.exact && conversionFromUnit && conversionToUnit ? <span className="micro-conversion-equation" dir="ltr" aria-label={conversionPreview.text ?? undefined}><bdi>{quantityLabel(conversionPreview.sourceQuantityMilli)}</bdi><span className="micro-conversion-unit" dir="rtl">{conversionFromUnit.nameAr}</span><bdi>× {conversionNumerator} ÷ {conversionDenominator} =</bdi><bdi>{quantityLabel(conversionPreview.targetQuantityMilli ?? 0)}</bdi><span className="micro-conversion-unit" dir="rtl">{conversionToUnit.nameAr}</span></span> : <span>{conversionPreview.warning}</span>}</p></div> : null}<button className="micro-button micro-button-secondary" type="button" disabled={!conversionFrom || !conversionTo || !conversionNumerator || !conversionDenominator || !conversionNote.trim()} onClick={createConversion}><GitCompareArrows aria-hidden="true" /> أضف تحويلًا صريحًا</button><div className="micro-list micro-list-compact">{conversions.length ? conversions.map(conversion => { const from = units.find(unit => unit.id === conversion.fromUnitId); const to = units.find(unit => unit.id === conversion.toUnitId); return <div className="micro-list-item" key={conversion.id}><div><strong>{catalogConversionDirectionText(from?.nameAr ?? "وحدة قديمة", to?.nameAr ?? "وحدة قديمة")}</strong><p dir="ltr">× {conversion.numerator} ÷ {conversion.denominator} · {conversion.note}{conversion.active ? "" : " · موقوف"}</p></div>{conversion.active ? <button className="micro-button micro-button-secondary" type="button" onClick={() => deactivateConversion(conversion.id)}><ArchiveX aria-hidden="true" /> إيقاف</button> : null}</div>; }) : <p className="micro-empty-copy">لا توجد تحويلات. لن نحتاج إليها ما دامت الوحدات متطابقة.</p>}</div></div>
      </div> : null}
    </section>
