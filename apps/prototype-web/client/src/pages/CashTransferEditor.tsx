/** Style: Micro «مسار القرار» — a transfer has two visible sides and never changes total cash. */
/* مبدأ Micro: التحويل الداخلي ينقل الكاش بين المحافظ فقط، ويعرض تاريخه دون أثر ربحي. */
...
export default function CashTransferEditor() {
  ...
  const operationKey = useRef(`cash-transfer-ui-${globalThis.crypto?.randomUUID?.() ?? Date.now()}`);
  useEffect(() => {
  ...
  return (
    <section className="micro-page micro-finance-page">
      ...
      <div className="micro-page-heading">
        <span className="micro-overline">حركة داخلية</span>
        <h1>حوّل كاشًا بين محافظك</h1>
        <p>ينقص مصدر التحويل ويزيد وجهته بالمبلغ نفسه. لا يغير الكاش الكلي أو الربح أو المصروف.</p>
      </div>
      ...
      <section className="micro-form-card">
        ...
        <LocalDateField label="تاريخ التحويل" value={date} onChange={event => setDate(event.target.value)} />
        <label className="micro-field">
          <span>ما الذي حدث؟</span>
          <textarea
            value={note}
            onChange={event => setNote(event.target.value)}
            placeholder="مثال: أودعت كاش الدرج في الحساب البنكي"
          />
        </label>
        {message ? (
