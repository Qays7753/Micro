/** Style: Micro «مسار القرار» — introduce one declared cash location without confusing it with sales or capital. */
/* مبدأ Micro: يظل رصيد البداية أثرًا محليًا واضحًا، ويعرض تاريخه دون تغيير معناه المالي. */
...
export default function CashWalletEditor() {
  ...
  async function save(): Promise<boolean> {
    if (!name.trim() || !note.trim() || !validAmount || openingMinor < 0) {
      setMessage("أدخل اسم المحفظة ورصيد بداية صالحًا بالأرقام 0–9.");
      return false;
    }
    setSaving(true);
    const result = await cashContinuity.openWallet({
      name,
      kind,
      openingMinor,
      occurredOn: date,
  ...
  return (
    <section className="micro-page micro-finance-page">
      ...
      <div className="micro-page-heading">
        <span className="micro-overline">بداية معلنة</span>
        <h1>أين يوجد الكاش الآن؟</h1>
        <p>أضف المكان والمبلغ كما تعرفهما في يوم البدء. لا يعالج هذا ربحًا أو مبيعات سابقة.</p>
      </div>
      <section className="micro-decision-card">
        <WalletCards aria-hidden="true" />
        <div>
          <span>حد الحقيقة</span>
          <strong>رصيد البداية ليس مال مالك ولا دخلًا.</strong>
          <p>إنه نقطة واضحة لتبدأ منها مراجعة كاش مشروعك على هذا الجهاز.</p>
        </div>
      </section>
      <section className="micro-form-card">
        <label className="micro-field">
          ...
          <input
            ...
            placeholder="مثال: درج الورشة"
          />
        </label>
        <label className="micro-field">
          <span>نوع المكان</span>
          <select value={kind} onChange={event => setKind(event.target.value as CashWalletKind)}>
            <option value="cash_drawer">درج كاش</option>
        ...
        <label className="micro-field">
          <span>رصيد البداية بالدينار الأردني</span>
          <EnglishNumberInput
            value={openingMinor}
            kind="money"
            onNumericChange={setOpeningMinor}
            onTextValidityChange={setValidAmount}
            aria-label="رصيد البداية"
          />
          <small>يمكن أن يكون صفرًا إذا أردت تعريف المكان الآن فقط.</small>
        </label>
        <LocalDateField label="تاريخ البداية" value={date} onChange={event => setDate(event.target.value)} />
        <label className="micro-field">
          <span>بيان مختصر</span>
          <textarea
            value={note}
            onChange={event => setNote(event.target.value)}
            placeholder="مثال: عدّ الدرج في بداية استخدام Micro"
          />
        </label>
        {message ? (
