...
export default function CashCount() {
  ...
  async function settle(): Promise<boolean> {
    ...
    if (differenceMinor === 0) {
      setMessage("العدّ يطابق الرصيد المسجل — لا تسوية مطلوبة.");
      return false;
    }
    setSaving(true);
    setMessage(null);
            ...
            >
              ...
              <p>
                {differenceMinor === 0
                  ...
                  : differenceMinor > 0
                    ? "غالبًا قبضات لم تُسجَّل — ستُسجَّل الفرق زيادة بتاريخ اليوم، ولن يتغير أي رقم قديم."
                    : "غالبًا صرف أو نقص لم يُسجَّل — ستُسجَّل الفرق نقصًا بتاريخ اليوم، ولن يتغير أي رقم قديم."}
              </p>
            </section>
          ) : null}
          {message ? (
