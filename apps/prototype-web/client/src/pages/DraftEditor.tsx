...
export default function DraftEditor() {
  ...
  async function save(andContinue: boolean) {
    if (!draft) return;
    if (andContinue && !draft.itemName.trim()) { setMessage("وصف القطعة: اكتب وصفًا مختصرًا ثم أعد المحاولة للانتقال للتكلفة."); return; }
    if (!isQuantityValid) { setMessage("الكمية: استخدم أرقامًا إنجليزية صحيحة ثم أعد الحفظ."); return; }
    if (andContinue && !draft.specifications.trim()) { setMessage("ملاحظات التخصيص: أضف ما يلزم للاتفاق قبل الانتقال للتكلفة."); return; }
