...
function MonthDayDetail({ day, onOpen }: { day: ScheduleDay; onOpen: (item: ScheduledOrder) => void }) { const warnings = [day.conflictCount > 0 ? `تعارض في ${day.conflictCount} موعد` : null, day.overCapacity ? "يتجاوز السعة اليومية المعلنة" : null, day.unknownTimingCount > 0 ? `${day.unknownTimingCount} وقت غير محدد` : null].filter(Boolean); return <section className="micro-month-day-detail" aria-labelledby={`month-day-${day.date}`}><div className="micro-month-detail-heading"><div><span className="micro-overline">تفاصيل اليوم</span><h3 id={`month-day-${day.date}`}>{dateLabel(day.date)}</h3></div><span className="micro-number">{day.scheduledMinutes} دقيقة معروفة</span></div>{warnings.length > 0 ? <p className="micro-month-warning"><CircleAlert aria-hidden="true" />{warnings.join(" · ")}</p> : null}{day.items.length === 0 ? <p>لا مواعيد مسجلة في هذا اليوم.</p> : <div className="micro-month-day-list">{day.items.map((item) => <button type="button" key={item.schedule.id} onClick={() => onOpen(item)}><span className="micro-draft-symbol"><CalendarClock aria-hidden="true" /></span><span><strong>{item.order.order.itemName}</strong><small>{orderStatus[item.order.order.status] ?? "يحتاج مراجعة"} · {timingLabel(item.schedule)}</small></span><ChevronLeft aria-hidden="true" /></button>)}</div>}</section>; }
function ScheduleSection({ title, description, tone, items, onOpen }: { title: string; description: string; tone: "warning" | "accent" | "support"; items: readonly ScheduledOrder[]; onOpen: (item: ScheduledOrder) => void }) { return <section className="micro-schedule-section" data-tone={tone}><div className="micro-schedule-heading"><div><span className="micro-overline">{title}</span><h2>{description}</h2></div><span>{items.length}</span></div><div className="micro-draft-list">{items.map((item) => <button className="micro-draft-row" type="button" key={item.schedule.id} onClick={() => onOpen(item)}><span className="micro-draft-symbol"><CalendarClock aria-hidden="true" /></span><span><strong>{item.order.order.itemName}</strong><small>{orderStatus[item.order.order.status] ?? "يحتاج مراجعة"} · {item.schedule.scheduledFor}</small><small>{timingLabel(item.schedule)}</small><small className="micro-row-next-action">الفعل التالي: {item.order.order.nextAction}</small></span><ChevronLeft aria-hidden="true" /></button>)}</div></section>; }
