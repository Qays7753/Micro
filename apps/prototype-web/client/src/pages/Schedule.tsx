...
const timingLabel = (schedule: ScheduledOrder["schedule"]) =>
  schedule.scheduledTime && schedule.durationMinutes ? (
    <>
      <TimeValue value={schedule.scheduledTime} /> ·{" "}
      <IntegerValue value={schedule.durationMinutes} className="micro-inline-number" /> دقيقة
    </>
  ) : (
    "وقت غير محدد"
  );
...
export default function Schedule() {
  ...
  const decisionDay =
    (selectedDate ? month.days.find(day => day.date === selectedDate) : null) ?? overview.week[0];
...
function CapacityDecisionSurface({
  day,
  capacityMinutes,
  onOpen,
}: {
  ...
  return (
    <section className="micro-capacity-decision" aria-label={`قرار السعة ليوم ${dateLabel(day.date)}`}>
      <DecisionPanel
        label={`${decision.label} · ${dateLabel(day.date)}`}
      ...
      {day.conflictCount > 0 ? (
        <p className="micro-month-warning">
          ...
          التعارض مستقل عن قرار السعة: يوجد تعارض في {day.conflictCount} موعد؛ راجع الموعد القائم دون اعتبار
          ذلك رفضًا أو ضمانًا للتوفر.
        </p>
      ) : null}
    </section>
  );
...
function RecurrencePanel({
  ...
  async function create() {
    ...
    const result = await service.create({
      sourceScheduleId: selectedSourceId,
      frequency,
  ...
  return (
    <section className="micro-recurrence-panel" aria-labelledby="recurrence-title">
      ...
      {sources.length === 0 ? (
        <div className="micro-recurrence-empty">
          <CircleAlert aria-hidden="true" />
          <p>أنشئ أو ثبّت موعدًا نشطًا أولًا حتى يمكن اختيار مصدر للتكرار.</p>
        </div>
      ) : (
        <div className="micro-form-card">
          <label className="micro-field">
            <span>الموعد المصدر</span>
            <select
          ...
          <p className="micro-recurrence-note">
      ...
      {views.length > 0 ? (
        <div className="micro-recurrence-list">
          {views.map(view => (
            <article className="micro-recurrence-card" key={view.recurrence.id}>
              <div className="micro-recurrence-card-heading">
                <div>
                  <span className="micro-overline">
                    {view.recurrence.status === "active" ? "قالب نشط" : "قالب موقوف"}
                  </span>
                ...
                {view.recurrence.status === "active" ? (
                  <Repeat2 aria-label="قالب تكرار نشط" />
                ) : (
                  <XCircle aria-label="قالب تكرار موقوف" />
                )}
              </div>
              <div className="micro-recurrence-appearances">
                {view.appearances.length > 0 ? (
                  view.appearances.map(appearance => (
                    <span key={appearance.id}>
                      <b>
                        <LocalDateValue value={appearance.scheduledFor} />
                      </b>
                      <small>
                        {appearance.scheduledTime && appearance.durationMinutes ? (
                          <>
                            <TimeValue value={appearance.scheduledTime} /> ·{" "}
                            <IntegerValue
                              value={appearance.durationMinutes}
              ...
              {view.recurrence.status === "active" ? (
                cancellingId === view.recurrence.id ? (
                  <div className="micro-recurrence-cancel">
                    <label className="micro-field">
                      <span>سبب إيقاف المستقبل</span>
                      <textarea
                        value={cancelReason}
                        onChange={event => setCancelReason(event.target.value)}
                        placeholder="مثال: تغير نمط العمل"
                      />
                    </label>
                    <div className="micro-form-actions">
                      <button
...
function MonthSchedulePanel({
  ...
  onOpen: (item: ScheduledOrder) => void;
}) {
  const selectedDay = month.days.find(day => day.date === selectedDate) ?? null;
  return (
    <section className="micro-month-panel" aria-labelledby="monthly-schedule-title">
      <div className="micro-month-heading">
        <div>
          <span className="micro-overline">قراءة شهرية مشتقة</span>
          <h2 id="monthly-schedule-title">{monthLabel(month.month)}</h2>
          <p>من مواعيد الطلبات المحلية · توقيت عمّان</p>
        </div>
        <CalendarDays aria-hidden="true" />
      </div>
      <div className="micro-month-navigation">
        <button
      ...
      <dl className="micro-month-summary">
        <div>
          <dt>مواعيد تشغيلية نشطة</dt>
          <dd>
            <IntegerValue value={month.scheduledCount} />
          </dd>
        </div>
        <div>
          <dt>دقائق معروفة</dt>
          <dd>
            <IntegerValue value={month.scheduledMinutes} />
          </dd>
        </div>
        <div>
          <dt>وقت غير محدد</dt>
          <dd>
            <IntegerValue value={month.unknownTimingCount} />
          </dd>
        </div>
        <div>
          <dt>متأخر ضمن الشهر</dt>
          <dd>
            <IntegerValue value={month.overdueCount} />
          </dd>
        </div>
      </dl>
      {month.scheduledCount === 0 ? (
        <div className="micro-month-empty" role="status">
          <CircleAlert aria-hidden="true" />
          <div>
            <strong>لا توجد مواعيد تشغيلية نشطة لهذا الشهر</strong>
            <p>المواعيد المكتملة أو المغلقة مستبعدة؛ لا توجد التزامات نشطة مسجلة لقراءتها.</p>
          </div>
        </div>
      ) : null}
      <div
        className="micro-month-calendar-scroll"
        role="region"
        aria-label="أيام الشهر، مرر أفقيًا عند الحاجة"
      >
        <div className="micro-month-weekdays" aria-hidden="true">
          {weekdayLabels.map(label => (
            <span key={label}>{label}</span>
          ))}
        </div>
        <div className="micro-month-grid" role="grid" aria-label={`أيام ${monthLabel(month.month)}`}>
          {Array.from({ length: monthFirstDayOffset(month.month) }, (_, index) => (
            <span
              className="micro-month-cell micro-month-cell-empty"
              aria-hidden="true"
              key={`leading-${index}`}
            />
          ))}
          {month.days.map(day => (
            <MonthDayCell
              ...
              today={day.date === currentLocalDate()}
              onSelect={() => onSelectDate(selectedDate === day.date ? null : day.date)}
            />
          ))}
        </div>
      </div>
      {selectedDay ? (
        <MonthDayDetail day={selectedDay} onOpen={onOpen} />
      ) : (
        <p className="micro-month-prompt">اضغط على يوم لعرض مواعيده المسجلة دون فتح محرر أو تغيير السجل.</p>
      )}
    </section>
  );
}
...
function MonthDayCell({
  ...
  const knownTimes = [
    ...new Set(day.items.flatMap(item => (item.schedule.scheduledTime ? [item.schedule.scheduledTime] : []))),
  ];
  const warnings = [
    day.conflictCount > 0 ? "تعارض" : null,
    day.overCapacity ? "فوق السعة" : null,
    day.unknownTimingCount > 0 ? "وقت غير محدد" : null,
  ].filter(Boolean);
  const summary =
    day.items.length === 0
      ? "لا مواعيد"
  ...
  return (
    <button
      ...
      type="button"
      role="gridcell"
      aria-label={`${dateLabel(day.date)}: ${summary}${warnings.length > 0 ? ` · ${warnings.join("، ")}` : ""}`}
      aria-pressed={selected}
      aria-current={today ? "date" : undefined}
      data-selected={selected}
      data-today={today}
      data-alert={warnings.length > 0}
      onClick={onSelect}
    >
      <span className="micro-month-date">
        <span dir="ltr">{monthDayNumber(day.date)}</span>
        {today ? <small>اليوم</small> : null}
      </span>
      <span className="micro-month-cell-count">
        {day.items.length > 0 ? (
          <>
            <b>
              <IntegerValue value={day.items.length} className="micro-inline-number" />
            </b>{" "}
            موعد
          </>
        ) : (
      ...
      {knownTimes.length > 0 ? (
        <span className="micro-month-cell-time" dir="ltr">
          {knownTimes[0]}
        </span>
      ) : null}
      {warnings.length > 0 ? (
        <span className="micro-month-cell-warning">
          <CircleAlert aria-hidden="true" />
          {warnings[0]}
        </span>
      ) : null}
...
function MonthDayDetail({ day, onOpen }: { day: ScheduleDay; onOpen: (item: ScheduledOrder) => void }) {
  const warnings = [
    day.conflictCount > 0 ? `تعارض في ${day.conflictCount} موعد` : null,
    day.overCapacity ? "يتجاوز السعة اليومية المعلنة" : null,
    day.unknownTimingCount > 0 ? `${day.unknownTimingCount} وقت غير محدد` : null,
  ].filter(Boolean);
  return (
    <section className="micro-month-day-detail" aria-labelledby={`month-day-${day.date}`}>
      <div className="micro-month-detail-heading">
        <div>
          <span className="micro-overline">تفاصيل اليوم</span>
          <h3 id={`month-day-${day.date}`}>{dateLabel(day.date)}</h3>
        </div>
        <span>
          <IntegerValue value={day.scheduledMinutes} className="micro-inline-number" /> دقيقة معروفة
        </span>
      </div>
      {warnings.length > 0 ? (
        <p className="micro-month-warning">
...
function ScheduleSection({
  ...
  title: string;
  description: string;
  tone: "warning" | "accent" | "support";
  items: readonly ScheduledOrder[];
  onOpen: (item: ScheduledOrder) => void;
}) {
  return (
    <section className="micro-schedule-section" data-tone={tone}>
      <div className="micro-schedule-heading">
        <div>
          <span className="micro-overline">{title}</span>
          <h2>{description}</h2>
        </div>
        <span>
          <IntegerValue value={items.length} className="micro-inline-number" />
        </span>
      </div>
...
function WeekDay({ day, onOpen }: { day: ScheduleDay; onOpen: (item: ScheduledOrder) => void }) {
  const warnings = [
    day.conflictCount > 0 ? `تعارض: ${day.conflictCount} موعد` : null,
    day.overCapacity ? "يتجاوز السعة المعلنة" : null,
    day.unknownTimingCount > 0 ? `${day.unknownTimingCount} وقت غير محدد` : null,
  ].filter(Boolean);
  return (
    <article className="micro-week-day" data-alert={warnings.length > 0}>
      <div className="micro-week-day-heading">
        <div>
          <strong>{dateLabel(day.date)}</strong>
          <small>
            <IntegerValue value={day.scheduledMinutes} className="micro-inline-number" /> دقيقة بوقت مسجل ·
            مواعيد تشغيلية نشطة
          </small>
        </div>
        {warnings.length > 0 ? <CircleAlert aria-label="تنبيه تشغيلي" /> : null}
      </div>
