...
export default function Schedule() {
  ...
  return <section className="micro-page micro-schedule-page">
    ...
    {decisionDay ? <CapacityDecisionSurface day={decisionDay} capacityMinutes={overview.dailyCapacityMinutes} onOpen={(item) => item ? navigate(`/schedule/${item.schedule.id}`) : navigate("/orders")} /> : null}
    {overview.overdue.length > 0 ? <ScheduleSection title="متأخر" description="مواعيد تشغيلية تجاوزت موعدها المسجل وتحتاج قرارًا." tone="warning" items={overview.overdue} onOpen={(item) => navigate(`/schedule/${item.schedule.id}`)} /> : null}
    ...
    {overview.completedOrClosed > 0 ? <p className="micro-schedule-closed-note">مواعيد مكتملة مستبعدة: {overview.completedOrClosed} موعدًا لطلبات أغلقت أو اكتملت عند التسليم؛ لا تحتاج متابعة تشغيلية.</p> : null}
    <section className="micro-schedule-status-summary" aria-label="ملخص حالة المواعيد"><div><span>مواعيد تشغيلية نشطة</span><strong><IntegerValue value={total} /></strong><small>متأخر، اليوم، أو قادم ويحتاج قراءة تشغيلية.</small></div><div><span>مواعيد مكتملة مستبعدة</span><strong><IntegerValue value={overview.completedOrClosed} /></strong><small>مكتملة أو مغلقة؛ لا تظهر في المتابعة النشطة.</small></div></section>
    <RecurrencePanel service={recurrences} sources={recurrenceSources} views={recurrenceViews} onChanged={notifyDataChanged} />
    <section className="micro-schedule-capacity"><div><Timer aria-hidden="true" /><div><span className="micro-overline">قدرة اليوم · إعداد اختياري</span><h2>ما المدة التي تستطيع الالتزام بها؟</h2><p>{overview.dailyCapacityMinutes === null ? <><strong>السعة غير محددة؛ لن نرفض الطلب بسببها.</strong> يمكنك قراءة الوقت المسجل فقط، وتفصيله يظهر في تحذير اليوم عند توفر البيانات.</> : `السعة المعلنة: ${overview.dailyCapacityMinutes} دقيقة. التحذير لا يمنعك من حفظ الموعد.`}</p></div></div><label className="micro-field"><span>سعة يومية اختيارية</span><select value={capacityChoice} onChange={(event) => setCapacityChoice(event.target.value)} aria-label="سعة اليوم بالدقائق"><option value="">غير محددة الآن</option>{capacityOptions.map((minutes) => <option key={minutes} value={minutes}>{minutes} دقيقة</option>)}</select></label>{capacityMessage ? <p className={capacityMessage.startsWith("تم ") || capacityMessage.startsWith("لم ") ? "micro-save-note" : "micro-field-error"} role={capacityMessage.startsWith("تم ") || capacityMessage.startsWith("لم ") ? "status" : "alert"}>{capacityMessage}</p> : null}<button className="micro-button micro-button-secondary" type="button" disabled={savingCapacity} onClick={saveCapacity}>{savingCapacity ? "جارٍ حفظ السعة…" : "حفظ سعة اليوم"}</button></section>
  </section>;
}

function CapacityDecisionSurface({ day, capacityMinutes, onOpen }: { day: ScheduleDay; capacityMinutes: number | null; onOpen: (item: ScheduledOrder | null) => void }) {
  ...
  return <section className="micro-capacity-decision" aria-label={`قرار السعة ليوم ${dateLabel(day.date)}`}>
    ...
    <button className="micro-button micro-button-primary micro-button-block" type="button" onClick={() => onOpen(nextItem)}>{nextItem ? "فتح أقرب متابعة" : "فتح الطلبات"}</button>
