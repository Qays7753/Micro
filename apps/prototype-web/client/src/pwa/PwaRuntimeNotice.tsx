...
export function PwaRuntimeNotice() {
  ...
  return <div className="micro-runtime-notices" aria-live="polite">
    {showUpdate ? <aside className="micro-runtime-card" data-tone="update">
      <span className="micro-runtime-icon" aria-hidden="true"><RefreshCw /></span>
      ...
      <div className="micro-runtime-actions">
        <button className="micro-button micro-button-primary" type="button" disabled={isApplying} onClick={updateNow}>
          <RefreshCw aria-hidden="true" />{isApplying ? "جارٍ التحديث…" : "حدّث الآن"}
        </button>
        <button className="micro-icon-button" type="button" onClick={() => setIsUpdateDismissed(true)} aria-label="لاحقًا" title="لاحقًا">
          <X aria-hidden="true" />
        </button>
      </div>
    </aside> : null}
    {showOffline ? <aside className="micro-runtime-card" data-tone="offline" role="status">
      <span className="micro-runtime-icon" aria-hidden="true"><WifiOff /></span>
      <div className="micro-runtime-copy">
        <h2>أنت غير متصل الآن</h2>
        <p>يستمر Micro بما تم حفظه محليًا على هذا الجهاز. لا توجد مزامنة أو نسخة سحابية في هذا الإصدار.</p>
      </div>
