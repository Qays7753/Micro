...
type PrototypeServices = { profiles: ProfileService; drafts: DraftService; costs: CostService; agreements: AgreementService; dataVersion: number; notifyDataChanged: () => void };
...
export function PrototypeServicesProvider({ children }: { children: ReactNode }) {
  ...
  const services = useMemo(() => {
    ...
    return { profiles: new ProfileService(store), drafts: new DraftService(store), costs, agreements: new AgreementService(store, costs), dataVersion, notifyDataChanged: () => setDataVersion(version => version + 1) };
  }, [dataVersion]);
