...
function seedVersionOneDraft() {
  return new Promise<void>((resolve, reject) => {
    ...
    request.onsuccess = () => {
      ...
      transaction.objectStore("order-drafts").put({
        id: "legacy",
        intent: "customer_order",
        customerName: "",
        itemName: "مسودة قديمة",
        specifications: "",
        quantity: 1,
...
function seedVersionTwentyFourLegacyMovement() {
  return new Promise<void>((resolve, reject) => {
    ...
    request.onerror = () => reject(request.error);
    request.onupgradeneeded = () => {
      const movements = request.result.createObjectStore("inventory-movements", { keyPath: "id" });
      movements.put({
        id: "legacy-waste",
        materialId: "material",
        type: "waste",
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      policies.put({
        ...
        note: "معدل لكل وحدة",
        status: "active",
        idempotencyKey: "legacy-per-unit",
        createdAt: "2026-08-22T00:00:00.000Z",
        updatedAt: "2026-08-22T00:00:00.000Z",
      });
    };
...
function seedVersionSevenSchedule() {
  return new Promise<void>((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      request.result.createObjectStore("activity-profile", { keyPath: "id" });
      request.result.createObjectStore("local-preferences", { keyPath: "id" });
      request.result.createObjectStore("order-drafts", { keyPath: "id" });
      request.result.createObjectStore("craft-orders", { keyPath: "id" });
      request.result.createObjectStore("financial-events", { keyPath: "id" });
      request.result.createObjectStore("schedule-entries", { keyPath: "id" });
    };
    request.onsuccess = () => {
      const database = request.result;
      const transaction = database.transaction(["local-preferences", "schedule-entries"], "readwrite");
      transaction
        .objectStore("local-preferences")
        .put({ id: localPreferencesId, theme: "light", updatedAt: "2026-08-22T00:00:00.000Z" });
      transaction.objectStore("schedule-entries").put({
        id: "legacy-schedule",
...
describe("IndexedDbLocalStore", () => {
  it("persists the profile and draft through a fresh adapter instance", async () => {
    ...
    ).resolves.toMatchObject({ ok: true, value: { theme: "dark" } });
  ...
  it("returns local drafts with the latest update first", async () => {
    ...
    const early: OrderDraft = {
      id: "early",
      intent: "planned_design",
      customerName: "",
      itemName: "قطعة أولى",
      specifications: "",
      quantity: 1,
    ...
    const late: OrderDraft = {
      ...early,
      id: "late",
      itemName: "قطعة أحدث",
      updatedAt: "2026-08-22T01:00:00.000Z",
    };
    await store.saveDraft(early);
    await store.saveDraft(late);
  ...
  it("migrates a Slice 1 draft to the current schema without dropping its pre-domain details", async () => {
    ...
    const store = new IndexedDbLocalStore();
    await expect(store.getDraft("legacy")).resolves.toMatchObject({
      ok: true,
      value: {
        id: "legacy",
        itemName: "مسودة قديمة",
  ...
  it("migrates a Slice B schedule to explicit unknown time, duration, and daily capacity fields", async () => {
    await seedVersionSevenSchedule();
    const store = new IndexedDbLocalStore();
    await expect(store.getPreferences()).resolves.toMatchObject({
      ok: true,
      value: { theme: "light", dailyScheduleCapacityMinutes: null },
    });
    await expect(store.getSchedule("legacy-schedule")).resolves.toMatchObject({
      ok: true,
      value: {
        scheduledTime: null,
  ...
  it("migrates schema 8 by adding an empty material-purchase store", async () => {
    ...
    await expect(new IndexedDbLocalStore().listSupplierPurchases()).resolves.toMatchObject({
      ok: true,
      value: [],
    });
  });
  ...
  it("migrates a schema 11 database missing cash stores without touching its prior stores", async () => {
    await new Promise<void>((resolve, reject) => {
      ...
      request.onupgradeneeded = () => {
        [
          ...
          "schedule-entries",
          "supplier-purchases",
        ].forEach(name => request.result.createObjectStore(name, { keyPath: "id" }));
      };
      request.onsuccess = () => {
        request.result.close();
        resolve();
    ...
    const store = new IndexedDbLocalStore();
    await expect(store.listCashWallets()).resolves.toMatchObject({ ok: true, value: [] });
    await expect(store.listCashContinuityEntries()).resolves.toMatchObject({ ok: true, value: [] });
  });
  ...
  it("migrates a schema 13 database missing material stores without touching G3 stores", async () => {
    ...
    const store = new IndexedDbLocalStore();
    await expect(store.listMaterials()).resolves.toMatchObject({ ok: true, value: [] });
  ...
  it("repairs schema 16 drafts and orders when its catalog store is missing, without name matching", async () => {
    await new Promise<void>((resolve, reject) => {
      const request = indexedDB.open(databaseName, 16);
      request.onerror = () => reject(request.error);
      request.onupgradeneeded = () => {
        [
          "activity-profile",
      ...
      request.onsuccess = () => {
        const database = request.result;
        const transaction = database.transaction(["order-drafts", "craft-orders"], "readwrite");
        transaction.objectStore("order-drafts").put({ id: "legacy-draft", itemName: "صندوق هدايا" });
        transaction
          .objectStore("craft-orders")
          .put({ id: "legacy-order", order: { itemName: "صندوق هدايا" } });
        transaction.oncomplete = () => {
          database.close();
          resolve();
        };
    ...
    const store = new IndexedDbLocalStore();
    await expect(store.getDraft("legacy-draft")).resolves.toMatchObject({
      ok: true,
      value: { itemName: "صندوق هدايا", catalogItemId: null },
    });
    await expect(store.getOrder("legacy-order")).resolves.toMatchObject({
      ok: true,
      value: {
        catalogItemId: null,
  ...
  it("migrates a schema-24 waste movement to an explicit general-project context", async () => {
    ...
    await expect(store.listInventoryMovements()).resolves.toMatchObject({
      ok: true,
      value: [{ id: "legacy-waste", wasteContext: { kind: "general_project" } }],
    });
  });
  ...
  it("migrates schema-25 per-unit policies to the explicit whole-unit rate field", async () => {
    ...
    const store = new IndexedDbLocalStore();
    await expect(store.listAllocationPolicies()).resolves.toMatchObject({
      ok: true,
      value: [{ id: "legacy-per-unit", kind: "per_output_unit", rateMinorPerWholeUnit: 50, rateMinor: null }],
    });
  });
  ...
  it("persists allocation policies through a fresh adapter and full snapshot replacement", async () => {
    ...
    const item = createCatalogItem({
      ...
      unitId: null,
      createdAt: "2026-08-22T00:00:00.000Z",
      createdOperationKey: "indexed-catalog",
    });
    const policy = createAllocationPolicy({
      id: "indexed-policy",
      seriesId: "indexed-series",
      successorOfPolicyId: null,
      version: 1,
      ...
      source: "فاتورة",
      reason: "توزيع",
      note: "اختبار IndexedDB",
      status: "active",
      idempotencyKey: "indexed-policy",
      createdAt: "2026-08-22T00:00:00.000Z",
      updatedAt: "2026-08-22T00:00:00.000Z",
    });
    await store.saveCatalogItem(item);
    await store.saveAllocationPolicy(policy);
    await expect(new IndexedDbLocalStore().listAllocationPolicies()).resolves.toMatchObject({
      ok: true,
      value: [{ id: policy.id, catalogItemId: item.id }],
    });
    const snapshot = await store.readSnapshot();
    if (!snapshot.ok) throw new Error("snapshot should read");
    expect(snapshot.value.allocationPolicies).toEqual([expect.objectContaining({ id: policy.id })]);
    await expect(
      store.replaceSnapshot({ ...snapshot.value, allocationPolicies: [policy] }),
    ).resolves.toMatchObject({ ok: true, value: { allocationPolicies: [{ id: policy.id }] } });
    await expect(new IndexedDbLocalStore().listAllocationPolicies()).resolves.toMatchObject({
      ok: true,
      value: [{ id: policy.id }],
    });
  ...
  it("commits one local order and its linked draft together", async () => {
    ...
    await expect(store.commitOrderFromDraft(stored, draft)).resolves.toMatchObject({
      ok: true,
      value: { order: { id: "order-1" }, draft: { linkedOrderId: "order-1" } },
    });
    const resumed = new IndexedDbLocalStore();
    await expect(resumed.listOrders()).resolves.toMatchObject({ ok: true, value: [{ id: "order-1" }] });
    await expect(resumed.getDraft("draft-linked")).resolves.toMatchObject({
      ok: true,
  ...
  it("commits one financial reversal atomically and preserves it across adapter instances", async () => {
    ...
    const reversal = createFinancialReversal({
      ...
      idempotencyKey: "indexed-reversal",
      reason: "تصحيح ذري",
    });
    await expect(store.saveFinancialEvent(original)).resolves.toMatchObject({
      ok: true,
      value: { id: original.id },
    });
    const [first, second] = await Promise.all([
      store.commitFinancialEventCorrection(original.id, reversal),
      new IndexedDbLocalStore().commitFinancialEventCorrection(original.id, reversal),
    ]);
    expect(first).toMatchObject({ ok: true, value: { id: reversal.id } });
    expect(second).toMatchObject({ ok: true, value: { id: reversal.id } });
    const resumed = new IndexedDbLocalStore();
    await expect(resumed.listFinancialEvents()).resolves.toMatchObject({
      ok: true,
      value: [
        { id: reversal.id, correctionOfEventId: original.id, operatingExpenseDeltaMinor: -900 },
        { id: original.id, operatingExpenseDeltaMinor: 900 },
      ],
    });
    const secondAttempt = createFinancialReversal({
      id: "indexed-reversal-2",
      sourceEvent: original,
      occurredOn: "2026-08-21",
      recordedAt: "2026-08-21T01:00:00.000Z",
      idempotencyKey: "indexed-reversal-2",
      reason: "محاولة ثانية",
    });
    await expect(resumed.commitFinancialEventCorrection(original.id, secondAttempt)).resolves.toMatchObject({
      ok: false,
      code: "storage_error",
    });
    await expect(
      resumed.commitFinancialEventCorrection("missing-source", secondAttempt),
    ).resolves.toMatchObject({ ok: false, code: "storage_error" });
    await expect(new IndexedDbLocalStore().listFinancialEvents()).resolves.toMatchObject({
      ok: true,
      value: [{ id: reversal.id }, { id: original.id }],
    });
  });

  it("replaces the full local snapshot in one IndexedDB transaction", async () => {
    const store = new IndexedDbLocalStore();
    const oldDraft: OrderDraft = {
      id: "old",
      intent: "customer_order",
      customerName: "",
      itemName: "قديم",
    ...
    await store.saveDraft(oldDraft);
    const replacement: ActivityProfile = {
      id: localProfileId,
      activityName: "مستورد",
      currency: "JOD",
      activityType: "custom_craft",
    ...
    const replacementDraft: OrderDraft = {
      ...oldDraft,
      id: "imported",
      itemName: "مستورد",
      createdAt: "2026-08-22T01:00:00.000Z",
    ...
    const resumed = new IndexedDbLocalStore();
    await expect(resumed.getProfile()).resolves.toMatchObject({ ok: true, value: replacement });
    await expect(resumed.getPreferences()).resolves.toMatchObject({ ok: true, value: importedPreferences });
    await expect(resumed.listDrafts()).resolves.toMatchObject({ ok: true, value: [{ id: "imported" }] });
    await expect(resumed.getDraft("old")).resolves.toMatchObject({ ok: true, value: null });
  });
  ...
  it("keeps cash wallets and their continuity entries through a snapshot replacement", async () => {
    ...
    await expect(store.commitCashContinuity(wallet, [opening])).resolves.toMatchObject({
      ok: true,
      value: { wallet: { id: "drawer" }, entries: [{ id: "opening" }] },
    });
    const snapshot = await store.readSnapshot();
    if (!snapshot.ok) throw new Error("snapshot should read");
    expect(snapshot.value).toMatchObject({
      cashWallets: [{ id: "drawer" }],
      cashContinuityEntries: [{ id: "opening" }],
    });
    await expect(
      store.replaceSnapshot({
        profile: null,
        ...
        supplierPurchases: [],
        cashWallets: [wallet],
        cashContinuityEntries: [opening],
      }),
    ).resolves.toMatchObject({
      ok: true,
      value: { cashWallets: [{ id: "drawer" }], cashContinuityEntries: [{ id: "opening" }] },
    });
    await expect(new IndexedDbLocalStore().listCashContinuityEntries()).resolves.toMatchObject({
      ok: true,
  ...
  it("keeps materials and inventory movements through a snapshot replacement", async () => {
    ...
    await expect(store.commitInventory(material, [opening])).resolves.toMatchObject({
      ok: true,
      value: { material: { id: "wood" }, movements: [{ id: "material-opening" }] },
    });
    const snapshot = await store.readSnapshot();
    if (!snapshot.ok) throw new Error("snapshot should read");
    expect(snapshot.value).toMatchObject({
      materials: [{ id: "wood" }],
      inventoryMovements: [{ id: "material-opening" }],
    });
    await expect(
      store.replaceSnapshot({
        profile: null,
        ...
        materials: [material],
        inventoryMovements: [opening],
      }),
    ).resolves.toMatchObject({
      ok: true,
      value: { materials: [{ id: "wood" }], inventoryMovements: [{ id: "material-opening" }] },
    });
    await expect(new IndexedDbLocalStore().listInventoryMovements()).resolves.toMatchObject({
      ok: true,
      value: [{ id: "material-opening", valueDeltaMinor: 4000 }],
    });
  });
});
