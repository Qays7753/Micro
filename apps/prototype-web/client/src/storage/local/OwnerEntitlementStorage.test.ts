...
describe("O1 IndexedDB adapter", () => {
  it("creates O1 stores and commits the owner movement with its cash entry atomically", async () => {
    ...
    await expect(store.commitOwnerMovement(movement, cashEntry)).resolves.toMatchObject({
      ok: true,
      value: { movement: { id: movement.id }, cashEntry: { id: cashEntry.id } },
    });
    const resumed = new IndexedDbLocalStore();
    await expect(resumed.listOwnerEntitlementPolicies()).resolves.toMatchObject({
      ok: true,
      value: [{ id: policy.id }],
    });
    await expect(resumed.listOwnerEntitlementRecords()).resolves.toMatchObject({
      ok: true,
      value: [{ id: record.id }],
    });
    await expect(resumed.listOwnerMovements()).resolves.toMatchObject({
      ok: true,
      value: [{ id: movement.id }],
    });
    await expect(resumed.listCashContinuityEntries()).resolves.toMatchObject({
      ok: true,
      value: [{ id: cashEntry.id, cashDeltaMinor: -500 }],
    });
  });
  ...
  it("atomically stores a policy successor and append-only record reversals", async () => {
    ...
    await expect(store.commitOwnerEntitlementPolicySuccessor(ended, successor)).resolves.toMatchObject({
      ok: true,
      value: { successor: { id: "policy-2" } },
    });
    ...
    await expect(
      store.commitOwnerEntitlementRecordReversal(record.id, recordReversal),
    ).resolves.toMatchObject({ ok: true, value: { reversalOfId: record.id } });
    ...
    await expect(
      store.commitOwnerEntitlementOpeningBalanceReversal(opening.id, openingReversal),
    ).resolves.toMatchObject({ ok: true, value: { reversalOfId: opening.id } });
    const snapshot = await store.readSnapshot();
    if (!snapshot.ok) throw new Error("snapshot should read");
    expect(snapshot.value.ownerEntitlementPolicies).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ id: policy.id, status: "ended", amountMinor: 1500 }),
    ...
    expect(snapshot.value.ownerEntitlementRecords).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ id: record.id }),
        expect.objectContaining({ reversalOfId: record.id }),
      ]),
    );
    expect(snapshot.value.ownerEntitlementOpeningBalances).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ id: opening.id }),
        expect.objectContaining({ reversalOfId: opening.id }),
      ]),
    );
  });
  ...
  it("keeps O1 collections through an atomic snapshot replacement and tolerates a legacy snapshot without them", async () => {
    ...
    expect(snapshot.value).toMatchObject({
      ownerEntitlementPolicies: [{ id: policy.id }],
      ownerEntitlementRecords: [{ id: record.id }],
    ...
    await expect(
      store.replaceSnapshot({
        ...
        schedules: [],
        financialEvents: [],
        ownerEntitlementPolicies: [policy],
        ownerEntitlementRecords: [record],
        ownerEntitlementOpeningBalances: [],
        ownerMovements: [],
      }),
    ).resolves.toMatchObject({
      ok: true,
      value: { ownerEntitlementPolicies: [{ id: policy.id }], ownerEntitlementRecords: [{ id: record.id }] },
    });
    const replacement = new IndexedDbLocalStore();
    await expect(replacement.listOwnerEntitlementRecords()).resolves.toMatchObject({
      ok: true,
      value: [{ id: record.id }],
    });
  });
