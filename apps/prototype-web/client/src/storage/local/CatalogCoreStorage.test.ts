...
describe("IndexedDB G4-A stores", () => {
  it("round-trips units, conversions, templates, and inactive history across adapters", async () => {
    ...
    const count = await service.createUnit({
      nameAr: "قطعة",
      dimension: "count",
      operationKey: "unit:piece:indexed",
    });
    const gram = await service.createUnit({
      nameAr: "غرام",
      dimension: "mass",
      operationKey: "unit:gram:indexed",
    });
    ...
    const item = await service.create({
      ...
      operationKey: "catalog:item:indexed",
    });
    ...
    const template = await service.createTemplate({
      ...
      title: "مرجع",
      note: null,
      components: [{ id: "component", name: "مادة", quantityMilli: 1000, unitId: gram.unit.id, note: null }],
      yield: null,
      operationKey: "template:item:indexed",
    });
    ...
    expect(listedUnits.value).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ id: count.unit.id, active: true }),
        expect.objectContaining({ id: gram.unit.id, active: false }),
      ]),
    );
    await expect(resumed.listCatalogTemplates(item.item.id)).resolves.toMatchObject({
      ok: true,
      value: [{ id: template.template.id, active: true }],
    });
    const snapshot = await resumed.readSnapshot();
    if (!snapshot.ok) throw new Error("snapshot should read");
    expect(snapshot.value.measurementUnits).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ id: count.unit.id }),
        expect.objectContaining({ id: gram.unit.id }),
      ]),
    );
  ...
  it("adds unitId null to a schema-23 legacy catalog item without matching or rewriting its name", async () => {
    await new Promise<void>((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        transaction.objectStore("catalog-items").put({
          id: "legacy-item",
          kind: "product",
          name: "اسم قديم",
          unitLabel: "قطعة",
          ...
          createdAt: at,
          updatedAt: at,
          createdOperationKey: "legacy-item",
        });
    ...
    const store = new IndexedDbLocalStore();
    await expect(store.getCatalogItem("legacy-item")).resolves.toMatchObject({
      ok: true,
      value: { id: "legacy-item", name: "اسم قديم", unitId: null },
    });
  });
});
