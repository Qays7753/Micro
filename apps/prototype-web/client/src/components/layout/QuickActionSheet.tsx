...
export type QuickActionItem = { action: QuickAction; label: string; description: string; icon: typeof ClipboardPlus; disabled?: boolean };
...
export const actionItems: readonly QuickActionItem[] = [
  ...
  { action: "estimate", label: "تقدير تصميم — قريبًا", description: "لا يوجد مسار تقدير مستقل في هذا الإصدار.", icon: PackagePlus, disabled: true },
...
export function QuickActionSheet({ open, onOpenChange, onAction }: QuickActionSheetProps) {
  return <Drawer open={open} onOpenChange={onOpenChange} direction="bottom"><DrawerContent className="micro-bottom-sheet" dir="rtl">
    ...
    <div className="micro-sheet-actions">{actionItems.map(item => { const Icon = item.icon; return <button key={item.action} className="micro-sheet-action" type="button" disabled={item.disabled} aria-disabled={item.disabled || undefined} onClick={() => { if (!item.disabled) onAction(item.action); }}><span className="micro-sheet-action-icon"><Icon aria-hidden="true" /></span><span><strong>{item.label}</strong><small>{item.description}</small></span></button>; })}</div>
