...
هذه الدفعة هي **مرجع قرار وDomain Core قابل للاختبار وApp Shell أولي للـPrototype**. تحتوي على عقود المجال، ونواة أولية لمسار طلب حرفة يدوية مخصصة، وحزمة Web App في `apps/prototype-web/` لواجهة RTL محلية وسياق التنقل وLight/Dark والحالات الفارغة. لا تحتوي بعد على مسودات أو LocalStore أو تكلفة أو سعر أو اتفاق أو عربون أو تسليم أو تحصيل أو export/import. كما لا تحتوي SaaS قابلًا للإطلاق أو استقبال بيانات حقيقية، ولا نسخة من Accounting أو zman-app أو كود خارجي منقول.
...
| `apps/prototype-web/` | Web-first App Shell للـPrototype، مستقل عن Domain وLocalStore إلى حين بناء طبقاتها |
...
المرحلة الحالية هي Domain Core والعقود والاختبارات، مع **Slice 0 من Prototype الهاتف المحلي**: Web App بملء الشاشة، RTL، App Shell، تنقل، Bottom Sheet، Light/Dark، وحالات بداية صادقة بلا أرقام مالية وهمية. الشريحة الأولى المكتملة مستقبلاً هي: قطعة حرفة مخصصة → خامات ووقت تقديري → تكلفة بدرجة معرفة → سعر حماية → اتفاق وموعد → عربون اختياري → تنفيذ → تسليم → قبض أو دين → نتيجة وفعل تالٍ. النواة الحالية تطبق نسخ التكلفة، حالات المعرفة، `resultStatus`، فصل العربون عن التحصيل، وإلغاء العربون بتسوية صريحة أو `needs_review`. لم تُربط الواجهة بعد بـDomain أو LocalStore، ولم يُنفذ PWA أو Cloudflare deployment أو Native wrapper أو SaaS أو استقبال بيانات مستخدمين حقيقية؛ لذلك لا يجوز الادعاء بالجاهزية التجارية.
