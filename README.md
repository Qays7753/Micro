...
المرحلة الحالية هي Foundation/Domain Core والعقود والاختبارات قبل واجهة الهاتف. الشريحة الأولى هي: قطعة حرفة مخصصة → خامات ووقت تقديري → تكلفة بدرجة معرفة → سعر حماية → اتفاق وموعد → عربون اختياري → تنفيذ → تسليم → قبض أو دين → نتيجة وفعل تالٍ. النواة الحالية تطبق نسخ التكلفة، حالات المعرفة، `resultStatus`، فصل العربون عن التحصيل، وإلغاء العربون بتسوية صريحة أو `needs_review`. لا يزال إطلاق SaaS أو استقبال بيانات مستخدمين حقيقية أو ادعاء الجاهزية التجارية ممنوعًا؛ الإغلاق الحالي يجيز الانتقال إلى Prototype الهاتف فقط.
