...
- إذا دخل طلب مسلّم إلى `needs_review`، تُحجب النتيجة وتُمنع **كل العمليات العامة التي تغيّر الحالة أو Snapshot أو الأثر المالي** إلا عبر Use Case تصحيح موثق — وهو **منفذ فعلًا**: `reverseDelivery` (الحد الكنوني في النطاق؛ لا توجد دالة `correctDelivery` منفصلة بالمقصود) تقبل الحالة المقفلة نفسها بعلاقة موثقة (`reversesEventId`) وسبب إلزامي ومفتاح حتمية وحياد صادق للإيراد والتكلفة — نفّذته المجموعة ٣ من برنامج التحصين الكامل (`docs/operations/current-state.md` §32/§33؛ الاختبار: `reviewLockBoundary.test.ts` وحارس علاقة العكس المشترك في المحوّلين).
...
## 10. قواعد التنفيذ غير القابلة للتفاوض (المجموعة ٦ — برنامج التحصين الكامل 2026)

هذه القواعد تكمل §2–§9 لا تستبدلها، وتُلزم كل شريحة كود قادمة:

**قبل أول سطر كود:**

1. قراءة إلزامية: `docs/operations/current-state.md` ثم `AGENTS.md` ثم `docs/00-document-index.md` ثم العقد الحاكم للشريحة (بحسب جدول §2) — وإثبات ما قُرئ في بطاقة الفهم.
2. بطاقة فهم مكتوبة قبل الكود تتضمن: المستخدم وسؤال القرار، النطاق الصريح، **خارج النطاق** (non-goals)، الـinvariants المالية والحسابية، حالات الفشل المتوقعة، معيار القبول، و**حد الاستعادة** (rollback boundary: آخر التزام آمن للعودة إليه وأثر الرجوع).
3. تمييز صريح بين المنفذ فعلًا (كود + اختبار على الفرع/`main`) والهدف والمقترح وSpike والمؤجل والمرفوض — الوثيقة وحدها ليست دليل تنفيذ؛ افتح الكود.

**أثناء الكود:**

4. شريحة واحدة قابلة للمراجعة في الوقت نفسه؛ لا قدرة خارج Prototype المرحلة 2 المعتمدة (لا Auth ولا Cloud/Sync ولا تعدد مستخدمين ولا SaaS/Billing ولا قطاعات إضافية) قبل بوابة المالك.
5. لا قواعد مجال (مال/تسعير/ربح/كاش/مخزون/دين) داخل React/UI؛ المجال في `src/domain/` وحده — يفرضه ESLint (نقاء استيراد المجال) والاختبارات.
6. لا وصول مباشر من الصفحات/المكونات إلى `@/storage/local/*` ولا إلى `localStorage`/`sessionStorage` في زمن التشغيل؛ المرور عبر خدمات Application حصرًا — يفرضه ESLint، والاستثناء الوحيد الموثق هو أدوات ملفات اختبار الصفحات/المكونات نفسها.
7. لا كتابة عمياء (blind read-modify-write) لبيانات حساسة: كل كتابة حساسة تتحقق الحالة الحية داخل حد الكتابة وترفض التعارض بـ`storage_stale` بلا كتابة، وكل إعادة محاولة حساسة حتمية بمفتاح idempotency — ومع كل مسار كتابة حساس جديد: اختبارات تزامن وإعادة محاولة.
8. كل كيان دائم جديد (object store أو سجل محلي دائم) يسجل في `docs/quality/persistent-entity-touchpoints.json` داخل نفس الشريحة مع كل نقاط اتصاله (المهايئان، اختبارات التخزين، التصدير/الاستيراد أو سبب الاستثناء الصريح، التوافق، التوثيق) — الحارس `scripts/check-entity-touchpoints.mjs` يفشل عند غيابه.
9. لا أسرار ولا قيم مالية ولا استثناءات خام ولا معرفات شخصية في السجلات أو التشخيصات أو رسائل الأخطاء؛ تشخيص Micro المحلي محدود بحقول الثمانية في `localDiagnosticsService` ولا يُرفع أبدًا — `scripts/check-secrets.mjs` يمسح المستودع، واختبارات `.only`/`.skip` يفحصها `scripts/check-test-focus.mjs`.

**عند الإغلاق (فوق §9):**

10. تحديث `current-state.md` و`todo.md` وسجل القرار/QA عند إغلاق الشريحة، ووصف PR يحمل: قائمة الملفات المتغيرة، نتائج `pnpm check` والأوامر المركزة، أثر المخطط/التصدير/الترحيل (أو «لا ينطبق — السبب»)، وقائمة ما لم يتغير عمدًا — وفق `.github/pull_request_template.md`.
11. عند أي تعارض عقدي أو غموض مالي أو فشل فحص حاجز: توقف واطلب قرار المالك؛ **الافتراض الصامت لاستكمال التنفيذ ممنوع**.

**حدود رقمية محروسة لا تُرفع بصمت:** `localSchemaVersion`/`localExportVersion` (36/28 اليوم — رُفعا من 35/27 بقرار المالك D-037 مع عقد ٤١ لـOPS-003: مخازن المصروف المتكرر الثلاثة وعائلات تصديره) مثبتة في `storage/local/types.ts` ومربوطة باختبارات توثيق؛ سقف تحذيرات lint (37 عبر `--max-warnings`) وسقفا ميزانية الحزمة (650,000/155,000 بايت في `apps/prototype-web/scripts/check-bundle-budget.mjs`) مربوطة باختبار حوكم يطابق الأرقام بين الكود والوثيقة — رفع أي منها قرار مالك يظهر في diff مزدوج (الكود + التوثيق + الاختبار) لا تعديل صامت.

## 11. بوابة ما بعد المجموعة ٦ — المسح الهيكلي (منفذ ومقبول) وخطة المعالجة الرباعية الموافَقة

**حالة البوابة:** المسح الهيكلي/المعماري/تنظيم الكود **للقراءة فقط نُفّذ وأُغلق** (المجموعة ٧ من برنامج التحصين، 2026-09-11): مسح شامل عند `9a8c949` — حدود الوحدات والميزات، الملفات المتضخمة، خرق الطبقات والاعتماديات والدورات، التكرار ومصادر الحقيقة، قابلية الاكتشاف، تركيب الطبقات، خرائط الاختبار/التوثيق، وخريطة وحدات هدف — دون تغيير أي ملف في Micro. التقرير الواحد (سجل مكتشفات: أصلح الآن/أبقِ/أجّل/خارج النطاق، مع موجات آمنة وحدود استعادة) مُسلَّم في مستودع التوثيق: `audits/micro/structure-architecture-code-organization-scan-v1.md` عند الالتزام `17b264c`. **المالك راجع المكتشفات وقبلها ووافق على خطة المعالجة الرباعية (المجموعات ٨–١١) المتسلسلة على الفرع الوحيد الطويل نفسه.**

**القواعد الملزمة للمرحلة الحالية (لا تُضعف بها حدود ما قبلها):**

1. **لا معالجة هيكلية خارج الخطة الموافَقة:** لا إعادة هيكلة بنيوية أو نقل ملفات جماعي أو طبقة `features/` أو تقسيم `IndexedDbLocalStore`/`MemoryLocalStore`/`localTransferService`/`Finance`/`OrderDetail`/`Catalog`/`OwnerEntitlement` إلا بموجة معتمدة من الخطة وقرار مالك صريح لكل مجموعة.
...
3. **الحدود الرقمية وقرارات الرفض تبقى كما هي في §10** (35/27، سقف lint، سقفا الحزمة، ورفض `features/` الآن — قرار المالك D8 على تقرير المسح).
...
هذا القسم يوسّع نظام `docs/operations` القائم ولا ينشئ سلطة بديلة.

قبل أي تعديل، اقرأ `docs/operations/control/generated/AGENT-BRIEF.md` وشغّل `python3 scripts/operations-control/validate.py`. افحص PRs المفتوحة و`generated/ACTIVE-WORK.md`، ثم ابحث عن ID والسبب الجذري في السجل والكود والاختبارات وGit history.

- أنشئ Workstream claim في `docs/operations/control/workstreams/` قبل لمس الكود، متضمنًا branch وbase SHA وitems وareas وcontracts.
- لا تبدأ عند تداخل area أو contract مع Workstream نشط. حل التعارض أو انتظر الدمج.
- لا تعيد تنفيذ بند يقول السجل إنه منجز دون التحقق من `main` والاختبار. الاختلاف يحوّل البند إلى `REOPENED` بدليل.
- بعد PR استخدم `IN_REVIEW`؛ بعد الدمج `MERGED_UNVERIFIED`؛ وبعد الاختبار على `main` فقط `VERIFIED`.
- حدّث JSON المصدر ثم شغّل المولّد. لا تعدّل ملفات `generated/` أو Excel يدويًا.
- العقود والقرارات و`current-state.md` تحتفظ بسلطتها حسب `docs/operations/control/authority-map.md`.
- `docs/expansion/TRACKER.md` يبقى حاكمًا لنطاق Expansion؛ اربطه ولا تنسخه.
- اكتشاف جديد لا يصبح بندًا منفصلًا قبل تصنيف NEW/DEEPENS/DUPLICATE/REOPENED/FALSE_POSITIVE، وقرار المنتج يتوقف عند المالك.
