...
**حدود رقمية محروسة لا تُرفع بصمت:** `localSchemaVersion`/`localExportVersion` (35/27 اليوم) مثبتة في `storage/local/types.ts` ومربوطة باختبارات توثيق؛ سقف تحذيرات lint (37 عبر `--max-warnings`) وسقفا ميزانية الحزمة (650,000/155,000 بايت في `scripts/check-bundle-budget.mjs`) مربوطة باختبار حوكم يطابق الأرقام بين الكود والوثيقة — رفع أي منها قرار مالك يظهر في diff مزدوج (الكود + التوثيق + الاختبار) لا تعديل صامت.

## 11. بوابة ما بعد المجموعة ٦ — المسح الهيكلي للقراءة فقط (إلزامية وموثقة، لم تبدأ)

بعد إغلاق المجموعات ١–٦ من برنامج التحصين الكامل: **لا يبدأ أي إعادة هيكلة بنيوية أو نقل ملفات جماعي أو تغيير تنظيم كود أو تنظيف معماري فورًا.** المرحلة التالية إلزاميًا هي **مسح هيكلي/معماري/تنظيم كود شامل للقراءة فقط** يغطي: حدود الوحدات والميزات، مسؤوليات الملفات والملفات المتضخمة (مثل `IndexedDbLocalStore` و`localTransferService` و`ProjectFinancialService` وصفحات `Finance`/`OrderDetail`/`Catalog`)، خرق الطبقات والاعتماديات والدورات، التكرار ومصادر الحقيقة، قابلية اكتشاف الميزات، تركيب التخزين/التطبيق/المجال/الواجهة، خريطة الاختبارات والتوثيق، وخريطة وحدات هدف. ينتج المسح **تقرير مكتشفات واحدًا** يصنف كل بندًا: أصلح الآن / أبقِ / أجّل / خارج النطاق، مع موجات معالجة آمنة دنيا واعتمادياتها ومخاطرها ومعايير قبولها وحدود استعادتها. **المالك يراجع المكتشفات ويقررها قبل بدء أي معالجة هيكلية معتمدة.**

هذه بوابة موثقة للمرحلة القادمة فقط: لم يُنفذ المسح، ولا يجوز البدء به أو بأي نقل/تقسيم/إعادة تسمية هيكلية تلقائيًا بعد هذه المجموعة دون قرار مالك صريح بقبول مكتشفات المسح أصلًا.
