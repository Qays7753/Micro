...
const order = (overrides: Partial<Parameters<typeof calculateContributionMargin>[2][number]> = {}) => ({ id: "order-1", itemName: "صندوق", deliveredOn: "2026-08-10", resultStatus: "final" as const, quantityMilli: 2000, unitKey: "piece", unitLabel: "قطعة", quantityIssue: null, recognizedRevenueMinor: 5000, recognizedCostMinor: 1800, ...overrides });
...
const shortCash = (overrides: Partial<Parameters<typeof calculateShortCash>[0]> = {}) => ({ from: "2026-08-01", to: "2026-08-31", recordedCashMinor: 10000, receivables: [], payables: [], declarations: [], ...overrides });
...
describe("G5 pure domain", () => {
  it("calculates recorded contribution margin and break-even from final orders only", () => {
    ...
    expect(result).toMatchObject({ status: "available", totalRevenueMinor: 5000, totalVariableCostMinor: 1800, contributionMarginMinor: 3200, contributionMarginPerUnitMinor: 1600, totalQuantityMilli: 2000, quantityUnitKey: "piece", fixedExpenseMinor: 1000, breakEvenUnits: 1, finalOrderCount: 1, excludedOrderCount: 0 });
    expect(result.mix).toEqual([{ itemName: "صندوق", orderCount: 1, quantityMilli: 2000, unitKey: "piece", unitLabel: "قطعة", revenueMinor: 5000, variableCostMinor: 1800, contributionMarginMinor: 3200 }]);
  });

  it("uses JOD minor units and excludes deposits, collections, debt, purchases, and owner movements", () => {
    const result = calculateContributionMargin("2026-08-01", "2026-08-31", [order({ recognizedRevenueMinor: 10000, recognizedCostMinor: 6000 })], [fixedExpense({ amountMinor: 2000 })]);
  ...
  it("excludes a delivered non-final order with an explicit reason", () => {
    const result = calculateBreakEven("2026-08-01", "2026-08-31", [order({ resultStatus: "incomplete" })], [fixedExpense()]);
  ...
  it("marks estimated fixed cost as needs_review while preserving the number", () => {
    ...
    expect(result).toMatchObject({ status: "needs_review", breakEvenUnits: 1, assumptions: ["مبلغ ثابت اشتراك معلن تقديري معلن."] });
  });

  it("returns incomplete for mixed, unknown, or unlinked general variable expenses", () => {
    const mixed = calculateBreakEven("2026-08-01", "2026-08-31", [order()], [fixedExpense({ behavior: "mixed" })]);
    const unknown = calculateBreakEven("2026-08-01", "2026-08-31", [order()], [fixedExpense({ behavior: "unknown" })]);
    const variable = calculateBreakEven("2026-08-01", "2026-08-31", [order()], [fixedExpense({ behavior: "variable" })]);
  ...
  it("returns incomplete for a shared expense without a declared basis", () => {
    const result = calculateBreakEven("2026-08-01", "2026-08-31", [order()], [fixedExpense({ relationship: "shared", sharedProjectShareBasis: null })]);
  ...
  it("prevents the quantity-thousandths x1000 error", () => {
    const oneUnit = calculateBreakEven("2026-08-01", "2026-08-31", [order({ quantityMilli: 1000, recognizedRevenueMinor: 1000, recognizedCostMinor: 500 })], [fixedExpense({ amountMinor: 500 })]);
    const twelveUnits = calculateBreakEven("2026-08-01", "2026-08-31", [order({ quantityMilli: 12000, recognizedRevenueMinor: 12000, recognizedCostMinor: 6000 })], [fixedExpense({ amountMinor: 600 })]);
  ...
  it("does not combine incompatible quantity units into one break-even scale", () => {
    const result = calculateBreakEven("2026-08-01", "2026-08-31", [order(), order({ id: "order-2", unitKey: "kilogram", unitLabel: "كغ", quantityMilli: 1000, recognizedRevenueMinor: 3000, recognizedCostMinor: 1000 })], [fixedExpense()]);
    expect(result).toMatchObject({ status: "incomplete", totalQuantityMilli: null, contributionMarginPerUnitMinor: null, breakEvenUnits: null });
    expect(result.reasons.join(" ")).toContain("غير متوافقة");
  });

  it("rejects invalid quantity rather than treating it as zero", () => {
    const result = calculateBreakEven("2026-08-01", "2026-08-31", [order({ quantityMilli: null, quantityIssue: "invalid" })], [fixedExpense()]);
  ...
  it("allows a complete known projection and marks estimated declarations for review", () => {
    const complete = calculateShortCash(shortCash({ receivables: [{ id: "customer-1", direction: "collection", amountMinor: 8000, dueOn: null, source: "دين عميلة" }], declarations: [declaration({ relatedOrderId: "customer-1", relatedEventId: null })] }));
    const estimated = calculateShortCash(shortCash({ declarations: [declaration({ knowledge: "estimated" })] }));
  ...
  it("keeps dated supplier purchase commitments separate from contribution margin", () => {
    const result = calculateShortCash(shortCash({ payables: [{ id: "purchase-1", direction: "commitment", amountMinor: 14000, dueOn: "2026-08-22", source: "مشتريات مورد" }] }));
    expect(result).toMatchObject({ status: "available", declaredCommitmentsMinor: 14000, projectedCashMinor: -4000 });
  });

  it("returns incomplete when there is no short-horizon evidence even if cash is known", () => {
    const result = calculateShortCash(shortCash());
    expect(result).toMatchObject({ status: "incomplete", recordedCashMinor: 10000, projectedCashMinor: null });
  ...
  it("invalidates a linked declaration that exceeds the known balance", () => {
    const result = calculateShortCash(shortCash({ receivables: [{ id: "customer-1", direction: "collection", amountMinor: 1000, dueOn: null, source: "دين عميلة" }], declarations: [declaration({ amountMinor: 1001, relatedOrderId: "customer-1", relatedEventId: null })] }));
