...
describe("G4-B recurring margin Domain", () => {
  it("calculates each allowed allocation basis from explicit evidence only", () => {
    ...
    const perUnit = calculateAllocationPolicy(createAllocationPolicy({ ...base, id: "per-unit", idempotencyKey: "per-unit", kind: "per_output_unit", amountMinor: null, rateMinor: null, rateMinorPerWholeUnit: 50, percentageBps: null, unitId: "unit-piece" }), evidence);
    ...
    expect(perUnit).toMatchObject({ status: "known", amountMinor: 250, resultMinor: 3_750, calculationNote: expect.stringContaining("1.000") });
  ...
  it("uses the canonical whole-unit scale and preserves the legacy input alias", () => {
    const legacyInput = createAllocationPolicy({ ...base, kind: "per_output_unit", amountMinor: null, rateMinor: 50, percentageBps: null, unitId: "unit-piece" });
    const oneUnit = calculateAllocationPolicy(legacyInput, { ...evidence, outputQuantityMilli: 1_000, finalOrderIds: ["order-1"] });
  ...
  it("sums thousandths before one half-up rounding", () => {
    const policy = createAllocationPolicy({ ...base, kind: "per_output_unit", amountMinor: null, rateMinor: null, rateMinorPerWholeUnit: 50, percentageBps: null, unitId: "unit-piece" });
    const aggregated = calculateAllocationPolicy(policy, { ...evidence, outputQuantityMilli: 333 + 333 + 334 });
    ...
    expect(halfUp).toMatchObject({ status: "known", amountMinor: 51, calculationNote: expect.stringContaining("مرة واحدة") });
  });

  it("keeps zero after rounding as a known, explained calculation", () => {
    const policy = createAllocationPolicy({ ...base, kind: "per_output_unit", amountMinor: null, rateMinor: null, rateMinorPerWholeUnit: 1, percentageBps: null, unitId: "unit-piece" });
    const result = calculateAllocationPolicy(policy, { ...evidence, outputQuantityMilli: 1 });
    expect(result).toMatchObject({ status: "known", amountMinor: 0, resultMinor: 4_000, reasons: [expect.stringContaining("نتيجة حسابية معلنة")], calculationNote: expect.stringContaining("الصفر نتيجة حسابية") });
  });

  it("returns incomplete instead of a misleading number on safe-integer overflow", () => {
    const policy = createAllocationPolicy({ ...base, kind: "per_output_unit", amountMinor: null, rateMinor: null, rateMinorPerWholeUnit: 2, percentageBps: null, unitId: "unit-piece" });
    const result = calculateAllocationPolicy(policy, { ...evidence, outputQuantityMilli: Number.MAX_SAFE_INTEGER });
    expect(result).toMatchObject({ status: "incomplete", amountMinor: null, resultMinor: null, reasons: [expect.stringContaining("الدقة الآمنة")] });
  });
