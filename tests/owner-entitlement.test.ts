...
function recordInput(overrides: Partial<Parameters<typeof createOwnerEntitlementRecord>[0]> = {}) {
  return {
    id: "entitlement-1",
    policyId: "policy-1",
    policyVersion: 1,
    ...
    calculationBasis: "time_period" as const,
    baseMinor: null,
    quantity: null,
    sourceKeys: ["period:policy-1:2026-08-01:2026-08-31"],
    note: "آب",
...
describe("owner entitlement Domain", () => {
  it("keeps dated policies immutable and calculates a fixed period amount", () => {
    ...
    ).toMatchObject({ amountMinor: 1500, knowledge: "known", calculationBasis: "time_period" });
  });
  ...
  it("requires complete local periods for monthly, weekly, and daily policies", () => {
    ...
    expect(
      calculateOwnerEntitlement(monthly, { periodFrom: "2026-08-15", periodTo: "2026-08-31" }),
    ).toMatchObject({ amountMinor: null, knowledge: "incomplete" });
    const bounded = createOwnerEntitlementPolicy({
      ...basePolicy,
      id: "bounded",
      endsOn: "2026-08-25",
    ...
    expect(
      calculateOwnerEntitlement(bounded, { periodFrom: "2026-08-01", periodTo: "2026-08-31" }),
    ).toMatchObject({ amountMinor: null, knowledge: "incomplete" });
    ...
    ).toMatchObject({ amountMinor: 1500, knowledge: "known" });
    expect(
      calculateOwnerEntitlement(weekly, { periodFrom: "2026-08-03", periodTo: "2026-08-08" }),
    ).toMatchObject({ amountMinor: null, knowledge: "incomplete" });
    ...
    expect(
      calculateOwnerEntitlement(daily, { periodFrom: "2026-08-03", periodTo: "2026-08-03" }),
    ).toMatchObject({ amountMinor: 1500, knowledge: "known" });
    expect(
      calculateOwnerEntitlement(daily, { periodFrom: "2026-08-03", periodTo: "2026-08-04" }),
    ).toMatchObject({ amountMinor: null, knowledge: "incomplete" });
  });
  ...
  it("does not turn missing hourly time, units, or sale sources into zero", () => {
    const hourly = createOwnerEntitlementPolicy({
      ...
      kind: "hourly",
      amountMinor: 600,
      idempotencyKey: "hourly",
    });
    expect(
      calculateOwnerEntitlement(hourly, { periodFrom: "2026-08-01", periodTo: "2026-08-31" }),
    ).toMatchObject({ amountMinor: null, knowledge: "incomplete" });
    ...
    expect(
      calculateOwnerEntitlement(unit, { periodFrom: "2026-08-01", periodTo: "2026-08-31", unitQuantity: 4 }),
    ).toMatchObject({ amountMinor: null, knowledge: "incomplete" });
    expect(
      calculateOwnerEntitlement(unit, {
        periodFrom: "2026-08-01",
        periodTo: "2026-08-31",
        unitQuantity: 4,
        unitSourceKeys: ["order-1"],
      }),
    ).toMatchObject({ amountMinor: 100, quantity: 4 });
    const sale = createOwnerEntitlementPolicy({
      ...basePolicy,
      id: "sale",
      family: "completed_sale_percentage",
      kind: "sale_percentage",
    ...
    expect(
      calculateOwnerEntitlement(sale, {
        periodFrom: "2026-08-01",
        periodTo: "2026-08-31",
        completedSaleMinor: 8000,
      }),
    ).toMatchObject({ amountMinor: null, knowledge: "incomplete" });
  });

  it("uses only a recorded G3 result for profit share", () => {
    const profit = createOwnerEntitlementPolicy({
      ...basePolicy,
      id: "profit",
      family: "profit_share",
      kind: "profit_share",
      amountMinor: null,
      percentageBps: 2500,
      idempotencyKey: "profit",
    });
    expect(
      calculateOwnerEntitlement(profit, {
        periodFrom: "2026-08-01",
        periodTo: "2026-08-31",
        recognizedProfitMinor: 1000,
    ...
    ).toMatchObject({ amountMinor: 250, knowledge: "known" });
  });

  it("does not claim a fixed-shift amount without shift evidence", () => {
    const shift = createOwnerEntitlementPolicy({
      ...basePolicy,
      id: "shift",
      family: "fixed_amount",
      kind: "fixed_shift",
      amountMinor: 1200,
      idempotencyKey: "shift",
    });
    expect(
      calculateOwnerEntitlement(shift, { periodFrom: "2026-08-01", periodTo: "2026-08-31" }),
    ).toMatchObject({ amountMinor: null, knowledge: "incomplete" });
  });
  ...
  it("separates entitlement settlement, owner draw, capital return, and prior-draw return", () => {
    const settlement = createOwnerMovement({
      ...
      relatedEntitlementId: "entitlement-1",
    });
    expect(settlement).toMatchObject({
      cashDeltaMinor: -400,
      entitlementDeltaMinor: -400,
      openingBalanceDeltaMinor: 0,
      ownerCapitalDeltaMinor: 0,
    });
    const ownerDraw = createOwnerMovement({
      id: "draw-2",
      kind: "draw",
      amountMinor: 200,
      walletId: "wallet-1",
      ...
      reason: "owner_draw",
      note: "سحب شخصي",
      idempotencyKey: "draw-2",
    });
    expect(ownerDraw).toMatchObject({
      cashDeltaMinor: -200,
      entitlementDeltaMinor: 0,
      ownerCapitalDeltaMinor: -200,
    });
    const returnMovement = createOwnerMovement({
      id: "return-1",
      kind: "return",
      amountMinor: 100,
      walletId: "wallet-1",
      ...
      relatedMovementId: "draw-1",
    });
    expect(returnMovement).toMatchObject({
      cashDeltaMinor: 100,
      entitlementDeltaMinor: 100,
      ownerCapitalDeltaMinor: 0,
    });
    const openingDraw = createOwnerMovement({
      ...
      note: "تسوية افتتاح موجب",
      idempotencyKey: "opening-draw",
      relatedOpeningBalanceId: "opening-1",
    });
  ...
  it("keeps reversals append-only and restores the source eligibility", () => {
    ...
    ).toMatchObject({ reversalOfId: "draw-3", cashDeltaMinor: 400, entitlementDeltaMinor: 400 });
  });
