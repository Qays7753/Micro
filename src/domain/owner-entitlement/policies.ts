...
function localDayNumber(value: string) { return Date.UTC(Number(value.slice(0, 4)), Number(value.slice(5, 7)) - 1, Number(value.slice(8, 10))); }
function inclusiveDays(from: string, to: string) { return Math.round((localDayNumber(to) - localDayNumber(from)) / 86_400_000) + 1; }
function lastDayOfMonth(value: string) { const year = Number(value.slice(0, 4)); const month = Number(value.slice(5, 7)); return `${value.slice(0, 7)}-${String(new Date(Date.UTC(year, month, 0)).getUTCDate()).padStart(2, "0")}`; }
function isFullCalendarMonth(from: string, to: string) { return from.endsWith("-01") && to === lastDayOfMonth(from); }
...
export function createOwnerEntitlementPolicy(input: CreateOwnerEntitlementPolicyInput): OwnerEntitlementPolicy {
  ...
  if (input.kind === "fixed_period" && input.endsOn === null) throw new Error("fixed_period policy requires an explicit endsOn");
...
export function createOwnerEntitlementRecord(input: OwnerEntitlementRecord): OwnerEntitlementRecord {
  ...
  const keys = sourceKeys(input.sourceKeys, "sourceKeys"); const reversalOfId = input.reversalOfId?.trim() || null;
  if (reversalOfId === input.id) throw new Error("record cannot reverse itself");
  if (reversalOfId === null && input.reversalReason !== null && input.reversalReason !== undefined) throw new Error("original record cannot have a reversal reason");
  if (reversalOfId !== null) nonBlank(input.reversalReason ?? "", "reversalReason");
  iso(input.recordedAt, "recordedAt");
  return Object.freeze({ ...input, sourceKeys: keys, reversalOfId, reversalReason: input.reversalReason?.trim() || null, note: input.note.trim() });
}

export function createOwnerEntitlementRecordReversal(input: OwnerEntitlementRecordReversalInput): OwnerEntitlementRecord {
  ...
  return Object.freeze({ ...input.source, id: input.id, occurredOn: input.occurredOn, recordedAt: input.recordedAt, idempotencyKey: input.idempotencyKey, note: `عكس: ${input.source.note}`, reversalOfId: input.source.id, reversalReason: input.reason.trim() });
}
...
export function createOwnerEntitlementOpeningBalance(input: OwnerEntitlementOpeningBalance): OwnerEntitlementOpeningBalance {
  ...
  if (!Number.isInteger(input.amountMinor) || input.amountMinor === 0) throw new Error("opening balance must be a non-zero integer");
  ...
  if (reversalOfId === null && input.reversalReason !== null && input.reversalReason !== undefined) throw new Error("original opening balance cannot have a reversal reason");
  if (reversalOfId !== null) nonBlank(input.reversalReason ?? "", "reversalReason");
  return Object.freeze({ ...input, reversalOfId, reversalReason: input.reversalReason?.trim() || null, reason: input.reason.trim(), note: input.note.trim() });
}

export function createOwnerEntitlementOpeningBalanceReversal(input: OwnerEntitlementOpeningBalanceReversalInput): OwnerEntitlementOpeningBalance {
  nonBlank(input.id, "id"); nonBlank(input.reason, "reason"); nonBlank(input.idempotencyKey, "idempotencyKey"); date(input.occurredOn, "occurredOn"); iso(input.recordedAt, "recordedAt");
  if (input.source.reversalOfId) throw new Error("cannot reverse a reversal");
  return Object.freeze({ ...input.source, id: input.id, occurredOn: input.occurredOn, recordedAt: input.recordedAt, idempotencyKey: input.idempotencyKey, note: `عكس: ${input.source.note}`, reversalOfId: input.source.id, reversalReason: input.reason.trim() });
}
...
export function createOwnerMovement(input: CreateOwnerMovementInput): OwnerMovement {
  ...
  const relatedEntitlementId = input.relatedEntitlementId?.trim() || null; const relatedOpeningBalanceId = input.relatedOpeningBalanceId?.trim() || null; const relatedMovementId = input.relatedMovementId?.trim() || null;
  ...
  if (input.reason === "opening_balance_settlement" && !relatedOpeningBalanceId) throw new Error("opening balance settlement requires relatedOpeningBalanceId");
  if (input.reason !== "opening_balance_settlement" && relatedOpeningBalanceId) throw new Error("only opening balance settlement may reference relatedOpeningBalanceId");
  ...
  const openingBalanceDeltaMinor = input.reason === "opening_balance_settlement" ? (input.kind === "draw" ? -input.amountMinor : input.amountMinor) : 0;
...
export type OwnerEntitlementCalculation = { amountMinor: number | null; knowledge: OwnerEntitlementKnowledge; baseMinor: number | null; quantity: number | null; calculationBasis: OwnerEntitlementPolicyFamily; sourceKeys: readonly string[]; nextAction: string };

export function isPolicyEffective(policy: OwnerEntitlementPolicy, from: string, to: string): boolean { return policy.status === "active" && policy.startsOn <= from && (policy.endsOn === null || policy.endsOn >= to); }
export function calculateOwnerEntitlement(policy: OwnerEntitlementPolicy, evidence: OwnerEntitlementEvidence): OwnerEntitlementCalculation {
  ...
  if (!isPolicyEffective(policy, evidence.periodFrom, evidence.periodTo)) return { amountMinor: null, knowledge: "incomplete", baseMinor: null, quantity: null, calculationBasis: policy.family, sourceKeys: [], nextAction: "اجعل الفترة بعد تاريخ بدء السياسة وقبل إيقافها، أو أنشئ نسخة سياسة جديدة من تاريخ صحيح." };
  const amount = policy.amountMinor ?? 0;
  if (policy.kind === "fixed_shift") return { amountMinor: null, knowledge: "incomplete", baseMinor: null, quantity: null, calculationBasis: "fixed_amount", sourceKeys: [], nextAction: "لا يوجد في النموذج الحالي سجل ورديات موثق؛ استخدم مبلغًا للفترة أو أكمل دليل الوردية قبل اعتماد الاستحقاق." };
  if (policy.kind === "monthly" && !isFullCalendarMonth(evidence.periodFrom, evidence.periodTo)) return { amountMinor: null, knowledge: "incomplete", baseMinor: null, quantity: null, calculationBasis: "time_period", sourceKeys: [], nextAction: "السياسة الشهرية تحتاج شهرًا تقويميًا كاملًا؛ لا يسجل النظام مبلغ الشهر عن فترة قصيرة أو جزئية." };
  if (policy.kind === "weekly" && inclusiveDays(evidence.periodFrom, evidence.periodTo) !== 7) return { amountMinor: null, knowledge: "incomplete", baseMinor: null, quantity: null, calculationBasis: "time_period", sourceKeys: [], nextAction: "السياسة الأسبوعية تحتاج سبعة أيام متصلة كاملة؛ اختر فترة أسبوع واضحة ولا يسجل النظام أسبوعًا جزئيًا." };
  if (policy.kind === "daily" && evidence.periodFrom !== evidence.periodTo) return { amountMinor: null, knowledge: "incomplete", baseMinor: null, quantity: null, calculationBasis: "time_period", sourceKeys: [], nextAction: "السياسة اليومية تحتاج يومًا محليًا واحدًا فقط." };
  if (policy.kind === "fixed_period" && (policy.endsOn === null || evidence.periodFrom !== policy.startsOn || evidence.periodTo !== policy.endsOn)) return { amountMinor: null, knowledge: "incomplete", baseMinor: null, quantity: null, calculationBasis: "fixed_amount", sourceKeys: [], nextAction: "المبلغ الثابت يحتاج النطاق المعلن كاملًا في السياسة؛ لا تسجل فترة متداخلة أو أقصر بلا نسخة سياسة جديدة." };
  if (policy.kind === "hourly") {
    const minutes = evidence.timeQuantity ?? null; const keys = evidence.timeSourceKeys ?? []; if (minutes === null || !Number.isInteger(minutes) || minutes <= 0 || keys.length === 0) return { amountMinor: null, knowledge: "incomplete", baseMinor: null, quantity: null, calculationBasis: "time_period", sourceKeys: [], nextAction: "سجل مدة العمل بالدقائق مع مراجع سجل الوقت قبل اعتماد الاستحقاق بالساعة؛ لا يتحول الوقت المجهول إلى صفر." };
    return { amountMinor: Math.floor((amount * minutes) / 60), knowledge: "known", baseMinor: minutes, quantity: minutes, calculationBasis: "time_period", sourceKeys: keys, nextAction: "راجع مصدر الدقائق قبل الاعتماد." };
  }
  if (policy.kind === "per_completed_work") {
    const quantity = evidence.completedWorkCount ?? null; if (quantity === null || !Number.isInteger(quantity) || quantity <= 0) return { amountMinor: null, knowledge: "incomplete", baseMinor: null, quantity: null, calculationBasis: "completed_work", sourceKeys: [], nextAction: "أكمل عدد الطلبات أو الخدمات المكتملة والمعترف بإيرادها." };
    const keys = evidence.completedWorkKeys ?? []; if (keys.length !== quantity || new Set(keys).size !== keys.length) return { amountMinor: null, knowledge: "incomplete", baseMinor: null, quantity: null, calculationBasis: "completed_work", sourceKeys: [], nextAction: "احفظ مراجع الأعمال المكتملة حتى لا يعاد احتساب الطلب نفسه." };
    return { amountMinor: amount * quantity, knowledge: "known", baseMinor: amount, quantity, calculationBasis: "completed_work", sourceKeys: keys, nextAction: "راجع أن كل عمل محسوب نهائي ومكتمل، لا مسودة أو عربون." };
  }
  if (policy.kind === "per_unit") {
    const quantity = evidence.unitQuantity ?? null; if (quantity === null || !Number.isFinite(quantity) || quantity <= 0) return { amountMinor: null, knowledge: "incomplete", baseMinor: null, quantity: null, calculationBasis: "unit", sourceKeys: [], nextAction: "سجل كمية الوحدات المكتملة ووحدتها قبل الاعتماد؛ لا تخترع عددًا." };
    const keys = evidence.unitSourceKeys ?? []; if (keys.length === 0) return { amountMinor: null, knowledge: "incomplete", baseMinor: null, quantity: null, calculationBasis: "unit", sourceKeys: [], nextAction: "احفظ مراجع الوحدات أو العمل المكتمل حتى لا يعاد احتساب المصدر نفسه." };
    return { amountMinor: Math.floor(amount * quantity), knowledge: "known", baseMinor: amount, quantity, calculationBasis: "unit", sourceKeys: keys, nextAction: "راجع أن الوحدة والكمية صالحتان وموجودتان في السجل." };
  }
  if (policy.kind === "profit_share") {
    const base = evidence.recognizedProfitMinor ?? null; if (base === null || evidence.recognizedProfitStatus === "invalid") return { amountMinor: null, knowledge: "incomplete", baseMinor: base, quantity: null, calculationBasis: "profit_share", sourceKeys: [], nextAction: "أغلق الفترة وتحقق من قراءة G3 المسجلة الصحيحة؛ لا تحسب النسبة من الكاش أو المبيعات الخام." };
    if (evidence.recognizedProfitStatus !== "recorded_only") return { amountMinor: null, knowledge: "incomplete", baseMinor: base, quantity: null, calculationBasis: "profit_share", sourceKeys: [], nextAction: "راجع أسباب نقص نتيجة G3 قبل تسجيل نسبة الاستحقاق؛ لا تعرض دقة كاذبة." };
    const share = Math.floor((base * (policy.percentageBps ?? 0) + 5_000) / 10_000); if (share <= 0) return { amountMinor: null, knowledge: "incomplete", baseMinor: base, quantity: null, calculationBasis: "profit_share", sourceKeys: evidence.recognizedProfitKeys ?? [], nextAction: "راجع النسبة أو أساس الربح؛ لا يسجل استحقاق صفري من نسبة موجبة." };
    return { amountMinor: share, knowledge: "known", baseMinor: base, quantity: null, calculationBasis: "profit_share", sourceKeys: evidence.recognizedProfitKeys ?? [`g3:${evidence.periodFrom}:${evidence.periodTo}`], nextAction: "راجع فترة G3 ومصدرها قبل الاعتماد." };
  }
  if (policy.kind === "sale_percentage") {
    const base = evidence.completedSaleMinor ?? null; if (base === null || !Number.isInteger(base) || base <= 0) return { amountMinor: null, knowledge: "incomplete", baseMinor: base, quantity: null, calculationBasis: "completed_sale_percentage", sourceKeys: [], nextAction: "أكمل قيمة البيع المكتمل والمعترف بإيراده؛ لا تحسب من العربون أو الدين غير المعترف به." };
    const keys = evidence.completedSaleKeys ?? []; if (keys.length === 0) return { amountMinor: null, knowledge: "incomplete", baseMinor: base, quantity: null, calculationBasis: "completed_sale_percentage", sourceKeys: [], nextAction: "احفظ مراجع البيوع المكتملة حتى لا يعاد احتساب البيع نفسه." };
    const share = Math.floor((base * (policy.percentageBps ?? 0) + 5_000) / 10_000); if (share <= 0) return { amountMinor: null, knowledge: "incomplete", baseMinor: base, quantity: null, calculationBasis: "completed_sale_percentage", sourceKeys: keys, nextAction: "راجع قيمة البيع أو النسبة؛ لا يسجل استحقاق صفري من نسبة موجبة." };
    return { amountMinor: share, knowledge: "known", baseMinor: base, quantity: null, calculationBasis: "completed_sale_percentage", sourceKeys: keys, nextAction: "راجع أن البيع مكتمل ومعترف بإيراده، لا عربونًا أو دينًا." };
  }
  return { amountMinor: amount, knowledge: "known", baseMinor: null, quantity: null, calculationBasis: policy.family === "fixed_amount" ? "fixed_amount" : "time_period", sourceKeys: [], nextAction: "راجع الفترة والسياسة المصدر قبل الاعتماد." };
}
...
export function isValidOwnerMovement(value: unknown): value is OwnerMovement { try { if (!value || typeof value !== "object") return false; const candidate = value as OwnerMovement; if (candidate.reversalOfId !== null && (typeof candidate.reversalOfId !== "string" || candidate.reversalOfId.trim().length === 0 || candidate.reversalOfId === candidate.id)) return false; const expected = createOwnerMovement({ id: candidate.id, kind: candidate.kind, amountMinor: candidate.amountMinor, walletId: candidate.walletId, occurredOn: candidate.occurredOn, recordedAt: candidate.recordedAt, reason: candidate.reason, note: candidate.note, idempotencyKey: candidate.idempotencyKey, relatedEntitlementId: candidate.relatedEntitlementId, relatedOpeningBalanceId: candidate.relatedOpeningBalanceId, relatedMovementId: candidate.relatedMovementId }); if (candidate.reversalOfId === null) return candidate.reversalReason === null && candidate.cashDeltaMinor === expected.cashDeltaMinor && candidate.entitlementDeltaMinor === expected.entitlementDeltaMinor && candidate.openingBalanceDeltaMinor === expected.openingBalanceDeltaMinor && candidate.ownerCapitalDeltaMinor === expected.ownerCapitalDeltaMinor; if (!candidate.reversalReason || candidate.reversalReason.trim().length === 0) return false; return candidate.cashDeltaMinor === -expected.cashDeltaMinor && candidate.entitlementDeltaMinor === -expected.entitlementDeltaMinor && candidate.openingBalanceDeltaMinor === -expected.openingBalanceDeltaMinor && candidate.ownerCapitalDeltaMinor === -expected.ownerCapitalDeltaMinor; } catch { return false; } }
