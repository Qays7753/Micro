import { allocationPolicyKinds, type AllocationEvidence, type AllocationKnowledge, type AllocationPolicy, type AllocationPolicyKind, type AllocationCalculation, type CreateAllocationPolicyInput, type AllocationPolicyTerms, type WasteContext } from "./types.js";

const required = (value: string, message: string) => { if (!value.trim()) throw new Error(message); return value.trim(); };
const localDate = (value: string, label: string) => { if (!/^\d{4}-\d{2}-\d{2}$/.test(value) || Number.isNaN(new Date(`${value}T12:00:00.000Z`).getTime()) || new Date(`${value}T12:00:00.000Z`).toISOString().slice(0, 10) !== value) throw new Error(`${label} غير صالح.`); return value; };
const positiveMinor = (value: number | null, label: string) => { if (!Number.isSafeInteger(value) || value === null || value <= 0) throw new Error(`${label} يجب أن يكون مبلغًا موجبًا بوحدة JOD minor.`); return value; };
const positiveInteger = (value: number | null, label: string) => { if (!Number.isSafeInteger(value) || value === null || value <= 0) throw new Error(`${label} يجب أن يكون عددًا صحيحًا موجبًا.`); return value; };
const percentage = (value: number | null) => { if (!Number.isSafeInteger(value) || value === null || value < 1 || value > 10_000) throw new Error("النسبة يجب أن تكون بين 0.01% و100% بوحدة basis points."); return value; };
const validKind = (value: AllocationPolicyKind) => { if (!allocationPolicyKinds.includes(value)) throw new Error("أساس التحميل غير مدعوم."); return value; };
...
export function createAllocationPolicy(input: CreateAllocationPolicyInput): AllocationPolicy {
  ...
  if (!dateBefore(periodFrom, periodTo) || !dateBefore(startsOn, endsOn ?? "9999-12-31")) throw new Error("نطاق السياسة أو تاريخ نفاذها غير مرتب.");
...
export function createAllocationPolicySuccessor(previous: AllocationPolicy, terms: AllocationPolicyTerms & { id: string; seriesId: string; successorOfPolicyId: string; version: number; status: "active"; idempotencyKey: string; createdAt: string; updatedAt: string }): AllocationPolicy {
  ...
  if (terms.seriesId !== previous.seriesId || terms.successorOfPolicyId !== previous.id || terms.version !== previous.version + 1) throw new Error("سلسلة مراجعة سياسة التحميل غير متصلة.");
  if (terms.startsOn <= previous.startsOn || (previous.endsOn !== null && terms.startsOn !== dayAfter(previous.endsOn))) throw new Error("تاريخ نفاذ خليفة سياسة التحميل يجب أن يتبع نهاية النسخة السابقة مباشرة.");
  return createAllocationPolicy(terms);
}

export function isAllocationPolicyEffective(policy: AllocationPolicy, catalogItemId: string, from: string, to: string): boolean {
...
export function calculateAllocationPolicy(policy: AllocationPolicy, evidence: AllocationEvidence): AllocationCalculation {
  ...
  if (evidence.finalOrderIds.length === 0) return incomplete(policy, evidence, ["لا توجد طلبات final مرتبطة صراحة بهذا المرجع في الفترة."], "سجل الطلبات النهائية المرتبطة أو راجع نطاق الفترة قبل قراءة التحميل.");
  ...
  if (policy.kind === "actual_time") {
    if (policy.rateMinor === null || evidence.actualTimeMinutes === null || evidence.missingTimeOrderIds.length > 0) reasons.push("أكمل تسجيل وقت فعلي صالح لكل الطلبات الداخلة قبل الاعتماد على التحميل.");
    else {
      const calculated = policy.rateMinor * evidence.actualTimeMinutes;
      if (!Number.isSafeInteger(calculated) || calculated <= 0) reasons.push("معدل أو دقائق الوقت تتجاوز الدقة الآمنة."); else amountMinor = calculated;
    }
  }
  if (policy.kind === "completed_revenue_percentage") {
    if (policy.percentageBps === null || evidence.recognizedRevenueMinor === null || evidence.missingRevenueOrderIds.length > 0) reasons.push("أكمل الإيراد final/المعترف به للطلبات الداخلة قبل تطبيق نسبة التحميل؛ توجد مبيعات ناقصة أو غير مكتملة.");
    else {
      const calculated = Math.floor((evidence.recognizedRevenueMinor * policy.percentageBps + 5_000) / 10_000);
      if (!Number.isSafeInteger(calculated) || calculated <= 0) reasons.push("النسبة المعلنة لم تنتج مبلغ تحميل موجبًا من الإيراد المكتمل."); else amountMinor = calculated;
    }
  }
...
export function isValidWasteContext(value: unknown): value is WasteContext {
  ...
  if (context.kind === "order" || context.kind === "catalog_item") return typeof context[context.kind === "order" ? "orderId" : "catalogItemId"] === "string" && String(context[context.kind === "order" ? "orderId" : "catalogItemId"]).trim().length > 0;
