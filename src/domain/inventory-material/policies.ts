...
export function createInventoryMovement(input: CreateInventoryMovementInput): InventoryMovement {
  ...
  if (quantityDeltaMilli === 0 || valueDeltaMinor === 0)
    throw new Error("كمية وقيمة حركة المادة يجب أن تكونا غير صفريتين.");
...
export function consumptionValueMinor(quantityMilli: number, position: MaterialInventoryPosition): number {
