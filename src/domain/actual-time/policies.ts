...
export function reverseActualTimeRecord(
  ...
  required(input.id, "معرف عكس الوقت مطلوب.");
  required(input.recordedOn, "تاريخ العكس مطلوب.");
  required(input.createdAt, "وقت إنشاء العكس مطلوب.");
  required(input.operationKey, "مفتاح عملية العكس مطلوب.");
  required(input.reason, "عكس سجل الوقت يحتاج سببًا واضحًا.");
  if (input.target.reversalOfId !== null || input.target.minutesDelta <= 0)
    throw new Error("لا يمكن عكس سجل عكس أو سجل وقت غير صالح.");
  if (existing.some(record => record.reversalOfId === input.target.id))
    throw new Error("تم عكس سجل الوقت هذا سابقًا.");
  return {
    id: input.id,
    ...
    note: `عكس: ${input.target.note ?? "سجل وقت"}`,
