...
>
> **حالة الدمج الحالية:** PWA مدمجة على `main` عبر PR #54. شريحة G5 موجودة على فرع `g5/contribution-break-even-short-cash` وتحت PR #53، لكنها **لم تندمج بعد** في `main`.
...
**لا تبدأ أي مجموعة أو قدرة مالية/بنية جديدة الآن.** اكتملت G4-B، وأضيفت PWA التشغيلية المصرح بها كطبقة نشر ثابتة فقط، وهي مدمجة على `main` عبر PR #54. أما G5 فموجودة على فرعها وتحت PR #53 للمراجعة ولم تندمج بعد؛ لا تُعامل كقدرة منفذة على `main` ولا تنتقل إلى POS أو مزامنة أو Auth أو أي معنى سحابي قبل طلب صريح جديد من مالك المنتج. لا توسّع معنى الوقت الفعلي إلى أجر أو تكلفة أو توصية سعر من دون عقد مستقل.
...
عند دمج شريحة جديدة، يعدل الوكيل هذا الملف في نفس PR ليحدث: commit/PR، جدول «ما هو منفذ»، جدول «الحالة المتوقفة عمدًا»، إصدارات schema/export إن تغيرت، ومسار القراءة التالي. PWA مدمجة على `main` عبر PR #54؛ وتظل G5 في PR #53 غير مدمجة حتى اكتمال المراجعة والقبول. في PR #54 لا تتغير schema/export؛ ويظل الاختبار الفعلي على Android/iOS وCloudflare Pages قبل إعلان قبول الإنتاج. ثم يحدث `todo.md` فقط بما يطابق الواقع المندمج.
