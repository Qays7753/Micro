...
| A2 عرض المال والتاريخ | قيد المراجعة على فرع الإصلاح الحالي: طبقة عرض مشتركة لمبالغ minor بوحدات JOD مع أرقام ASCII واتجاه LTR، وتواريخ ظاهرة `DD/MM/YYYY` وأوقات/كميات معزولة؛ بعد الدمج لا يغير قيمة التخزين أو الحساب أو الإدخال ISO ولا Domain أو CostSnapshot أو schema/export. |
...
إصلاح A2 في فرع المراجعة الحالي كطبقة عرض مشتركة فوق الأسطح الحالية: `formatMoneyMinor` و`formatLocalDate` و`formatLocalDateTime` و`formatMonthLabel` و`formatQuantityMilli`، مع مكونات عرض تعزل القيم بصريًا في RTL. الإدخالات تبقى ASCII/ISO، والحسابات والخدمات والعقود المالية لم تتغير. دليل القبول الخاص بالشريحة يثبت الاختبارات الآلية وفحص السطوح؛ لا يعني ذلك قبولًا إنتاجيًا متعدد الأجهزة، ولا يصبح جزءًا من قدرات `main` إلا بعد PR وCI والدمج.
...
عند إغلاق/دمج شريحة جديدة، يعدل الوكيل هذا الملف في نفس PR ليحدث: commit/PR، جدول «ما هو منفذ»، جدول «الحالة المتوقفة عمدًا»، إصدارات schema/export إن تغيرت، ومسار القراءة التالي. PWA مدمجة على `main` عبر PR #54 وG5 عبر PR #53 وG6-B/G7-A عبر PR #60، وA1 عبر PR #70. إصلاح A2 على الفرع الحالي ولم يُفتح PR له بعد؛ لا تبدأ A3 أو B1 قبل دمج A2 بفرع وPR مستقلين. يظل اختبار iOS الفعلي مطلوبًا قبل إعلان قبول PWA الإنتاجي متعدد المنصات. ثم يحدث `todo.md` فقط بما يطابق الواقع المندمج.
