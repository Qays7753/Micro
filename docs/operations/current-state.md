...
| حساب المستخدم/السحابة | غير منفذين؛ لا مزامنة أو PWA أو Auth أو بيانات حقيقية |
...
عند دمج شريحة جديدة، يعدل الوكيل هذا الملف في نفس PR ليحدث: commit/PR، جدول «ما هو منفذ»، جدول «الحالة المتوقفة عمدًا»، إصدارات schema/export إن تغيرت، ومسار القراءة التالي. ثم يحدث `todo.md` فقط بما يطابق الواقع المندمج.
