...
|---|---|
| `main` | يحتوي الحزمة المالية عبر PR #115، وإصلاحات First-use عبر #118، وG18 عبر #119، وB-02 عبر #120، وبطاقات مراجعة النظام المعتمدة عبر #121. جميعها مراجع دمج تاريخية؛ رأس `main` الحالي يتحقق دائمًا عبر `git fetch origin --prune`. |
...
| برامج UX السابقة | برنامج إصلاح القرار البصري V1–V6 وتصحيحه مدمج على `main` عبر PR #96. وإصلاحات First-use مدمجة عبر #118، ثم G18 للمسارات العميقة والتاريخ عبر #119؛ لا تغير هذه الشرائح معنى المال أو Schema أو Export/Import. |
...
شرائح UX G19 ثم G20 ثم G21 ثم G22 ثم G23 ما زالت سلسلة مستقلة عن التوسعة. قبل فتح عمل جديد، افحص GitHub والرأس الحي؛ لا يثبت هذا السجل أن PR محلية أو غير مرفوعة اندمجت. تظل اختبارات الجهاز الفعلي وPWA وOffline الإنتاجي وPilot البشري حدود قبول مستقلة.

## 9. G19 — Order clarity and incomplete cost truth قيد المراجعة

بدأت G19 من رأس `origin/main` عند `9fb1b15d98ba6a04363bb95ce0fb3e64c19c49aa` على الفرع المستقل `agent-ux/g19-order-clarity-cost-truth`. هذه الشريحة تعالج M-08 وM-09 فقط، مع تحقق محدود من M-10 وM-05؛ الكود غير مدمج في `main` حتى الآن.
...
| الحالة التالية | بعد تصحيح قبول G19 المحدود، يعاد تثبيت الفحوص وQA الحي ثم تحديث PR الواحدة وانتظار CI وCloudflare Pages وقبول مستقل؛ لا دمج ذاتي ولا بدء G20. |
