...
| B1 حماية المدخلات غير المحفوظة | قيد المراجعة على فرع B1: حارس مشترك في `CostEditor` و`AgreementEditor` و`ScheduleEditor` لتمييز dirty/no-dirty، وعرض «احفظ واستمر»/«اخرج دون حفظ»/«إلغاء» قبل الخروج الداخلي؛ لا يدّعي اعتراض browser back أو tab close. |
