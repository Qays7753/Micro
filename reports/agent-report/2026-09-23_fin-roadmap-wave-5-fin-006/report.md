...
5. **Entry-bundle discipline**: the service lives in the app-root context (entry chunk); the additions initially pushed the entry past the D-034 hard gate. The first PR head (d96a8b0) failed CI **RAW_OVER — 650,030/650,000 (GitHub Actions run 35794222223)** even though the local build measured 649,919 PASS. **Root cause (not nondeterminism):** CI injects `__MICRO_APP_IDENTITY__` from `GITHUB_SHA` (40-char value + kept identity branches ≈ +112 bytes), while local builds without `GITHUB_SHA` fall back to `null` and minifier-fold those branches away — local null-identity builds under-report the entry by ~112 bytes. **Verified locally:** building the same failed head with `GITHUB_SHA` set reproduces CI byte-exactly (650,031 vs 650,030). **Fix (behavior-preserving compaction, no gate weakened):** the FIN-006 inventory drops the unused `{ stored, deliveredOn }` wrapper (deliveredOn was never read after period filtering) and 22 redundant `.stored` member accesses across the single-pass classification — identity keying, final/estimated/incomplete separation, visible exclusions, and all output values are unchanged (272/272 finance tests + full suites green). **CI-parity measurement: 649,720/650,000 raw (headroom 280) + 154,349/155,000 gzip.** Final authority remains the PR's GitHub Actions + Cloudflare Pages checks, not local numbers.
...
- Full CI-equivalent pipeline green after the compaction fix: typecheck (root + workspace); lint 0 errors / **37 warnings (budget 37)**; prettier; **text-density all caps — Catalog 93/93 unchanged (collapsed-details rule)**; design-guards; guards; root suite **465/465**; prototype suite **263 files / 1865/1865** (+8); build + PWA; bundle **649,608/650,000 raw + 154,355/155,000 gzip local (null identity)** and **649,720/650,000 raw + 154,349/155,000 gzip CI-parity (GITHUB_SHA set)** — PASS. CI/Cloudflare on the PR head is the closing authority.
